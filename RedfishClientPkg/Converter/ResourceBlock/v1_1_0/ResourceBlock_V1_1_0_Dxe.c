...
//
#include <RedfishJsonStructure/ResourceBlock/v1_1_0/EfiResourceBlockV1_1_0.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_ResourceBlock_V1_1_0_To_CS (ResoruceRaw, &ResourceBlockV1_1_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ResourceBlockV1_1_0;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ResourceBlock_V1_1_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ResourceBlock_V1_1_0_JSON (*((EFI_REDFISH_RESOURCEBLOCK_V1_1_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ResourceBlock_V1_1_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyResourceBlock_V1_1_0_Json (ResoruceRaw);
}
