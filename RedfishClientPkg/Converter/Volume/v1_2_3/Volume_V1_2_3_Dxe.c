...
//
#include <RedfishJsonStructure/Volume/v1_2_3/EfiVolumeV1_2_3.h>
CHAR8 mResourceTypeStr [] = "#Volume";
BOOLEAN IsRevisonController = TRUE;

// Support Volume V1_2_3
...
{
  EFI_REDFISH_VOLUME_V1_2_3 *VolumeV1_2_3;
  EFI_REDFISH_VOLUME_V1_2_3_CS *VolumeV1_2_3Cs;
  ...
  Status = (EFI_STATUS)Json_Volume_V1_2_3_To_CS (ResoruceRaw, &VolumeV1_2_3Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  VolumeV1_2_3 = (EFI_REDFISH_VOLUME_V1_2_3 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_VOLUME_V1_2_3));
  if (VolumeV1_2_3 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)VolumeV1_2_3;
  VolumeV1_2_3->Volume = VolumeV1_2_3Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Volume"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Volume"), "Volume");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Volume"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Volume"), "Volume");
...
/**
  The wrapper function of VolumeToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Volume_V1_2_3_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Volume_V1_2_3_JSON (*((EFI_REDFISH_VOLUME_V1_2_3_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Volume_V1_2_3_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  VolumeV1_2_3 = (EFI_REDFISH_VOLUME_V1_2_3 *)InterpProp;
  DestroyVolume_V1_2_3_CS (VolumeV1_2_3->Volume); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from VolumeFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyVolume_V1_2_3_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishVolume_V1_2_3EntryPoint (
