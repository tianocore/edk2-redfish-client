...
//
#include <RedfishJsonStructure/Storage/v1_0_4/EfiStorageV1_0_4.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Storage_V1_0_4_To_CS (ResoruceRaw, &StorageV1_0_4Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)StorageV1_0_4;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Storage_V1_0_4_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Storage_V1_0_4_JSON (*((EFI_REDFISH_STORAGE_V1_0_4_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Storage_V1_0_4_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyStorage_V1_0_4_Json (ResoruceRaw);
}
