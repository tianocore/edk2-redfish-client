...
//
#include <RedfishJsonStructure/RouteSetEntryCollection/EfiRouteSetEntryCollection.h>
CHAR8 mResourceTypeStr [] = "#RouteSetEntryCollection";
...
EFI_STATUS
EFIAPI
RouteSetEntryCollectionToStruct (
...
{
  EFI_REDFISH_ROUTESETENTRYCOLLECTION *RouteSetEntryCollection;
  EFI_REDFISH_ROUTESETENTRYCOLLECTION_CS *RouteSetEntryCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "RouteSetEntryCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_RouteSetEntryCollection_To_CS (ResoruceRaw, &RouteSetEntryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  RouteSetEntryCollection = (EFI_REDFISH_ROUTESETENTRYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ROUTESETENTRYCOLLECTION));
  if (RouteSetEntryCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)RouteSetEntryCollection;
  RouteSetEntryCollection->RouteSetEntryCollection = RouteSetEntryCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("RouteSetEntryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("RouteSetEntryCollection"), "RouteSetEntryCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("RouteSetEntryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("RouteSetEntryCollection"), "RouteSetEntryCollection");
...
/**
  The wrapper function of RouteSetEntryCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
RouteSetEntryCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return RouteSetEntryCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_RouteSetEntryCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_RouteSetEntryCollection_JSON (*((EFI_REDFISH_ROUTESETENTRYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_RouteSetEntryCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in RouteSetEntryCollectionToStruct.
...
EFI_STATUS
EFIAPI
RouteSetEntryCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  RouteSetEntryCollection = (EFI_REDFISH_ROUTESETENTRYCOLLECTION *)InterpProp;
  DestroyRouteSetEntryCollection_CS (RouteSetEntryCollection->RouteSetEntryCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from RouteSetEntryCollectionFromStruct
...
EFI_STATUS
EFIAPI
RouteSetEntryCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyRouteSetEntryCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishRouteSetEntryCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishRouteSetEntryCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
