...
//
#include <RedfishJsonStructure/Power/v1_3_4/EfiPowerV1_3_4.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Power_V1_3_4_To_CS (ResoruceRaw, &PowerV1_3_4Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerV1_3_4;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Power_V1_3_4_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Power_V1_3_4_JSON (*((EFI_REDFISH_POWER_V1_3_4_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Power_V1_3_4_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyPower_V1_3_4_Json (ResoruceRaw);
}
