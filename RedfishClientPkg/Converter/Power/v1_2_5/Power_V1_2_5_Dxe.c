...
//
#include <RedfishJsonStructure/Power/v1_2_5/EfiPowerV1_2_5.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Power_V1_2_5_To_CS (ResoruceRaw, &PowerV1_2_5Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerV1_2_5;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Power_V1_2_5_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Power_V1_2_5_JSON (*((EFI_REDFISH_POWER_V1_2_5_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Power_V1_2_5_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyPower_V1_2_5_Json (ResoruceRaw);
}
