...
//
#include <RedfishJsonStructure/ThermalSubsystem/v1_0_0/EfiThermalSubsystemV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#ThermalSubsystem";
BOOLEAN IsRevisonController = TRUE;

// Support ThermalSubsystem V1_0_0
...
{
  EFI_REDFISH_THERMALSUBSYSTEM_V1_0_0 *ThermalSubsystemV1_0_0;
  EFI_REDFISH_THERMALSUBSYSTEM_V1_0_0_CS *ThermalSubsystemV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ThermalSubsystem") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_ThermalSubsystem_V1_0_0_To_CS (ResoruceRaw, &ThermalSubsystemV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ThermalSubsystemV1_0_0 = (EFI_REDFISH_THERMALSUBSYSTEM_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_THERMALSUBSYSTEM_V1_0_0));
  if (ThermalSubsystemV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ThermalSubsystemV1_0_0;
  ThermalSubsystemV1_0_0->ThermalSubsystem = ThermalSubsystemV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ThermalSubsystem"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ThermalSubsystem"), "ThermalSubsystem");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ThermalSubsystem"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ThermalSubsystem"), "ThermalSubsystem");
...
/**
  The wrapper function of ThermalSubsystemToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ThermalSubsystemToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ThermalSubsystemToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ThermalSubsystem_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ThermalSubsystem_V1_0_0_JSON (*((EFI_REDFISH_THERMALSUBSYSTEM_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ThermalSubsystem_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ThermalSubsystemToStruct.
...
)
{
  ...
  ThermalSubsystemV1_0_0 = (EFI_REDFISH_THERMALSUBSYSTEM_V1_0_0 *)InterpProp;
  DestroyThermalSubsystem_V1_0_0_CS (ThermalSubsystemV1_0_0->ThermalSubsystem); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ThermalSubsystemFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyThermalSubsystem_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishThermalSubsystem_V1_0_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishThermalSubsystem_V1_0_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
