...
//
#include <RedfishJsonStructure/Drive/v1_3_0/EfiDriveV1_3_0.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Drive_V1_3_0_To_CS (ResoruceRaw, &DriveV1_3_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)DriveV1_3_0;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Drive_V1_3_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Drive_V1_3_0_JSON (*((EFI_REDFISH_DRIVE_V1_3_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Drive_V1_3_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyDrive_V1_3_0_Json (ResoruceRaw);
}
