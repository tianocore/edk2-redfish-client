...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_MetricReportDefinition_V1_0_4_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_MetricReportDefinition_V1_0_4_JSON (*((EFI_REDFISH_METRICREPORTDEFINITION_V1_0_4_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_MetricReportDefinition_V1_0_4_JSON fail (%d)\n", __func__, RedfishStatus));
  }
