...
//
#include <RedfishJsonStructure/VCATEntryCollection/EfiVCATEntryCollection.h>
CHAR8 mResourceTypeStr [] = "#VCATEntryCollection";
BOOLEAN IsRevisonController = FALSE;

// Support VCATEntryCollection 
...
{
  EFI_REDFISH_VCATENTRYCOLLECTION *VCATEntryCollection;
  EFI_REDFISH_VCATENTRYCOLLECTION_CS *VCATEntryCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "VCATEntryCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_VCATEntryCollection_To_CS (ResoruceRaw, &VCATEntryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  VCATEntryCollection = (EFI_REDFISH_VCATENTRYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_VCATENTRYCOLLECTION));
  if (VCATEntryCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)VCATEntryCollection;
  VCATEntryCollection->VCATEntryCollection = VCATEntryCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("VCATEntryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("VCATEntryCollection"), "VCATEntryCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("VCATEntryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("VCATEntryCollection"), "VCATEntryCollection");
...
/**
  The wrapper function of VCATEntryCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
VCATEntryCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return VCATEntryCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_VCATEntryCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_VCATEntryCollection_JSON (*((EFI_REDFISH_VCATENTRYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_VCATEntryCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in VCATEntryCollectionToStruct.
...
EFI_STATUS
EFIAPI
VCATEntryCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  VCATEntryCollection = (EFI_REDFISH_VCATENTRYCOLLECTION *)InterpProp;
  DestroyVCATEntryCollection_CS (VCATEntryCollection->VCATEntryCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from VCATEntryCollectionFromStruct
...
EFI_STATUS
EFIAPI
VCATEntryCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyVCATEntryCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishVCATEntryCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishVCATEntryCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
