...
//
#include <RedfishJsonStructure/CircuitCollection/EfiCircuitCollection.h>
CHAR8 mResourceTypeStr [] = "#CircuitCollection";
BOOLEAN IsRevisonController = FALSE;

// Support CircuitCollection 
...
{
  EFI_REDFISH_CIRCUITCOLLECTION *CircuitCollection;
  EFI_REDFISH_CIRCUITCOLLECTION_CS *CircuitCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "CircuitCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_CircuitCollection_To_CS (ResoruceRaw, &CircuitCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  CircuitCollection = (EFI_REDFISH_CIRCUITCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CIRCUITCOLLECTION));
  if (CircuitCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)CircuitCollection;
  CircuitCollection->CircuitCollection = CircuitCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("CircuitCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("CircuitCollection"), "CircuitCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("CircuitCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("CircuitCollection"), "CircuitCollection");
...
/**
  The wrapper function of CircuitCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
CircuitCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return CircuitCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_CircuitCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_CircuitCollection_JSON (*((EFI_REDFISH_CIRCUITCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_CircuitCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in CircuitCollectionToStruct.
...
EFI_STATUS
EFIAPI
CircuitCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  CircuitCollection = (EFI_REDFISH_CIRCUITCOLLECTION *)InterpProp;
  DestroyCircuitCollection_CS (CircuitCollection->CircuitCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from CircuitCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyCircuitCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishCircuitCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishCircuitCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
