...
//
#include <RedfishJsonStructure/Manager/v1_2_5/EfiManagerV1_2_5.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Manager_V1_2_5_To_CS (ResoruceRaw, &ManagerV1_2_5Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ManagerV1_2_5;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Manager_V1_2_5_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Manager_V1_2_5_JSON (*((EFI_REDFISH_MANAGER_V1_2_5_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Manager_V1_2_5_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyManager_V1_2_5_Json (ResoruceRaw);
}
