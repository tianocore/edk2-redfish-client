...
//
#include <RedfishJsonStructure/ActionInfo/v1_0_1/EfiActionInfoV1_0_1.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_ActionInfo_V1_0_1_To_CS (ResoruceRaw, &ActionInfoV1_0_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ActionInfoV1_0_1;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ActionInfo_V1_0_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ActionInfo_V1_0_1_JSON (*((EFI_REDFISH_ACTIONINFO_V1_0_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ActionInfo_V1_0_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyActionInfo_V1_0_1_Json (ResoruceRaw);
}
