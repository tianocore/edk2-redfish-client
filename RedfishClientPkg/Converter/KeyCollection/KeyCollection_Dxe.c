...
//
#include <RedfishJsonStructure/KeyCollection/EfiKeyCollection.h>
CHAR8 mResourceTypeStr [] = "#KeyCollection";
BOOLEAN IsRevisonController = FALSE;

// Support KeyCollection 
...
{
  EFI_REDFISH_KEYCOLLECTION *KeyCollection;
  EFI_REDFISH_KEYCOLLECTION_CS *KeyCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "KeyCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_KeyCollection_To_CS (ResoruceRaw, &KeyCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  KeyCollection = (EFI_REDFISH_KEYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_KEYCOLLECTION));
  if (KeyCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)KeyCollection;
  KeyCollection->KeyCollection = KeyCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("KeyCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("KeyCollection"), "KeyCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("KeyCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("KeyCollection"), "KeyCollection");
...
/**
  The wrapper function of KeyCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_KeyCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_KeyCollection_JSON (*((EFI_REDFISH_KEYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_KeyCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in KeyCollectionToStruct.
...
)
{
  ...
  KeyCollection = (EFI_REDFISH_KEYCOLLECTION *)InterpProp;
  DestroyKeyCollection_CS (KeyCollection->KeyCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from KeyCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyKeyCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishKeyCollectionEntryPoint (
