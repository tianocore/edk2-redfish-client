...
//
#include <RedfishJsonStructure/JobCollection/EfiJobCollection.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_JobCollection_To_CS (ResoruceRaw, &JobCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)JobCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("JobCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("JobCollection"), "JobCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("JobCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("JobCollection"), "JobCollection");
...
/**
  The wrapper function of JobCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_JobCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_JobCollection_JSON (*((EFI_REDFISH_JOBCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_JobCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
/**
  This function destory JSON raw text returned from JobCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyJobCollection_Json (ResoruceRaw);
}
