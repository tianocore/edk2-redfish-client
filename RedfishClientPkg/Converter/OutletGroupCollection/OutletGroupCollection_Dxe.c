...
//
#include <RedfishJsonStructure/OutletGroupCollection/EfiOutletGroupCollection.h>
CHAR8 mResourceTypeStr [] = "#OutletGroupCollection";
BOOLEAN IsRevisonController = FALSE;

// Support OutletGroupCollection 
...
{
  EFI_REDFISH_OUTLETGROUPCOLLECTION *OutletGroupCollection;
  EFI_REDFISH_OUTLETGROUPCOLLECTION_CS *OutletGroupCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "OutletGroupCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_OutletGroupCollection_To_CS (ResoruceRaw, &OutletGroupCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  OutletGroupCollection = (EFI_REDFISH_OUTLETGROUPCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_OUTLETGROUPCOLLECTION));
  if (OutletGroupCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)OutletGroupCollection;
  OutletGroupCollection->OutletGroupCollection = OutletGroupCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("OutletGroupCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("OutletGroupCollection"), "OutletGroupCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("OutletGroupCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("OutletGroupCollection"), "OutletGroupCollection");
...
/**
  The wrapper function of OutletGroupCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
OutletGroupCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return OutletGroupCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_OutletGroupCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_OutletGroupCollection_JSON (*((EFI_REDFISH_OUTLETGROUPCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_OutletGroupCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in OutletGroupCollectionToStruct.
...
EFI_STATUS
EFIAPI
OutletGroupCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  OutletGroupCollection = (EFI_REDFISH_OUTLETGROUPCOLLECTION *)InterpProp;
  DestroyOutletGroupCollection_CS (OutletGroupCollection->OutletGroupCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from OutletGroupCollectionFromStruct
...
EFI_STATUS
EFIAPI
OutletGroupCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyOutletGroupCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishOutletGroupCollectionEntryPoint (
