...
//
#include <RedfishJsonStructure/RoleCollection/EfiRoleCollection.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_RoleCollection_To_CS (ResoruceRaw, &RoleCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)RoleCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("RoleCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("RoleCollection"), "RoleCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("RoleCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("RoleCollection"), "RoleCollection");
...
/**
  The wrapper function of RoleCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_RoleCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_RoleCollection_JSON (*((EFI_REDFISH_ROLECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_RoleCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
/**
  This function destory JSON raw text returned from RoleCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyRoleCollection_Json (ResoruceRaw);
}
