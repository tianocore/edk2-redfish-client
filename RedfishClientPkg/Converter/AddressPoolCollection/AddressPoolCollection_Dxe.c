...
//
#include <RedfishJsonStructure/AddressPoolCollection/EfiAddressPoolCollection.h>
CHAR8 mResourceTypeStr [] = "#AddressPoolCollection";
BOOLEAN IsRevisonController = FALSE;

// Support AddressPoolCollection 
...
{
  EFI_REDFISH_ADDRESSPOOLCOLLECTION *AddressPoolCollection;
  EFI_REDFISH_ADDRESSPOOLCOLLECTION_CS *AddressPoolCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "AddressPoolCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_AddressPoolCollection_To_CS (ResoruceRaw, &AddressPoolCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AddressPoolCollection = (EFI_REDFISH_ADDRESSPOOLCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ADDRESSPOOLCOLLECTION));
  if (AddressPoolCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AddressPoolCollection;
  AddressPoolCollection->AddressPoolCollection = AddressPoolCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("AddressPoolCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("AddressPoolCollection"), "AddressPoolCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("AddressPoolCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("AddressPoolCollection"), "AddressPoolCollection");
...
/**
  The wrapper function of AddressPoolCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
AddressPoolCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return AddressPoolCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AddressPoolCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AddressPoolCollection_JSON (*((EFI_REDFISH_ADDRESSPOOLCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AddressPoolCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AddressPoolCollectionToStruct.
...
EFI_STATUS
EFIAPI
AddressPoolCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  AddressPoolCollection = (EFI_REDFISH_ADDRESSPOOLCOLLECTION *)InterpProp;
  DestroyAddressPoolCollection_CS (AddressPoolCollection->AddressPoolCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AddressPoolCollectionFromStruct
...
EFI_STATUS
EFIAPI
AddressPoolCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyAddressPoolCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAddressPoolCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishAddressPoolCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
