...
//
#include <RedfishJsonStructure/ControlCollection/EfiControlCollection.h>
CHAR8 mResourceTypeStr [] = "#ControlCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ControlCollection 
...
{
  EFI_REDFISH_CONTROLCOLLECTION *ControlCollection;
  EFI_REDFISH_CONTROLCOLLECTION_CS *ControlCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ControlCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_ControlCollection_To_CS (ResoruceRaw, &ControlCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ControlCollection = (EFI_REDFISH_CONTROLCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CONTROLCOLLECTION));
  if (ControlCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ControlCollection;
  ControlCollection->ControlCollection = ControlCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ControlCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ControlCollection"), "ControlCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ControlCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ControlCollection"), "ControlCollection");
...
/**
  The wrapper function of ControlCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ControlCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ControlCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ControlCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ControlCollection_JSON (*((EFI_REDFISH_CONTROLCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ControlCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ControlCollectionToStruct.
...
EFI_STATUS
EFIAPI
ControlCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  ControlCollection = (EFI_REDFISH_CONTROLCOLLECTION *)InterpProp;
  DestroyControlCollection_CS (ControlCollection->ControlCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ControlCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyControlCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishControlCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishControlCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
