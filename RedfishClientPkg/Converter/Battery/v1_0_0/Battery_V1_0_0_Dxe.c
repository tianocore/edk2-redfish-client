...
//
#include <RedfishJsonStructure/Battery/v1_0_0/EfiBatteryV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#Battery";
BOOLEAN IsRevisonController = TRUE;

// Support Battery V1_0_0
...
{
  EFI_REDFISH_BATTERY_V1_0_0 *BatteryV1_0_0;
  EFI_REDFISH_BATTERY_V1_0_0_CS *BatteryV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Battery") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Battery_V1_0_0_To_CS (ResoruceRaw, &BatteryV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  BatteryV1_0_0 = (EFI_REDFISH_BATTERY_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_BATTERY_V1_0_0));
  if (BatteryV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)BatteryV1_0_0;
  BatteryV1_0_0->Battery = BatteryV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Battery"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Battery"), "Battery");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Battery"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Battery"), "Battery");
...
/**
  The wrapper function of BatteryToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Battery_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Battery_V1_0_0_JSON (*((EFI_REDFISH_BATTERY_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Battery_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in BatteryToStruct.
...
)
{
  ...
  BatteryV1_0_0 = (EFI_REDFISH_BATTERY_V1_0_0 *)InterpProp;
  DestroyBattery_V1_0_0_CS (BatteryV1_0_0->Battery); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from BatteryFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyBattery_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishBattery_V1_0_0EntryPoint (
