...
//
#include <RedfishJsonStructure/PowerDomainCollection/EfiPowerDomainCollection.h>
CHAR8 mResourceTypeStr [] = "#PowerDomainCollection";
...
{
  EFI_REDFISH_POWERDOMAINCOLLECTION *PowerDomainCollection;
  EFI_REDFISH_POWERDOMAINCOLLECTION_CS *PowerDomainCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PowerDomainCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PowerDomainCollection_To_CS (ResoruceRaw, &PowerDomainCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerDomainCollection = (EFI_REDFISH_POWERDOMAINCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWERDOMAINCOLLECTION));
  if (PowerDomainCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerDomainCollection;
  PowerDomainCollection->PowerDomainCollection = PowerDomainCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerDomainCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PowerDomainCollection"), "PowerDomainCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerDomainCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PowerDomainCollection"), "PowerDomainCollection");
...
/**
  The wrapper function of PowerDomainCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
PowerDomainCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return PowerDomainCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PowerDomainCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PowerDomainCollection_JSON (*((EFI_REDFISH_POWERDOMAINCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PowerDomainCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PowerDomainCollectionToStruct.
...
EFI_STATUS
EFIAPI
PowerDomainCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  PowerDomainCollection = (EFI_REDFISH_POWERDOMAINCOLLECTION *)InterpProp;
  DestroyPowerDomainCollection_CS (PowerDomainCollection->PowerDomainCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerDomainCollectionFromStruct
...
EFI_STATUS
EFIAPI
PowerDomainCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyPowerDomainCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPowerDomainCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPowerDomainCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
