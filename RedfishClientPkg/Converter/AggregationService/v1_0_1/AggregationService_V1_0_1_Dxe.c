...
//
#include <RedfishJsonStructure/AggregationService/v1_0_1/EfiAggregationServiceV1_0_1.h>
CHAR8 mResourceTypeStr [] = "#AggregationService";
BOOLEAN IsRevisonController = TRUE;

// Support AggregationService V1_0_1
...
{
  EFI_REDFISH_AGGREGATIONSERVICE_V1_0_1 *AggregationServiceV1_0_1;
  EFI_REDFISH_AGGREGATIONSERVICE_V1_0_1_CS *AggregationServiceV1_0_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "AggregationService") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_AggregationService_V1_0_1_To_CS (ResoruceRaw, &AggregationServiceV1_0_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AggregationServiceV1_0_1 = (EFI_REDFISH_AGGREGATIONSERVICE_V1_0_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_AGGREGATIONSERVICE_V1_0_1));
  if (AggregationServiceV1_0_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AggregationServiceV1_0_1;
  AggregationServiceV1_0_1->AggregationService = AggregationServiceV1_0_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("AggregationService"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("AggregationService"), "AggregationService");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("AggregationService"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("AggregationService"), "AggregationService");
...
/**
  The wrapper function of AggregationServiceToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
AggregationServiceToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return AggregationServiceToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AggregationService_V1_0_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AggregationService_V1_0_1_JSON (*((EFI_REDFISH_AGGREGATIONSERVICE_V1_0_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AggregationService_V1_0_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AggregationServiceToStruct.
...
EFI_STATUS
EFIAPI
AggregationServiceDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  AggregationServiceV1_0_1 = (EFI_REDFISH_AGGREGATIONSERVICE_V1_0_1 *)InterpProp;
  DestroyAggregationService_V1_0_1_CS (AggregationServiceV1_0_1->AggregationService); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AggregationServiceFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyAggregationService_V1_0_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAggregationService_V1_0_1EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishAggregationService_V1_0_1Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
