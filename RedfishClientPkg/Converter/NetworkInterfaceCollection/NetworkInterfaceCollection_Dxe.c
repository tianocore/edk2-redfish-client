...
//
#include <RedfishJsonStructure/NetworkInterfaceCollection/EfiNetworkInterfaceCollection.h>
...
EFI_STATUS
EFIAPI
NetworkInterfaceCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_NetworkInterfaceCollection_To_CS (ResoruceRaw, &NetworkInterfaceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)NetworkInterfaceCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkInterfaceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("NetworkInterfaceCollection"), "NetworkInterfaceCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkInterfaceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("NetworkInterfaceCollection"), "NetworkInterfaceCollection");
...
/**
  The wrapper function of NetworkInterfaceCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
NetworkInterfaceCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return NetworkInterfaceCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
NetworkInterfaceCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_NetworkInterfaceCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_NetworkInterfaceCollection_JSON (*((EFI_REDFISH_NETWORKINTERFACECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_NetworkInterfaceCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
NetworkInterfaceCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from NetworkInterfaceCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyNetworkInterfaceCollection_Json (ResoruceRaw);
}
