...
//
#include <RedfishJsonStructure/NetworkDeviceFunction/v1_2_1/EfiNetworkDeviceFunctionV1_2_1.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_NetworkDeviceFunction_V1_2_1_To_CS (ResoruceRaw, &NetworkDeviceFunctionV1_2_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)NetworkDeviceFunctionV1_2_1;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_NetworkDeviceFunction_V1_2_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_NetworkDeviceFunction_V1_2_1_JSON (*((EFI_REDFISH_NETWORKDEVICEFUNCTION_V1_2_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_NetworkDeviceFunction_V1_2_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyNetworkDeviceFunction_V1_2_1_Json (ResoruceRaw);
}
