...
//
#include <RedfishJsonStructure/Assembly/v1_2_1/EfiAssemblyV1_2_1.h>
CHAR8 mResourceTypeStr [] = "#Assembly";
BOOLEAN IsRevisonController = TRUE;

// Support Assembly V1_2_1
...
{
  EFI_REDFISH_ASSEMBLY_V1_2_1 *AssemblyV1_2_1;
  EFI_REDFISH_ASSEMBLY_V1_2_1_CS *AssemblyV1_2_1Cs;
  ...
  Status = (EFI_STATUS)Json_Assembly_V1_2_1_To_CS (ResoruceRaw, &AssemblyV1_2_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AssemblyV1_2_1 = (EFI_REDFISH_ASSEMBLY_V1_2_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ASSEMBLY_V1_2_1));
  if (AssemblyV1_2_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AssemblyV1_2_1;
  AssemblyV1_2_1->Assembly = AssemblyV1_2_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Assembly"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Assembly"), "Assembly");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Assembly"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Assembly"), "Assembly");
...
/**
  The wrapper function of AssemblyToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Assembly_V1_2_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Assembly_V1_2_1_JSON (*((EFI_REDFISH_ASSEMBLY_V1_2_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Assembly_V1_2_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  AssemblyV1_2_1 = (EFI_REDFISH_ASSEMBLY_V1_2_1 *)InterpProp;
  DestroyAssembly_V1_2_1_CS (AssemblyV1_2_1->Assembly); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AssemblyFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyAssembly_V1_2_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAssembly_V1_2_1EntryPoint (
