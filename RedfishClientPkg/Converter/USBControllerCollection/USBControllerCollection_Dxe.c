...
//
#include <RedfishJsonStructure/USBControllerCollection/EfiUSBControllerCollection.h>
CHAR8 mResourceTypeStr [] = "#USBControllerCollection";
BOOLEAN IsRevisonController = FALSE;

// Support USBControllerCollection 
...
EFI_STATUS
EFIAPI
USBControllerCollectionToStruct (
...
{
  EFI_REDFISH_USBCONTROLLERCOLLECTION *USBControllerCollection;
  EFI_REDFISH_USBCONTROLLERCOLLECTION_CS *USBControllerCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "USBControllerCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_USBControllerCollection_To_CS (ResoruceRaw, &USBControllerCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  USBControllerCollection = (EFI_REDFISH_USBCONTROLLERCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_USBCONTROLLERCOLLECTION));
  if (USBControllerCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)USBControllerCollection;
  USBControllerCollection->USBControllerCollection = USBControllerCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("USBControllerCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("USBControllerCollection"), "USBControllerCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("USBControllerCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("USBControllerCollection"), "USBControllerCollection");
...
/**
  The wrapper function of USBControllerCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
USBControllerCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return USBControllerCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_USBControllerCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_USBControllerCollection_JSON (*((EFI_REDFISH_USBCONTROLLERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_USBControllerCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in USBControllerCollectionToStruct.
...
EFI_STATUS
EFIAPI
USBControllerCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  USBControllerCollection = (EFI_REDFISH_USBCONTROLLERCOLLECTION *)InterpProp;
  DestroyUSBControllerCollection_CS (USBControllerCollection->USBControllerCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from USBControllerCollectionFromStruct
...
EFI_STATUS
EFIAPI
USBControllerCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyUSBControllerCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishUSBControllerCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishUSBControllerCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
