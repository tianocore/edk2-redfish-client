...
//
#include <RedfishJsonStructure/ComputerSystem/v1_3_3/EfiComputerSystemV1_3_3.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_ComputerSystem_V1_3_3_To_CS (ResoruceRaw, &ComputerSystemV1_3_3Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ComputerSystemV1_3_3;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ComputerSystem_V1_3_3_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ComputerSystem_V1_3_3_JSON (*((EFI_REDFISH_COMPUTERSYSTEM_V1_3_3_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ComputerSystem_V1_3_3_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyComputerSystem_V1_3_3_Json (ResoruceRaw);
}
