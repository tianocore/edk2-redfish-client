...
//
#include <RedfishJsonStructure/ComputerSystem/v1_2_4/EfiComputerSystemV1_2_4.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_ComputerSystem_V1_2_4_To_CS (ResoruceRaw, &ComputerSystemV1_2_4Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ComputerSystemV1_2_4;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ComputerSystem_V1_2_4_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ComputerSystem_V1_2_4_JSON (*((EFI_REDFISH_COMPUTERSYSTEM_V1_2_4_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ComputerSystem_V1_2_4_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyComputerSystem_V1_2_4_Json (ResoruceRaw);
}
