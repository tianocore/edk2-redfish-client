...
//
#include <RedfishJsonStructure/ComputerSystem/v1_0_9/EfiComputerSystemV1_0_9.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_ComputerSystem_V1_0_9_To_CS (ResoruceRaw, &ComputerSystemV1_0_9Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ComputerSystemV1_0_9;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ComputerSystem_V1_0_9_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ComputerSystem_V1_0_9_JSON (*((EFI_REDFISH_COMPUTERSYSTEM_V1_0_9_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ComputerSystem_V1_0_9_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyComputerSystem_V1_0_9_Json (ResoruceRaw);
}
