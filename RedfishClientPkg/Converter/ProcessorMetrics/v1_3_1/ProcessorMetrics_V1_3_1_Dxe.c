...
//
#include <RedfishJsonStructure/ProcessorMetrics/v1_3_1/EfiProcessorMetricsV1_3_1.h>
CHAR8 mResourceTypeStr [] = "#ProcessorMetrics";
BOOLEAN IsRevisonController = TRUE;

// Support ProcessorMetrics V1_3_1
...
{
  EFI_REDFISH_PROCESSORMETRICS_V1_3_1 *ProcessorMetricsV1_3_1;
  EFI_REDFISH_PROCESSORMETRICS_V1_3_1_CS *ProcessorMetricsV1_3_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ProcessorMetrics") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_ProcessorMetrics_V1_3_1_To_CS (ResoruceRaw, &ProcessorMetricsV1_3_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ProcessorMetricsV1_3_1 = (EFI_REDFISH_PROCESSORMETRICS_V1_3_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_PROCESSORMETRICS_V1_3_1));
  if (ProcessorMetricsV1_3_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ProcessorMetricsV1_3_1;
  ProcessorMetricsV1_3_1->ProcessorMetrics = ProcessorMetricsV1_3_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ProcessorMetrics"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ProcessorMetrics"), "ProcessorMetrics");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ProcessorMetrics"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ProcessorMetrics"), "ProcessorMetrics");
...
/**
  The wrapper function of ProcessorMetricsToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ProcessorMetricsToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ProcessorMetricsToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ProcessorMetrics_V1_3_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ProcessorMetrics_V1_3_1_JSON (*((EFI_REDFISH_PROCESSORMETRICS_V1_3_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ProcessorMetrics_V1_3_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ProcessorMetricsToStruct.
...
)
{
  ...
  ProcessorMetricsV1_3_1 = (EFI_REDFISH_PROCESSORMETRICS_V1_3_1 *)InterpProp;
  DestroyProcessorMetrics_V1_3_1_CS (ProcessorMetricsV1_3_1->ProcessorMetrics); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ProcessorMetricsFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyProcessorMetrics_V1_3_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishProcessorMetrics_V1_3_1EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishProcessorMetrics_V1_3_1Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
