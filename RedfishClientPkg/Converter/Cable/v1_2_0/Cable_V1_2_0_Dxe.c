...
//
#include <RedfishJsonStructure/Cable/v1_2_0/EfiCableV1_2_0.h>
CHAR8 mResourceTypeStr [] = "#Cable";
BOOLEAN IsRevisonController = TRUE;

// Support Cable V1_2_0
...
{
  EFI_REDFISH_CABLE_V1_2_0 *CableV1_2_0;
  EFI_REDFISH_CABLE_V1_2_0_CS *CableV1_2_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Cable") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Cable_V1_2_0_To_CS (ResoruceRaw, &CableV1_2_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  CableV1_2_0 = (EFI_REDFISH_CABLE_V1_2_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CABLE_V1_2_0));
  if (CableV1_2_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)CableV1_2_0;
  CableV1_2_0->Cable = CableV1_2_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Cable"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Cable"), "Cable");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Cable"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Cable"), "Cable");
...
/**
  The wrapper function of CableToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Cable_V1_2_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Cable_V1_2_0_JSON (*((EFI_REDFISH_CABLE_V1_2_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Cable_V1_2_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in CableToStruct.
...
)
{
  ...
  CableV1_2_0 = (EFI_REDFISH_CABLE_V1_2_0 *)InterpProp;
  DestroyCable_V1_2_0_CS (CableV1_2_0->Cable); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from CableFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyCable_V1_2_0_Json (ResoruceRaw);
}
