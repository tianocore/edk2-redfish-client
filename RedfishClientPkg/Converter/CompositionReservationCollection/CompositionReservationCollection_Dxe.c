...
//
#include <RedfishJsonStructure/CompositionReservationCollection/EfiCompositionReservationCollection.h>
CHAR8 mResourceTypeStr [] = "#CompositionReservationCollection";
BOOLEAN IsRevisonController = FALSE;

// Support CompositionReservationCollection 
...
EFI_STATUS
EFIAPI
CompositionReservationCollectionToStruct (
...
{
  EFI_REDFISH_COMPOSITIONRESERVATIONCOLLECTION *CompositionReservationCollection;
  EFI_REDFISH_COMPOSITIONRESERVATIONCOLLECTION_CS *CompositionReservationCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "CompositionReservationCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_CompositionReservationCollection_To_CS (ResoruceRaw, &CompositionReservationCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  CompositionReservationCollection = (EFI_REDFISH_COMPOSITIONRESERVATIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_COMPOSITIONRESERVATIONCOLLECTION));
  if (CompositionReservationCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)CompositionReservationCollection;
  CompositionReservationCollection->CompositionReservationCollection = CompositionReservationCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("CompositionReservationCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("CompositionReservationCollection"), "CompositionReservationCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("CompositionReservationCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("CompositionReservationCollection"), "CompositionReservationCollection");
...
/**
  The wrapper function of CompositionReservationCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
CompositionReservationCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return CompositionReservationCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
CompositionReservationCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_CompositionReservationCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_CompositionReservationCollection_JSON (*((EFI_REDFISH_COMPOSITIONRESERVATIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_CompositionReservationCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in CompositionReservationCollectionToStruct.
...
EFI_STATUS
EFIAPI
CompositionReservationCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  CompositionReservationCollection = (EFI_REDFISH_COMPOSITIONRESERVATIONCOLLECTION *)InterpProp;
  DestroyCompositionReservationCollection_CS (CompositionReservationCollection->CompositionReservationCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from CompositionReservationCollectionFromStruct
...
EFI_STATUS
EFIAPI
CompositionReservationCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyCompositionReservationCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishCompositionReservationCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishCompositionReservationCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
