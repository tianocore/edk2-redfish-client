...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_NetworkDeviceFunctionMetrics_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_NetworkDeviceFunctionMetrics_V1_0_0_JSON (*((EFI_REDFISH_NETWORKDEVICEFUNCTIONMETRICS_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_NetworkDeviceFunctionMetrics_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
