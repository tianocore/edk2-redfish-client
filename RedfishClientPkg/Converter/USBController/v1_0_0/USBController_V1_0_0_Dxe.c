...
//
#include <RedfishJsonStructure/USBController/v1_0_0/EfiUSBControllerV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#USBController";
BOOLEAN IsRevisonController = TRUE;

// Support USBController V1_0_0
...
{
  EFI_REDFISH_USBCONTROLLER_V1_0_0 *USBControllerV1_0_0;
  EFI_REDFISH_USBCONTROLLER_V1_0_0_CS *USBControllerV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "USBController") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_USBController_V1_0_0_To_CS (ResoruceRaw, &USBControllerV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  USBControllerV1_0_0 = (EFI_REDFISH_USBCONTROLLER_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_USBCONTROLLER_V1_0_0));
  if (USBControllerV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)USBControllerV1_0_0;
  USBControllerV1_0_0->USBController = USBControllerV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("USBController"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("USBController"), "USBController");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("USBController"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("USBController"), "USBController");
...
/**
  The wrapper function of USBControllerToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_USBController_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_USBController_V1_0_0_JSON (*((EFI_REDFISH_USBCONTROLLER_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_USBController_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in USBControllerToStruct.
...
)
{
  ...
  USBControllerV1_0_0 = (EFI_REDFISH_USBCONTROLLER_V1_0_0 *)InterpProp;
  DestroyUSBController_V1_0_0_CS (USBControllerV1_0_0->USBController); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from USBControllerFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyUSBController_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishUSBController_V1_0_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishUSBController_V1_0_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
