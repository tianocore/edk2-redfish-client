...
//
#include <RedfishJsonStructure/BatteryCollection/EfiBatteryCollection.h>
CHAR8 mResourceTypeStr [] = "#BatteryCollection";
BOOLEAN IsRevisonController = FALSE;

// Support BatteryCollection 
...
{
  EFI_REDFISH_BATTERYCOLLECTION *BatteryCollection;
  EFI_REDFISH_BATTERYCOLLECTION_CS *BatteryCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "BatteryCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_BatteryCollection_To_CS (ResoruceRaw, &BatteryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  BatteryCollection = (EFI_REDFISH_BATTERYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_BATTERYCOLLECTION));
  if (BatteryCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)BatteryCollection;
  BatteryCollection->BatteryCollection = BatteryCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("BatteryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("BatteryCollection"), "BatteryCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("BatteryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("BatteryCollection"), "BatteryCollection");
...
/**
  The wrapper function of BatteryCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
BatteryCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return BatteryCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_BatteryCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_BatteryCollection_JSON (*((EFI_REDFISH_BATTERYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_BatteryCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in BatteryCollectionToStruct.
...
EFI_STATUS
EFIAPI
BatteryCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  BatteryCollection = (EFI_REDFISH_BATTERYCOLLECTION *)InterpProp;
  DestroyBatteryCollection_CS (BatteryCollection->BatteryCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from BatteryCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyBatteryCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishBatteryCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishBatteryCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
