...
//
#include <RedfishJsonStructure/StorageControllerCollection/EfiStorageControllerCollection.h>
CHAR8 mResourceTypeStr [] = "#StorageControllerCollection";
BOOLEAN IsRevisonController = FALSE;

// Support StorageControllerCollection 
...
EFI_STATUS
EFIAPI
StorageControllerCollectionToStruct (
...
{
  EFI_REDFISH_STORAGECONTROLLERCOLLECTION *StorageControllerCollection;
  EFI_REDFISH_STORAGECONTROLLERCOLLECTION_CS *StorageControllerCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "StorageControllerCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_StorageControllerCollection_To_CS (ResoruceRaw, &StorageControllerCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  StorageControllerCollection = (EFI_REDFISH_STORAGECONTROLLERCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_STORAGECONTROLLERCOLLECTION));
  if (StorageControllerCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)StorageControllerCollection;
  StorageControllerCollection->StorageControllerCollection = StorageControllerCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("StorageControllerCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("StorageControllerCollection"), "StorageControllerCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("StorageControllerCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("StorageControllerCollection"), "StorageControllerCollection");
...
/**
  The wrapper function of StorageControllerCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
StorageControllerCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return StorageControllerCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
StorageControllerCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_StorageControllerCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_StorageControllerCollection_JSON (*((EFI_REDFISH_STORAGECONTROLLERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_StorageControllerCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in StorageControllerCollectionToStruct.
...
EFI_STATUS
EFIAPI
StorageControllerCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  StorageControllerCollection = (EFI_REDFISH_STORAGECONTROLLERCOLLECTION *)InterpProp;
  DestroyStorageControllerCollection_CS (StorageControllerCollection->StorageControllerCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from StorageControllerCollectionFromStruct
...
EFI_STATUS
EFIAPI
StorageControllerCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyStorageControllerCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishStorageControllerCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishStorageControllerCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
