...
//
#include <RedfishJsonStructure/PowerSupplyCollection/EfiPowerSupplyCollection.h>
CHAR8 mResourceTypeStr [] = "#PowerSupplyCollection";
BOOLEAN IsRevisonController = FALSE;

// Support PowerSupplyCollection 
...
{
  EFI_REDFISH_POWERSUPPLYCOLLECTION *PowerSupplyCollection;
  EFI_REDFISH_POWERSUPPLYCOLLECTION_CS *PowerSupplyCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PowerSupplyCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PowerSupplyCollection_To_CS (ResoruceRaw, &PowerSupplyCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerSupplyCollection = (EFI_REDFISH_POWERSUPPLYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWERSUPPLYCOLLECTION));
  if (PowerSupplyCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerSupplyCollection;
  PowerSupplyCollection->PowerSupplyCollection = PowerSupplyCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerSupplyCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PowerSupplyCollection"), "PowerSupplyCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerSupplyCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PowerSupplyCollection"), "PowerSupplyCollection");
...
/**
  The wrapper function of PowerSupplyCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
PowerSupplyCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return PowerSupplyCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PowerSupplyCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PowerSupplyCollection_JSON (*((EFI_REDFISH_POWERSUPPLYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PowerSupplyCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PowerSupplyCollectionToStruct.
...
EFI_STATUS
EFIAPI
PowerSupplyCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  PowerSupplyCollection = (EFI_REDFISH_POWERSUPPLYCOLLECTION *)InterpProp;
  DestroyPowerSupplyCollection_CS (PowerSupplyCollection->PowerSupplyCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerSupplyCollectionFromStruct
...
EFI_STATUS
EFIAPI
PowerSupplyCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyPowerSupplyCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPowerSupplyCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPowerSupplyCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
