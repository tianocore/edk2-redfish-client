...
//
#include <RedfishJsonStructure/SignatureCollection/EfiSignatureCollection.h>
CHAR8 mResourceTypeStr [] = "#SignatureCollection";
BOOLEAN IsRevisonController = FALSE;

// Support SignatureCollection 
...
{
  EFI_REDFISH_SIGNATURECOLLECTION *SignatureCollection;
  EFI_REDFISH_SIGNATURECOLLECTION_CS *SignatureCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "SignatureCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_SignatureCollection_To_CS (ResoruceRaw, &SignatureCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SignatureCollection = (EFI_REDFISH_SIGNATURECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SIGNATURECOLLECTION));
  if (SignatureCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SignatureCollection;
  SignatureCollection->SignatureCollection = SignatureCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SignatureCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("SignatureCollection"), "SignatureCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SignatureCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("SignatureCollection"), "SignatureCollection");
...
/**
  The wrapper function of SignatureCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
SignatureCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return SignatureCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_SignatureCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_SignatureCollection_JSON (*((EFI_REDFISH_SIGNATURECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_SignatureCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in SignatureCollectionToStruct.
...
EFI_STATUS
EFIAPI
SignatureCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  SignatureCollection = (EFI_REDFISH_SIGNATURECOLLECTION *)InterpProp;
  DestroySignatureCollection_CS (SignatureCollection->SignatureCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SignatureCollectionFromStruct
...
EFI_STATUS
EFIAPI
SignatureCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroySignatureCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSignatureCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishSignatureCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
