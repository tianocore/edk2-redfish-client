...
//
#include <RedfishJsonStructure/DriveCollection/EfiDriveCollection.h>
CHAR8 mResourceTypeStr [] = "#DriveCollection";
...
{
  EFI_REDFISH_DRIVECOLLECTION *DriveCollection;
  EFI_REDFISH_DRIVECOLLECTION_CS *DriveCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "DriveCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_DriveCollection_To_CS (ResoruceRaw, &DriveCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  DriveCollection = (EFI_REDFISH_DRIVECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_DRIVECOLLECTION));
  if (DriveCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)DriveCollection;
  DriveCollection->DriveCollection = DriveCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("DriveCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("DriveCollection"), "DriveCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("DriveCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("DriveCollection"), "DriveCollection");
...
/**
  The wrapper function of DriveCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
DriveCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return DriveCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_DriveCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_DriveCollection_JSON (*((EFI_REDFISH_DRIVECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_DriveCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in DriveCollectionToStruct.
...
)
{
  ...
  DriveCollection = (EFI_REDFISH_DRIVECOLLECTION *)InterpProp;
  DestroyDriveCollection_CS (DriveCollection->DriveCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from DriveCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyDriveCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishDriveCollectionEntryPoint (
