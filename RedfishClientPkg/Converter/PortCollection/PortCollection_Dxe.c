...
//
#include <RedfishJsonStructure/PortCollection/EfiPortCollection.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_PortCollection_To_CS (ResoruceRaw, &PortCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PortCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PortCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PortCollection"), "PortCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PortCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PortCollection"), "PortCollection");
...
/**
  The wrapper function of PortCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PortCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PortCollection_JSON (*((EFI_REDFISH_PORTCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PortCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
/**
  This function destory JSON raw text returned from PortCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyPortCollection_Json (ResoruceRaw);
}
