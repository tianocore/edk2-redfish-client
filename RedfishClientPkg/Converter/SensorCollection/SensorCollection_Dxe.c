...
//
#include <RedfishJsonStructure/SensorCollection/EfiSensorCollection.h>
CHAR8 mResourceTypeStr [] = "#SensorCollection";
BOOLEAN IsRevisonController = FALSE;

// Support SensorCollection 
...
{
  EFI_REDFISH_SENSORCOLLECTION *SensorCollection;
  EFI_REDFISH_SENSORCOLLECTION_CS *SensorCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "SensorCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_SensorCollection_To_CS (ResoruceRaw, &SensorCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SensorCollection = (EFI_REDFISH_SENSORCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SENSORCOLLECTION));
  if (SensorCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SensorCollection;
  SensorCollection->SensorCollection = SensorCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SensorCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("SensorCollection"), "SensorCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SensorCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("SensorCollection"), "SensorCollection");
...
/**
  The wrapper function of SensorCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
SensorCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return SensorCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_SensorCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_SensorCollection_JSON (*((EFI_REDFISH_SENSORCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_SensorCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in SensorCollectionToStruct.
...
)
{
  ...
  SensorCollection = (EFI_REDFISH_SENSORCOLLECTION *)InterpProp;
  DestroySensorCollection_CS (SensorCollection->SensorCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SensorCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroySensorCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSensorCollectionEntryPoint (
