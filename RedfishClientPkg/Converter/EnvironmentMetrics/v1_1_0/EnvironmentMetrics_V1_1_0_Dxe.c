...
//
#include <RedfishJsonStructure/EnvironmentMetrics/v1_1_0/EfiEnvironmentMetricsV1_1_0.h>
CHAR8 mResourceTypeStr [] = "#EnvironmentMetrics";
BOOLEAN IsRevisonController = TRUE;

// Support EnvironmentMetrics V1_1_0
...
{
  EFI_REDFISH_ENVIRONMENTMETRICS_V1_1_0 *EnvironmentMetricsV1_1_0;
  EFI_REDFISH_ENVIRONMENTMETRICS_V1_1_0_CS *EnvironmentMetricsV1_1_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "EnvironmentMetrics") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_EnvironmentMetrics_V1_1_0_To_CS (ResoruceRaw, &EnvironmentMetricsV1_1_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  EnvironmentMetricsV1_1_0 = (EFI_REDFISH_ENVIRONMENTMETRICS_V1_1_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ENVIRONMENTMETRICS_V1_1_0));
  if (EnvironmentMetricsV1_1_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)EnvironmentMetricsV1_1_0;
  EnvironmentMetricsV1_1_0->EnvironmentMetrics = EnvironmentMetricsV1_1_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("EnvironmentMetrics"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("EnvironmentMetrics"), "EnvironmentMetrics");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("EnvironmentMetrics"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("EnvironmentMetrics"), "EnvironmentMetrics");
...
/**
  The wrapper function of EnvironmentMetricsToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
EnvironmentMetricsToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return EnvironmentMetricsToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_EnvironmentMetrics_V1_1_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_EnvironmentMetrics_V1_1_0_JSON (*((EFI_REDFISH_ENVIRONMENTMETRICS_V1_1_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_EnvironmentMetrics_V1_1_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in EnvironmentMetricsToStruct.
...
EFI_STATUS
EFIAPI
EnvironmentMetricsDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  EnvironmentMetricsV1_1_0 = (EFI_REDFISH_ENVIRONMENTMETRICS_V1_1_0 *)InterpProp;
  DestroyEnvironmentMetrics_V1_1_0_CS (EnvironmentMetricsV1_1_0->EnvironmentMetrics); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from EnvironmentMetricsFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyEnvironmentMetrics_V1_1_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishEnvironmentMetrics_V1_1_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishEnvironmentMetrics_V1_1_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
