...
//
#include <RedfishJsonStructure/SimpleStorage/v1_2_4/EfiSimpleStorageV1_2_4.h>
CHAR8 mResourceTypeStr [] = "#SimpleStorage";
BOOLEAN IsRevisonController = TRUE;

// Support SimpleStorage V1_2_4
...
{
  EFI_REDFISH_SIMPLESTORAGE_V1_2_4 *SimpleStorageV1_2_4;
  EFI_REDFISH_SIMPLESTORAGE_V1_2_4_CS *SimpleStorageV1_2_4Cs;
  ...
  Status = (EFI_STATUS)Json_SimpleStorage_V1_2_4_To_CS (ResoruceRaw, &SimpleStorageV1_2_4Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SimpleStorageV1_2_4 = (EFI_REDFISH_SIMPLESTORAGE_V1_2_4 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SIMPLESTORAGE_V1_2_4));
  if (SimpleStorageV1_2_4 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SimpleStorageV1_2_4;
  SimpleStorageV1_2_4->SimpleStorage = SimpleStorageV1_2_4Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SimpleStorage"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("SimpleStorage"), "SimpleStorage");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SimpleStorage"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("SimpleStorage"), "SimpleStorage");
...
/**
  The wrapper function of SimpleStorageToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_SimpleStorage_V1_2_4_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_SimpleStorage_V1_2_4_JSON (*((EFI_REDFISH_SIMPLESTORAGE_V1_2_4_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_SimpleStorage_V1_2_4_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  SimpleStorageV1_2_4 = (EFI_REDFISH_SIMPLESTORAGE_V1_2_4 *)InterpProp;
  DestroySimpleStorage_V1_2_4_CS (SimpleStorageV1_2_4->SimpleStorage); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SimpleStorageFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroySimpleStorage_V1_2_4_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSimpleStorage_V1_2_4EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishSimpleStorage_V1_2_4Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
