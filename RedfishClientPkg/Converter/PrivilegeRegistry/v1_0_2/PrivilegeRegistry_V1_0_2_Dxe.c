...
//
#include <RedfishJsonStructure/PrivilegeRegistry/v1_0_2/EfiPrivilegeRegistryV1_0_2.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_PrivilegeRegistry_V1_0_2_To_CS (ResoruceRaw, &PrivilegeRegistryV1_0_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PrivilegeRegistryV1_0_2;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PrivilegeRegistry_V1_0_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PrivilegeRegistry_V1_0_2_JSON (*((EFI_REDFISH_PRIVILEGEREGISTRY_V1_0_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PrivilegeRegistry_V1_0_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyPrivilegeRegistry_V1_0_2_Json (ResoruceRaw);
}
