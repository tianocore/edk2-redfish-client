...
//
#include <RedfishJsonStructure/BootOption/v1_0_2/EfiBootOptionV1_0_2.h>
CHAR8 mResourceTypeStr [] = "#BootOption";
BOOLEAN IsRevisonController = TRUE;

// Support BootOption V1_0_2
...
{
  EFI_REDFISH_BOOTOPTION_V1_0_2 *BootOptionV1_0_2;
  EFI_REDFISH_BOOTOPTION_V1_0_2_CS *BootOptionV1_0_2Cs;
  ...
  Status = (EFI_STATUS)Json_BootOption_V1_0_2_To_CS (ResoruceRaw, &BootOptionV1_0_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  BootOptionV1_0_2 = (EFI_REDFISH_BOOTOPTION_V1_0_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_BOOTOPTION_V1_0_2));
  if (BootOptionV1_0_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)BootOptionV1_0_2;
  BootOptionV1_0_2->BootOption = BootOptionV1_0_2Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("BootOption"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("BootOption"), "BootOption");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("BootOption"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("BootOption"), "BootOption");
...
/**
  The wrapper function of BootOptionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_BootOption_V1_0_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_BootOption_V1_0_2_JSON (*((EFI_REDFISH_BOOTOPTION_V1_0_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_BootOption_V1_0_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  BootOptionV1_0_2 = (EFI_REDFISH_BOOTOPTION_V1_0_2 *)InterpProp;
  DestroyBootOption_V1_0_2_CS (BootOptionV1_0_2->BootOption); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from BootOptionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyBootOption_V1_0_2_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishBootOption_V1_0_2EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishBootOption_V1_0_2Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
