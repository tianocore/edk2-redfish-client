...
//
#include <RedfishJsonStructure/AggregationSourceCollection/EfiAggregationSourceCollection.h>
CHAR8 mResourceTypeStr [] = "#AggregationSourceCollection";
BOOLEAN IsRevisonController = FALSE;

// Support AggregationSourceCollection 
...
EFI_STATUS
EFIAPI
AggregationSourceCollectionToStruct (
...
{
  EFI_REDFISH_AGGREGATIONSOURCECOLLECTION *AggregationSourceCollection;
  EFI_REDFISH_AGGREGATIONSOURCECOLLECTION_CS *AggregationSourceCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "AggregationSourceCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_AggregationSourceCollection_To_CS (ResoruceRaw, &AggregationSourceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AggregationSourceCollection = (EFI_REDFISH_AGGREGATIONSOURCECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_AGGREGATIONSOURCECOLLECTION));
  if (AggregationSourceCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AggregationSourceCollection;
  AggregationSourceCollection->AggregationSourceCollection = AggregationSourceCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("AggregationSourceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("AggregationSourceCollection"), "AggregationSourceCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("AggregationSourceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("AggregationSourceCollection"), "AggregationSourceCollection");
...
/**
  The wrapper function of AggregationSourceCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
AggregationSourceCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return AggregationSourceCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
AggregationSourceCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AggregationSourceCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AggregationSourceCollection_JSON (*((EFI_REDFISH_AGGREGATIONSOURCECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AggregationSourceCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AggregationSourceCollectionToStruct.
...
)
{
  ...
  AggregationSourceCollection = (EFI_REDFISH_AGGREGATIONSOURCECOLLECTION *)InterpProp;
  DestroyAggregationSourceCollection_CS (AggregationSourceCollection->AggregationSourceCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AggregationSourceCollectionFromStruct
...
EFI_STATUS
EFIAPI
AggregationSourceCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyAggregationSourceCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAggregationSourceCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishAggregationSourceCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
