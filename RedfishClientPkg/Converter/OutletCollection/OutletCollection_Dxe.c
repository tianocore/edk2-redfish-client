...
//
#include <RedfishJsonStructure/OutletCollection/EfiOutletCollection.h>
CHAR8 mResourceTypeStr [] = "#OutletCollection";
BOOLEAN IsRevisonController = FALSE;

// Support OutletCollection 
...
{
  EFI_REDFISH_OUTLETCOLLECTION *OutletCollection;
  EFI_REDFISH_OUTLETCOLLECTION_CS *OutletCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "OutletCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_OutletCollection_To_CS (ResoruceRaw, &OutletCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  OutletCollection = (EFI_REDFISH_OUTLETCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_OUTLETCOLLECTION));
  if (OutletCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)OutletCollection;
  OutletCollection->OutletCollection = OutletCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("OutletCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("OutletCollection"), "OutletCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("OutletCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("OutletCollection"), "OutletCollection");
...
/**
  The wrapper function of OutletCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
OutletCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return OutletCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_OutletCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_OutletCollection_JSON (*((EFI_REDFISH_OUTLETCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_OutletCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in OutletCollectionToStruct.
...
)
{
  ...
  OutletCollection = (EFI_REDFISH_OUTLETCOLLECTION *)InterpProp;
  DestroyOutletCollection_CS (OutletCollection->OutletCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from OutletCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyOutletCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishOutletCollectionEntryPoint (
