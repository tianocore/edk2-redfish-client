...
//
#include <RedfishJsonStructure/AggregationSource/v1_2_0/EfiAggregationSourceV1_2_0.h>
CHAR8 mResourceTypeStr [] = "#AggregationSource";
BOOLEAN IsRevisonController = TRUE;

// Support AggregationSource V1_2_0
...
{
  EFI_REDFISH_AGGREGATIONSOURCE_V1_2_0 *AggregationSourceV1_2_0;
  EFI_REDFISH_AGGREGATIONSOURCE_V1_2_0_CS *AggregationSourceV1_2_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "AggregationSource") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_AggregationSource_V1_2_0_To_CS (ResoruceRaw, &AggregationSourceV1_2_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AggregationSourceV1_2_0 = (EFI_REDFISH_AGGREGATIONSOURCE_V1_2_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_AGGREGATIONSOURCE_V1_2_0));
  if (AggregationSourceV1_2_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AggregationSourceV1_2_0;
  AggregationSourceV1_2_0->AggregationSource = AggregationSourceV1_2_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("AggregationSource"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("AggregationSource"), "AggregationSource");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("AggregationSource"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("AggregationSource"), "AggregationSource");
...
/**
  The wrapper function of AggregationSourceToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
AggregationSourceToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return AggregationSourceToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AggregationSource_V1_2_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AggregationSource_V1_2_0_JSON (*((EFI_REDFISH_AGGREGATIONSOURCE_V1_2_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AggregationSource_V1_2_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AggregationSourceToStruct.
...
EFI_STATUS
EFIAPI
AggregationSourceDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  AggregationSourceV1_2_0 = (EFI_REDFISH_AGGREGATIONSOURCE_V1_2_0 *)InterpProp;
  DestroyAggregationSource_V1_2_0_CS (AggregationSourceV1_2_0->AggregationSource); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AggregationSourceFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyAggregationSource_V1_2_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAggregationSource_V1_2_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishAggregationSource_V1_2_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
