...
//
#include <RedfishJsonStructure/Session/v1_1_1/EfiSessionV1_1_1.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Session_V1_1_1_To_CS (ResoruceRaw, &SessionV1_1_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SessionV1_1_1;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Session_V1_1_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Session_V1_1_1_JSON (*((EFI_REDFISH_SESSION_V1_1_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Session_V1_1_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroySession_V1_1_1_Json (ResoruceRaw);
}
