...
//
#include <RedfishJsonStructure/FabricAdapterCollection/EfiFabricAdapterCollection.h>
CHAR8 mResourceTypeStr [] = "#FabricAdapterCollection";
...
EFI_STATUS
EFIAPI
FabricAdapterCollectionToStruct (
...
{
  EFI_REDFISH_FABRICADAPTERCOLLECTION *FabricAdapterCollection;
  EFI_REDFISH_FABRICADAPTERCOLLECTION_CS *FabricAdapterCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "FabricAdapterCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_FabricAdapterCollection_To_CS (ResoruceRaw, &FabricAdapterCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  FabricAdapterCollection = (EFI_REDFISH_FABRICADAPTERCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_FABRICADAPTERCOLLECTION));
  if (FabricAdapterCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)FabricAdapterCollection;
  FabricAdapterCollection->FabricAdapterCollection = FabricAdapterCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("FabricAdapterCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("FabricAdapterCollection"), "FabricAdapterCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("FabricAdapterCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("FabricAdapterCollection"), "FabricAdapterCollection");
...
/**
  The wrapper function of FabricAdapterCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
FabricAdapterCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return FabricAdapterCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_FabricAdapterCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_FabricAdapterCollection_JSON (*((EFI_REDFISH_FABRICADAPTERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_FabricAdapterCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in FabricAdapterCollectionToStruct.
...
EFI_STATUS
EFIAPI
FabricAdapterCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  FabricAdapterCollection = (EFI_REDFISH_FABRICADAPTERCOLLECTION *)InterpProp;
  DestroyFabricAdapterCollection_CS (FabricAdapterCollection->FabricAdapterCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from FabricAdapterCollectionFromStruct
...
EFI_STATUS
EFIAPI
FabricAdapterCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyFabricAdapterCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishFabricAdapterCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishFabricAdapterCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
