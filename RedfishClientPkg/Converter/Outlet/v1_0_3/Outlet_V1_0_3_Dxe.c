...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Outlet_V1_0_3_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Outlet_V1_0_3_JSON (*((EFI_REDFISH_OUTLET_V1_0_3_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Outlet_V1_0_3_JSON fail (%d)\n", __func__, RedfishStatus));
  }
