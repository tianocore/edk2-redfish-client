...
//
#include <RedfishJsonStructure/Memory/v1_2_0/EfiMemoryV1_2_0.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Memory_V1_2_0_To_CS (ResoruceRaw, &MemoryV1_2_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)MemoryV1_2_0;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Memory_V1_2_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Memory_V1_2_0_JSON (*((EFI_REDFISH_MEMORY_V1_2_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Memory_V1_2_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyMemory_V1_2_0_Json (ResoruceRaw);
}
