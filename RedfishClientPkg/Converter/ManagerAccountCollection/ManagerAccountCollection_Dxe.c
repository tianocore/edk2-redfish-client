...
//
#include <RedfishJsonStructure/ManagerAccountCollection/EfiManagerAccountCollection.h>
...
EFI_STATUS
EFIAPI
ManagerAccountCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_ManagerAccountCollection_To_CS (ResoruceRaw, &ManagerAccountCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ManagerAccountCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ManagerAccountCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ManagerAccountCollection"), "ManagerAccountCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ManagerAccountCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ManagerAccountCollection"), "ManagerAccountCollection");
...
/**
  The wrapper function of ManagerAccountCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ManagerAccountCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ManagerAccountCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
ManagerAccountCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ManagerAccountCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ManagerAccountCollection_JSON (*((EFI_REDFISH_MANAGERACCOUNTCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ManagerAccountCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
ManagerAccountCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from ManagerAccountCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyManagerAccountCollection_Json (ResoruceRaw);
}
