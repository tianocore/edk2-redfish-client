...
//
#include <RedfishJsonStructure/SoftwareInventory/v1_0_0/EfiSoftwareInventoryV1_0_0.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_SoftwareInventory_V1_0_0_To_CS (ResoruceRaw, &SoftwareInventoryV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SoftwareInventoryV1_0_0;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_SoftwareInventory_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_SoftwareInventory_V1_0_0_JSON (*((EFI_REDFISH_SOFTWAREINVENTORY_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_SoftwareInventory_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroySoftwareInventory_V1_0_0_Json (ResoruceRaw);
}
