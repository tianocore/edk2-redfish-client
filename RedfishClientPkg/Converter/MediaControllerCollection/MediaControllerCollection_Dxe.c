...
//
#include <RedfishJsonStructure/MediaControllerCollection/EfiMediaControllerCollection.h>
CHAR8 mResourceTypeStr [] = "#MediaControllerCollection";
BOOLEAN IsRevisonController = FALSE;

// Support MediaControllerCollection 
...
EFI_STATUS
EFIAPI
MediaControllerCollectionToStruct (
...
{
  EFI_REDFISH_MEDIACONTROLLERCOLLECTION *MediaControllerCollection;
  EFI_REDFISH_MEDIACONTROLLERCOLLECTION_CS *MediaControllerCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MediaControllerCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_MediaControllerCollection_To_CS (ResoruceRaw, &MediaControllerCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MediaControllerCollection = (EFI_REDFISH_MEDIACONTROLLERCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MEDIACONTROLLERCOLLECTION));
  if (MediaControllerCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)MediaControllerCollection;
  MediaControllerCollection->MediaControllerCollection = MediaControllerCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MediaControllerCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("MediaControllerCollection"), "MediaControllerCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MediaControllerCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("MediaControllerCollection"), "MediaControllerCollection");
...
/**
  The wrapper function of MediaControllerCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
MediaControllerCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return MediaControllerCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
MediaControllerCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_MediaControllerCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_MediaControllerCollection_JSON (*((EFI_REDFISH_MEDIACONTROLLERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_MediaControllerCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in MediaControllerCollectionToStruct.
...
EFI_STATUS
EFIAPI
MediaControllerCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  DestroyMediaControllerCollection_CS (MediaControllerCollection->MediaControllerCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MediaControllerCollectionFromStruct
...
EFI_STATUS
EFIAPI
MediaControllerCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyMediaControllerCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMediaControllerCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMediaControllerCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
