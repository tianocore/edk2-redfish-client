...
//
#include <RedfishJsonStructure/MessageRegistryFileCollection/EfiMessageRegistryFileCollection.h>
...
EFI_STATUS
EFIAPI
MessageRegistryFileCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_MessageRegistryFileCollection_To_CS (ResoruceRaw, &MessageRegistryFileCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)MessageRegistryFileCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MessageRegistryFileCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("MessageRegistryFileCollection"), "MessageRegistryFileCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MessageRegistryFileCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("MessageRegistryFileCollection"), "MessageRegistryFileCollection");
...
/**
  The wrapper function of MessageRegistryFileCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
MessageRegistryFileCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return MessageRegistryFileCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
MessageRegistryFileCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_MessageRegistryFileCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_MessageRegistryFileCollection_JSON (*((EFI_REDFISH_MESSAGEREGISTRYFILECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_MessageRegistryFileCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
MessageRegistryFileCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from MessageRegistryFileCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyMessageRegistryFileCollection_Json (ResoruceRaw);
}
