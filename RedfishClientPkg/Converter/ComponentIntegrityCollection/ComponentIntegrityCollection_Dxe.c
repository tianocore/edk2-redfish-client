...
//
#include <RedfishJsonStructure/ComponentIntegrityCollection/EfiComponentIntegrityCollection.h>
CHAR8 mResourceTypeStr [] = "#ComponentIntegrityCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ComponentIntegrityCollection 
...
EFI_STATUS
EFIAPI
ComponentIntegrityCollectionToStruct (
...
{
  EFI_REDFISH_COMPONENTINTEGRITYCOLLECTION *ComponentIntegrityCollection;
  EFI_REDFISH_COMPONENTINTEGRITYCOLLECTION_CS *ComponentIntegrityCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ComponentIntegrityCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_ComponentIntegrityCollection_To_CS (ResoruceRaw, &ComponentIntegrityCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ComponentIntegrityCollection = (EFI_REDFISH_COMPONENTINTEGRITYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_COMPONENTINTEGRITYCOLLECTION));
  if (ComponentIntegrityCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ComponentIntegrityCollection;
  ComponentIntegrityCollection->ComponentIntegrityCollection = ComponentIntegrityCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ComponentIntegrityCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ComponentIntegrityCollection"), "ComponentIntegrityCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ComponentIntegrityCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ComponentIntegrityCollection"), "ComponentIntegrityCollection");
...
/**
  The wrapper function of ComponentIntegrityCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ComponentIntegrityCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ComponentIntegrityCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
ComponentIntegrityCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ComponentIntegrityCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ComponentIntegrityCollection_JSON (*((EFI_REDFISH_COMPONENTINTEGRITYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ComponentIntegrityCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ComponentIntegrityCollectionToStruct.
...
EFI_STATUS
EFIAPI
ComponentIntegrityCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  ComponentIntegrityCollection = (EFI_REDFISH_COMPONENTINTEGRITYCOLLECTION *)InterpProp;
  DestroyComponentIntegrityCollection_CS (ComponentIntegrityCollection->ComponentIntegrityCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ComponentIntegrityCollectionFromStruct
...
EFI_STATUS
EFIAPI
ComponentIntegrityCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyComponentIntegrityCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishComponentIntegrityCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishComponentIntegrityCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
