...
//
#include <RedfishJsonStructure/Aggregate/v1_0_1/EfiAggregateV1_0_1.h>
CHAR8 mResourceTypeStr [] = "#Aggregate";
BOOLEAN IsRevisonController = TRUE;

// Support Aggregate V1_0_1
...
{
  EFI_REDFISH_AGGREGATE_V1_0_1 *AggregateV1_0_1;
  EFI_REDFISH_AGGREGATE_V1_0_1_CS *AggregateV1_0_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Aggregate") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Aggregate_V1_0_1_To_CS (ResoruceRaw, &AggregateV1_0_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AggregateV1_0_1 = (EFI_REDFISH_AGGREGATE_V1_0_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_AGGREGATE_V1_0_1));
  if (AggregateV1_0_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AggregateV1_0_1;
  AggregateV1_0_1->Aggregate = AggregateV1_0_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Aggregate"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Aggregate"), "Aggregate");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Aggregate"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Aggregate"), "Aggregate");
...
/**
  The wrapper function of AggregateToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Aggregate_V1_0_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Aggregate_V1_0_1_JSON (*((EFI_REDFISH_AGGREGATE_V1_0_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Aggregate_V1_0_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AggregateToStruct.
...
)
{
  ...
  AggregateV1_0_1 = (EFI_REDFISH_AGGREGATE_V1_0_1 *)InterpProp;
  DestroyAggregate_V1_0_1_CS (AggregateV1_0_1->Aggregate); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AggregateFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyAggregate_V1_0_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAggregate_V1_0_1EntryPoint (
