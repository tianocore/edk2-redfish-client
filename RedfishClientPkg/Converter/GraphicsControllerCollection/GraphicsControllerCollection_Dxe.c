...
//
#include <RedfishJsonStructure/GraphicsControllerCollection/EfiGraphicsControllerCollection.h>
CHAR8 mResourceTypeStr [] = "#GraphicsControllerCollection";
BOOLEAN IsRevisonController = FALSE;

// Support GraphicsControllerCollection 
...
EFI_STATUS
EFIAPI
GraphicsControllerCollectionToStruct (
...
{
  EFI_REDFISH_GRAPHICSCONTROLLERCOLLECTION *GraphicsControllerCollection;
  EFI_REDFISH_GRAPHICSCONTROLLERCOLLECTION_CS *GraphicsControllerCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "GraphicsControllerCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_GraphicsControllerCollection_To_CS (ResoruceRaw, &GraphicsControllerCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  GraphicsControllerCollection = (EFI_REDFISH_GRAPHICSCONTROLLERCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_GRAPHICSCONTROLLERCOLLECTION));
  if (GraphicsControllerCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)GraphicsControllerCollection;
  GraphicsControllerCollection->GraphicsControllerCollection = GraphicsControllerCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("GraphicsControllerCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("GraphicsControllerCollection"), "GraphicsControllerCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("GraphicsControllerCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("GraphicsControllerCollection"), "GraphicsControllerCollection");
...
/**
  The wrapper function of GraphicsControllerCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
GraphicsControllerCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return GraphicsControllerCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
GraphicsControllerCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_GraphicsControllerCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_GraphicsControllerCollection_JSON (*((EFI_REDFISH_GRAPHICSCONTROLLERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_GraphicsControllerCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in GraphicsControllerCollectionToStruct.
...
EFI_STATUS
EFIAPI
GraphicsControllerCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  GraphicsControllerCollection = (EFI_REDFISH_GRAPHICSCONTROLLERCOLLECTION *)InterpProp;
  DestroyGraphicsControllerCollection_CS (GraphicsControllerCollection->GraphicsControllerCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from GraphicsControllerCollectionFromStruct
...
EFI_STATUS
EFIAPI
GraphicsControllerCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyGraphicsControllerCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishGraphicsControllerCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishGraphicsControllerCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
