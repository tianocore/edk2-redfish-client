...
//
#include <RedfishJsonStructure/ZoneCollection/EfiZoneCollection.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_ZoneCollection_To_CS (ResoruceRaw, &ZoneCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ZoneCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ZoneCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ZoneCollection"), "ZoneCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ZoneCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ZoneCollection"), "ZoneCollection");
...
/**
  The wrapper function of ZoneCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ZoneCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ZoneCollection_JSON (*((EFI_REDFISH_ZONECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ZoneCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
/**
  This function destory JSON raw text returned from ZoneCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyZoneCollection_Json (ResoruceRaw);
}
