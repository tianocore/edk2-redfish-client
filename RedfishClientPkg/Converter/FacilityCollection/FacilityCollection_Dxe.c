...
//
#include <RedfishJsonStructure/FacilityCollection/EfiFacilityCollection.h>
CHAR8 mResourceTypeStr [] = "#FacilityCollection";
BOOLEAN IsRevisonController = FALSE;

// Support FacilityCollection 
...
{
  EFI_REDFISH_FACILITYCOLLECTION *FacilityCollection;
  EFI_REDFISH_FACILITYCOLLECTION_CS *FacilityCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "FacilityCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_FacilityCollection_To_CS (ResoruceRaw, &FacilityCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  FacilityCollection = (EFI_REDFISH_FACILITYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_FACILITYCOLLECTION));
  if (FacilityCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)FacilityCollection;
  FacilityCollection->FacilityCollection = FacilityCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("FacilityCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("FacilityCollection"), "FacilityCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("FacilityCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("FacilityCollection"), "FacilityCollection");
...
/**
  The wrapper function of FacilityCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
FacilityCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return FacilityCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_FacilityCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_FacilityCollection_JSON (*((EFI_REDFISH_FACILITYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_FacilityCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in FacilityCollectionToStruct.
...
EFI_STATUS
EFIAPI
FacilityCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  FacilityCollection = (EFI_REDFISH_FACILITYCOLLECTION *)InterpProp;
  DestroyFacilityCollection_CS (FacilityCollection->FacilityCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from FacilityCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyFacilityCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishFacilityCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishFacilityCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
