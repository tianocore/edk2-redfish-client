...
//
#include <RedfishJsonStructure/LogEntry/v1_4_1/EfiLogEntryV1_4_1.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_LogEntry_V1_4_1_To_CS (ResoruceRaw, &LogEntryV1_4_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)LogEntryV1_4_1;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_LogEntry_V1_4_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_LogEntry_V1_4_1_JSON (*((EFI_REDFISH_LOGENTRY_V1_4_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_LogEntry_V1_4_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyLogEntry_V1_4_1_Json (ResoruceRaw);
}
