...
//
#include <RedfishJsonStructure/LogEntry/v1_0_4/EfiLogEntryV1_0_4.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_LogEntry_V1_0_4_To_CS (ResoruceRaw, &LogEntryV1_0_4Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)LogEntryV1_0_4;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_LogEntry_V1_0_4_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_LogEntry_V1_0_4_JSON (*((EFI_REDFISH_LOGENTRY_V1_0_4_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_LogEntry_V1_0_4_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyLogEntry_V1_0_4_Json (ResoruceRaw);
}
