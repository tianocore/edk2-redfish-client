...
//
#include <RedfishJsonStructure/LogEntry/v1_4_2/EfiLogEntryV1_4_2.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_LogEntry_V1_4_2_To_CS (ResoruceRaw, &LogEntryV1_4_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_LogEntry_V1_4_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_LogEntry_V1_4_2_JSON (*((EFI_REDFISH_LOGENTRY_V1_4_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_LogEntry_V1_4_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyLogEntry_V1_4_2_Json (ResoruceRaw);
}
