...
//
#include <RedfishJsonStructure/AccelerationFunctionCollection/EfiAccelerationFunctionCollection.h>
CHAR8 mResourceTypeStr [] = "#AccelerationFunctionCollection";
BOOLEAN IsRevisonController = FALSE;

// Support AccelerationFunctionCollection 
...
EFI_STATUS
EFIAPI
AccelerationFunctionCollectionToStruct (
...
{
  EFI_REDFISH_ACCELERATIONFUNCTIONCOLLECTION *AccelerationFunctionCollection;
  EFI_REDFISH_ACCELERATIONFUNCTIONCOLLECTION_CS *AccelerationFunctionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "AccelerationFunctionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_AccelerationFunctionCollection_To_CS (ResoruceRaw, &AccelerationFunctionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AccelerationFunctionCollection = (EFI_REDFISH_ACCELERATIONFUNCTIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ACCELERATIONFUNCTIONCOLLECTION));
  if (AccelerationFunctionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AccelerationFunctionCollection;
  AccelerationFunctionCollection->AccelerationFunctionCollection = AccelerationFunctionCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("AccelerationFunctionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("AccelerationFunctionCollection"), "AccelerationFunctionCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("AccelerationFunctionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("AccelerationFunctionCollection"), "AccelerationFunctionCollection");
...
/**
  The wrapper function of AccelerationFunctionCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
AccelerationFunctionCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return AccelerationFunctionCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
AccelerationFunctionCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AccelerationFunctionCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AccelerationFunctionCollection_JSON (*((EFI_REDFISH_ACCELERATIONFUNCTIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AccelerationFunctionCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AccelerationFunctionCollectionToStruct.
...
EFI_STATUS
EFIAPI
AccelerationFunctionCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  AccelerationFunctionCollection = (EFI_REDFISH_ACCELERATIONFUNCTIONCOLLECTION *)InterpProp;
  DestroyAccelerationFunctionCollection_CS (AccelerationFunctionCollection->AccelerationFunctionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AccelerationFunctionCollectionFromStruct
...
EFI_STATUS
EFIAPI
AccelerationFunctionCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyAccelerationFunctionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAccelerationFunctionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishAccelerationFunctionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
