...
//
#include <RedfishJsonStructure/FanCollection/EfiFanCollection.h>
CHAR8 mResourceTypeStr [] = "#FanCollection";
BOOLEAN IsRevisonController = FALSE;

// Support FanCollection 
...
{
  EFI_REDFISH_FANCOLLECTION *FanCollection;
  EFI_REDFISH_FANCOLLECTION_CS *FanCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "FanCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_FanCollection_To_CS (ResoruceRaw, &FanCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  FanCollection = (EFI_REDFISH_FANCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_FANCOLLECTION));
  if (FanCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)FanCollection;
  FanCollection->FanCollection = FanCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("FanCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("FanCollection"), "FanCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("FanCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("FanCollection"), "FanCollection");
...
/**
  The wrapper function of FanCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_FanCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_FanCollection_JSON (*((EFI_REDFISH_FANCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_FanCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in FanCollectionToStruct.
...
)
{
  ...
  FanCollection = (EFI_REDFISH_FANCOLLECTION *)InterpProp;
  DestroyFanCollection_CS (FanCollection->FanCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from FanCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyFanCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishFanCollectionEntryPoint (
