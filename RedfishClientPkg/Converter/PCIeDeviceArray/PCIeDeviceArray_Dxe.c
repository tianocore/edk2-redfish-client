...
CHAR8    mResourceTypeStr[]  = "#PCIeDeviceArray";
BOOLEAN  IsRevisonController = FALSE;

// Support PCIeDeviceArray
  ...
  )
{
  ...
  EFI_REDFISH_PCIEDEVICEARRAY_CS  *PCIeDeviceArrayCs;
      ...
      )
  {
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PCIeDeviceArray") != 0))
  {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PCIeDeviceArray_To_CS (ResoruceRaw, &PCIeDeviceArrayCs);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  PCIeDeviceArray = (EFI_REDFISH_PCIEDEVICEARRAY *)AllocateZeroPool (
                                                     sizeof (EFI_REDFISH_PCIEDEVICEARRAY)
                                                     );
  if (PCIeDeviceArray == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  RestIntrpHeader                  = (EFI_REST_JSON_STRUCTURE_HEADER *)PCIeDeviceArray;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PCIeDeviceArray"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PCIeDeviceArray"), "PCIeDeviceArray");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PCIeDeviceArray"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PCIeDeviceArray"), "PCIeDeviceArray");
  *InterpProp = RestIntrpHeader;
  return EFI_SUCCESS;
}

/**
  The wrapper function of PCIeDeviceArrayToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
PCIeDeviceArrayToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL        *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER  *RsrcIdentifier OPTIONAL,
  IN CHAR8                                   *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER         **InterpProp
...
{
  return PCIeDeviceArrayToStruct (
           This,
           RsrcIdentifier,
           ResoruceRaw,
           NULL,
  ...
  )
{
  ...
  DEBUG ((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PCIeDeviceArray_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PCIeDeviceArray_JSON (*((EFI_REDFISH_PCIEDEVICEARRAY_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR (RedfishStatus)) {
    DEBUG ((DEBUG_MANAGEABILITY, "%a Call to CS_To_PCIeDeviceArray_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
/**
  This function destory structure retunred in PCIeDeviceArrayToStruct.
  ...
  )
{
  ...
  PCIeDeviceArray = (EFI_REDFISH_PCIEDEVICEARRAY *)InterpProp;
  DestroyPCIeDeviceArray_CS (PCIeDeviceArray->PCIeDeviceArray);
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PCIeDeviceArrayFromStruct
  ...
  )
{
  ...
  return (EFI_STATUS)DestroyPCIeDeviceArray_Json (ResoruceRaw);
}
