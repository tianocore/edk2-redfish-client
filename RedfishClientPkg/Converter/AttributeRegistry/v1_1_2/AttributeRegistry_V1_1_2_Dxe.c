...
//
#include <RedfishJsonStructure/AttributeRegistry/v1_1_2/EfiAttributeRegistryV1_1_2.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_AttributeRegistry_V1_1_2_To_CS (ResoruceRaw, &AttributeRegistryV1_1_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AttributeRegistryV1_1_2;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AttributeRegistry_V1_1_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AttributeRegistry_V1_1_2_JSON (*((EFI_REDFISH_ATTRIBUTEREGISTRY_V1_1_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AttributeRegistry_V1_1_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyAttributeRegistry_V1_1_2_Json (ResoruceRaw);
}
