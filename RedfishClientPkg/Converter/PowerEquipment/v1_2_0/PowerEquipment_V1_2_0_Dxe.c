...
//
#include <RedfishJsonStructure/PowerEquipment/v1_2_0/EfiPowerEquipmentV1_2_0.h>
CHAR8 mResourceTypeStr [] = "#PowerEquipment";
BOOLEAN IsRevisonController = TRUE;

// Support PowerEquipment V1_2_0
...
{
  EFI_REDFISH_POWEREQUIPMENT_V1_2_0 *PowerEquipmentV1_2_0;
  EFI_REDFISH_POWEREQUIPMENT_V1_2_0_CS *PowerEquipmentV1_2_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PowerEquipment") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PowerEquipment_V1_2_0_To_CS (ResoruceRaw, &PowerEquipmentV1_2_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerEquipmentV1_2_0 = (EFI_REDFISH_POWEREQUIPMENT_V1_2_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWEREQUIPMENT_V1_2_0));
  if (PowerEquipmentV1_2_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerEquipmentV1_2_0;
  PowerEquipmentV1_2_0->PowerEquipment = PowerEquipmentV1_2_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerEquipment"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PowerEquipment"), "PowerEquipment");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerEquipment"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PowerEquipment"), "PowerEquipment");
...
/**
  The wrapper function of PowerEquipmentToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PowerEquipment_V1_2_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PowerEquipment_V1_2_0_JSON (*((EFI_REDFISH_POWEREQUIPMENT_V1_2_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PowerEquipment_V1_2_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PowerEquipmentToStruct.
...
)
{
  ...
  PowerEquipmentV1_2_0 = (EFI_REDFISH_POWEREQUIPMENT_V1_2_0 *)InterpProp;
  DestroyPowerEquipment_V1_2_0_CS (PowerEquipmentV1_2_0->PowerEquipment); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerEquipmentFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyPowerEquipment_V1_2_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPowerEquipment_V1_2_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPowerEquipment_V1_2_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
