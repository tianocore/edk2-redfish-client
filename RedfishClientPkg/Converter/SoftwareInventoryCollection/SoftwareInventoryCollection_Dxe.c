...
//
#include <RedfishJsonStructure/SoftwareInventoryCollection/EfiSoftwareInventoryCollection.h>
...
EFI_STATUS
EFIAPI
SoftwareInventoryCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_SoftwareInventoryCollection_To_CS (ResoruceRaw, &SoftwareInventoryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SoftwareInventoryCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SoftwareInventoryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("SoftwareInventoryCollection"), "SoftwareInventoryCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SoftwareInventoryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("SoftwareInventoryCollection"), "SoftwareInventoryCollection");
...
/**
  The wrapper function of SoftwareInventoryCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
SoftwareInventoryCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return SoftwareInventoryCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
SoftwareInventoryCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_SoftwareInventoryCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_SoftwareInventoryCollection_JSON (*((EFI_REDFISH_SOFTWAREINVENTORYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_SoftwareInventoryCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
SoftwareInventoryCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from SoftwareInventoryCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroySoftwareInventoryCollection_Json (ResoruceRaw);
}
