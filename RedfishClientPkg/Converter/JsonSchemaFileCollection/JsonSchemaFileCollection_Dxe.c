...
//
#include <RedfishJsonStructure/JsonSchemaFileCollection/EfiJsonSchemaFileCollection.h>
...
EFI_STATUS
EFIAPI
JsonSchemaFileCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_JsonSchemaFileCollection_To_CS (ResoruceRaw, &JsonSchemaFileCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)JsonSchemaFileCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("JsonSchemaFileCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("JsonSchemaFileCollection"), "JsonSchemaFileCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("JsonSchemaFileCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("JsonSchemaFileCollection"), "JsonSchemaFileCollection");
...
/**
  The wrapper function of JsonSchemaFileCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
JsonSchemaFileCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return JsonSchemaFileCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
JsonSchemaFileCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_JsonSchemaFileCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_JsonSchemaFileCollection_JSON (*((EFI_REDFISH_JSONSCHEMAFILECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_JsonSchemaFileCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
JsonSchemaFileCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from JsonSchemaFileCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyJsonSchemaFileCollection_Json (ResoruceRaw);
}
