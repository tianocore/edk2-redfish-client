...
//
#include <RedfishJsonStructure/SecureBootDatabase/v1_0_0/EfiSecureBootDatabaseV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#SecureBootDatabase";
BOOLEAN IsRevisonController = TRUE;

// Support SecureBootDatabase V1_0_0
...
{
  EFI_REDFISH_SECUREBOOTDATABASE_V1_0_0 *SecureBootDatabaseV1_0_0;
  EFI_REDFISH_SECUREBOOTDATABASE_V1_0_0_CS *SecureBootDatabaseV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "SecureBootDatabase") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_SecureBootDatabase_V1_0_0_To_CS (ResoruceRaw, &SecureBootDatabaseV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SecureBootDatabaseV1_0_0 = (EFI_REDFISH_SECUREBOOTDATABASE_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SECUREBOOTDATABASE_V1_0_0));
  if (SecureBootDatabaseV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SecureBootDatabaseV1_0_0;
  SecureBootDatabaseV1_0_0->SecureBootDatabase = SecureBootDatabaseV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SecureBootDatabase"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("SecureBootDatabase"), "SecureBootDatabase");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SecureBootDatabase"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("SecureBootDatabase"), "SecureBootDatabase");
...
/**
  The wrapper function of SecureBootDatabaseToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
SecureBootDatabaseToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return SecureBootDatabaseToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_SecureBootDatabase_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_SecureBootDatabase_V1_0_0_JSON (*((EFI_REDFISH_SECUREBOOTDATABASE_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_SecureBootDatabase_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in SecureBootDatabaseToStruct.
...
EFI_STATUS
EFIAPI
SecureBootDatabaseDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  SecureBootDatabaseV1_0_0 = (EFI_REDFISH_SECUREBOOTDATABASE_V1_0_0 *)InterpProp;
  DestroySecureBootDatabase_V1_0_0_CS (SecureBootDatabaseV1_0_0->SecureBootDatabase); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SecureBootDatabaseFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroySecureBootDatabase_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSecureBootDatabase_V1_0_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishSecureBootDatabase_V1_0_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
