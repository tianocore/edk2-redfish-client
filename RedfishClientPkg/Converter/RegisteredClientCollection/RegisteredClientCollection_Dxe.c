...
//
#include <RedfishJsonStructure/RegisteredClientCollection/EfiRegisteredClientCollection.h>
CHAR8 mResourceTypeStr [] = "#RegisteredClientCollection";
BOOLEAN IsRevisonController = FALSE;

// Support RegisteredClientCollection 
...
EFI_STATUS
EFIAPI
RegisteredClientCollectionToStruct (
...
{
  EFI_REDFISH_REGISTEREDCLIENTCOLLECTION *RegisteredClientCollection;
  EFI_REDFISH_REGISTEREDCLIENTCOLLECTION_CS *RegisteredClientCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "RegisteredClientCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_RegisteredClientCollection_To_CS (ResoruceRaw, &RegisteredClientCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  RegisteredClientCollection = (EFI_REDFISH_REGISTEREDCLIENTCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_REGISTEREDCLIENTCOLLECTION));
  if (RegisteredClientCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)RegisteredClientCollection;
  RegisteredClientCollection->RegisteredClientCollection = RegisteredClientCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("RegisteredClientCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("RegisteredClientCollection"), "RegisteredClientCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("RegisteredClientCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("RegisteredClientCollection"), "RegisteredClientCollection");
...
/**
  The wrapper function of RegisteredClientCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
RegisteredClientCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return RegisteredClientCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
RegisteredClientCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_RegisteredClientCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_RegisteredClientCollection_JSON (*((EFI_REDFISH_REGISTEREDCLIENTCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_RegisteredClientCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in RegisteredClientCollectionToStruct.
...
EFI_STATUS
EFIAPI
RegisteredClientCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  RegisteredClientCollection = (EFI_REDFISH_REGISTEREDCLIENTCOLLECTION *)InterpProp;
  DestroyRegisteredClientCollection_CS (RegisteredClientCollection->RegisteredClientCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from RegisteredClientCollectionFromStruct
...
EFI_STATUS
EFIAPI
RegisteredClientCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyRegisteredClientCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishRegisteredClientCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishRegisteredClientCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
