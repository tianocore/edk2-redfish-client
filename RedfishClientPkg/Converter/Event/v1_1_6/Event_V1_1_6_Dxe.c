...
//
#include <RedfishJsonStructure/Event/v1_1_6/EfiEventV1_1_6.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Event_V1_1_6_To_CS (ResoruceRaw, &EventV1_1_6Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)EventV1_1_6;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Event_V1_1_6_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Event_V1_1_6_JSON (*((EFI_REDFISH_EVENT_V1_1_6_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Event_V1_1_6_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyEvent_V1_1_6_Json (ResoruceRaw);
}
