...
//
#include <RedfishJsonStructure/ExternalAccountProviderCollection/EfiExternalAccountProviderCollection.h>
...
EFI_STATUS
EFIAPI
ExternalAccountProviderCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_ExternalAccountProviderCollection_To_CS (ResoruceRaw, &ExternalAccountProviderCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ExternalAccountProviderCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ExternalAccountProviderCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ExternalAccountProviderCollection"), "ExternalAccountProviderCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ExternalAccountProviderCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ExternalAccountProviderCollection"), "ExternalAccountProviderCollection");
...
/**
  The wrapper function of ExternalAccountProviderCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ExternalAccountProviderCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ExternalAccountProviderCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
ExternalAccountProviderCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ExternalAccountProviderCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ExternalAccountProviderCollection_JSON (*((EFI_REDFISH_EXTERNALACCOUNTPROVIDERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ExternalAccountProviderCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
ExternalAccountProviderCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from ExternalAccountProviderCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyExternalAccountProviderCollection_Json (ResoruceRaw);
}
