...
//
#include <RedfishJsonStructure/EventService/v1_0_8/EfiEventServiceV1_0_8.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_EventService_V1_0_8_To_CS (ResoruceRaw, &EventServiceV1_0_8Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)EventServiceV1_0_8;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_EventService_V1_0_8_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_EventService_V1_0_8_JSON (*((EFI_REDFISH_EVENTSERVICE_V1_0_8_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_EventService_V1_0_8_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyEventService_V1_0_8_Json (ResoruceRaw);
}
