...
//
#include <RedfishJsonStructure/VLanNetworkInterfaceCollection/EfiVLanNetworkInterfaceCollection.h>
...
EFI_STATUS
EFIAPI
VLanNetworkInterfaceCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_VLanNetworkInterfaceCollection_To_CS (ResoruceRaw, &VLanNetworkInterfaceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)VLanNetworkInterfaceCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("VLanNetworkInterfaceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("VLanNetworkInterfaceCollection"), "VLanNetworkInterfaceCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("VLanNetworkInterfaceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("VLanNetworkInterfaceCollection"), "VLanNetworkInterfaceCollection");
...
/**
  The wrapper function of VLanNetworkInterfaceCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
VLanNetworkInterfaceCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return VLanNetworkInterfaceCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
VLanNetworkInterfaceCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_VLanNetworkInterfaceCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_VLanNetworkInterfaceCollection_JSON (*((EFI_REDFISH_VLANNETWORKINTERFACECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_VLanNetworkInterfaceCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
VLanNetworkInterfaceCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from VLanNetworkInterfaceCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyVLanNetworkInterfaceCollection_Json (ResoruceRaw);
}
