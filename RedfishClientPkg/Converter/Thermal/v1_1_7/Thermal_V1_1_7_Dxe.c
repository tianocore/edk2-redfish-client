...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Thermal_V1_1_7_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Thermal_V1_1_7_JSON (*((EFI_REDFISH_THERMAL_V1_1_7_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Thermal_V1_1_7_JSON fail (%d)\n", __func__, RedfishStatus));
  }
