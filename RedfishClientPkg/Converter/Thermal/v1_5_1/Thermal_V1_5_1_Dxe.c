...
//
#include <RedfishJsonStructure/Thermal/v1_5_1/EfiThermalV1_5_1.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Thermal_V1_5_1_To_CS (ResoruceRaw, &ThermalV1_5_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ThermalV1_5_1;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Thermal_V1_5_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Thermal_V1_5_1_JSON (*((EFI_REDFISH_THERMAL_V1_5_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Thermal_V1_5_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyThermal_V1_5_1_Json (ResoruceRaw);
}
