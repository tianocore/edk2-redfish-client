...
//
#include <RedfishJsonStructure/Thermal/v1_4_2/EfiThermalV1_4_2.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Thermal_V1_4_2_To_CS (ResoruceRaw, &ThermalV1_4_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ThermalV1_4_2;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Thermal_V1_4_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Thermal_V1_4_2_JSON (*((EFI_REDFISH_THERMAL_V1_4_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Thermal_V1_4_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyThermal_V1_4_2_Json (ResoruceRaw);
}
