...
//
#include <RedfishJsonStructure/Chassis/v1_1_5/EfiChassisV1_1_5.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Chassis_V1_1_5_To_CS (ResoruceRaw, &ChassisV1_1_5Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ChassisV1_1_5;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Chassis_V1_1_5_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Chassis_V1_1_5_JSON (*((EFI_REDFISH_CHASSIS_V1_1_5_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Chassis_V1_1_5_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyChassis_V1_1_5_Json (ResoruceRaw);
}
