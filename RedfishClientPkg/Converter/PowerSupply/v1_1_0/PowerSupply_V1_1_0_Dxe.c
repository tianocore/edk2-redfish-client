...
//
#include <RedfishJsonStructure/PowerSupply/v1_1_0/EfiPowerSupplyV1_1_0.h>
CHAR8 mResourceTypeStr [] = "#PowerSupply";
BOOLEAN IsRevisonController = TRUE;

// Support PowerSupply V1_1_0
...
{
  EFI_REDFISH_POWERSUPPLY_V1_1_0 *PowerSupplyV1_1_0;
  EFI_REDFISH_POWERSUPPLY_V1_1_0_CS *PowerSupplyV1_1_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PowerSupply") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PowerSupply_V1_1_0_To_CS (ResoruceRaw, &PowerSupplyV1_1_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerSupplyV1_1_0 = (EFI_REDFISH_POWERSUPPLY_V1_1_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWERSUPPLY_V1_1_0));
  if (PowerSupplyV1_1_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerSupplyV1_1_0;
  PowerSupplyV1_1_0->PowerSupply = PowerSupplyV1_1_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerSupply"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PowerSupply"), "PowerSupply");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerSupply"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PowerSupply"), "PowerSupply");
...
/**
  The wrapper function of PowerSupplyToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PowerSupply_V1_1_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PowerSupply_V1_1_0_JSON (*((EFI_REDFISH_POWERSUPPLY_V1_1_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PowerSupply_V1_1_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PowerSupplyToStruct.
...
)
{
  ...
  PowerSupplyV1_1_0 = (EFI_REDFISH_POWERSUPPLY_V1_1_0 *)InterpProp;
  DestroyPowerSupply_V1_1_0_CS (PowerSupplyV1_1_0->PowerSupply); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerSupplyFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyPowerSupply_V1_1_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPowerSupply_V1_1_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPowerSupply_V1_1_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
