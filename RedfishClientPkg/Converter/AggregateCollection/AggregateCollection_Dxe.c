...
//
#include <RedfishJsonStructure/AggregateCollection/EfiAggregateCollection.h>
CHAR8 mResourceTypeStr [] = "#AggregateCollection";
BOOLEAN IsRevisonController = FALSE;

// Support AggregateCollection 
...
{
  EFI_REDFISH_AGGREGATECOLLECTION *AggregateCollection;
  EFI_REDFISH_AGGREGATECOLLECTION_CS *AggregateCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "AggregateCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_AggregateCollection_To_CS (ResoruceRaw, &AggregateCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AggregateCollection = (EFI_REDFISH_AGGREGATECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_AGGREGATECOLLECTION));
  if (AggregateCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AggregateCollection;
  AggregateCollection->AggregateCollection = AggregateCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("AggregateCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("AggregateCollection"), "AggregateCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("AggregateCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("AggregateCollection"), "AggregateCollection");
...
/**
  The wrapper function of AggregateCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
AggregateCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return AggregateCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AggregateCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AggregateCollection_JSON (*((EFI_REDFISH_AGGREGATECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AggregateCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AggregateCollectionToStruct.
...
EFI_STATUS
EFIAPI
AggregateCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  AggregateCollection = (EFI_REDFISH_AGGREGATECOLLECTION *)InterpProp;
  DestroyAggregateCollection_CS (AggregateCollection->AggregateCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AggregateCollectionFromStruct
...
EFI_STATUS
EFIAPI
AggregateCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyAggregateCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAggregateCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishAggregateCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
