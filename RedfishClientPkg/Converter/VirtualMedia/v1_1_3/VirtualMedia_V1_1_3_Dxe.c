...
//
#include <RedfishJsonStructure/VirtualMedia/v1_1_3/EfiVirtualMediaV1_1_3.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_VirtualMedia_V1_1_3_To_CS (ResoruceRaw, &VirtualMediaV1_1_3Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)VirtualMediaV1_1_3;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_VirtualMedia_V1_1_3_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_VirtualMedia_V1_1_3_JSON (*((EFI_REDFISH_VIRTUALMEDIA_V1_1_3_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_VirtualMedia_V1_1_3_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyVirtualMedia_V1_1_3_Json (ResoruceRaw);
}
