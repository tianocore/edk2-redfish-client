...
//
#include <RedfishJsonStructure/Fan/v1_0_2/EfiFanV1_0_2.h>
CHAR8 mResourceTypeStr [] = "#Fan";
...
{
  EFI_REDFISH_FAN_V1_0_2 *FanV1_0_2;
  EFI_REDFISH_FAN_V1_0_2_CS *FanV1_0_2Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Fan") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Fan_V1_0_2_To_CS (ResoruceRaw, &FanV1_0_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  FanV1_0_2 = (EFI_REDFISH_FAN_V1_0_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_FAN_V1_0_2));
  if (FanV1_0_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)FanV1_0_2;
  FanV1_0_2->Fan = FanV1_0_2Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Fan"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Fan"), "Fan");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Fan"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Fan"), "Fan");
...
/**
  The wrapper function of FanToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Fan_V1_0_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Fan_V1_0_2_JSON (*((EFI_REDFISH_FAN_V1_0_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Fan_V1_0_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in FanToStruct.
...
)
{
  ...
  FanV1_0_2 = (EFI_REDFISH_FAN_V1_0_2 *)InterpProp;
  DestroyFan_V1_0_2_CS (FanV1_0_2->Fan); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from FanFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyFan_V1_0_2_Json (ResoruceRaw);
}
