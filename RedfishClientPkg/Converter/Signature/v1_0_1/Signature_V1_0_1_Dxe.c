...
//
#include <RedfishJsonStructure/Signature/v1_0_1/EfiSignatureV1_0_1.h>
CHAR8 mResourceTypeStr [] = "#Signature";
BOOLEAN IsRevisonController = TRUE;

// Support Signature V1_0_1
...
{
  EFI_REDFISH_SIGNATURE_V1_0_1 *SignatureV1_0_1;
  EFI_REDFISH_SIGNATURE_V1_0_1_CS *SignatureV1_0_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Signature") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Signature_V1_0_1_To_CS (ResoruceRaw, &SignatureV1_0_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SignatureV1_0_1 = (EFI_REDFISH_SIGNATURE_V1_0_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SIGNATURE_V1_0_1));
  if (SignatureV1_0_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SignatureV1_0_1;
  SignatureV1_0_1->Signature = SignatureV1_0_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Signature"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Signature"), "Signature");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Signature"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Signature"), "Signature");
...
/**
  The wrapper function of SignatureToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Signature_V1_0_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Signature_V1_0_1_JSON (*((EFI_REDFISH_SIGNATURE_V1_0_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Signature_V1_0_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in SignatureToStruct.
...
)
{
  ...
  SignatureV1_0_1 = (EFI_REDFISH_SIGNATURE_V1_0_1 *)InterpProp;
  DestroySignature_V1_0_1_CS (SignatureV1_0_1->Signature); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SignatureFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroySignature_V1_0_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSignature_V1_0_1EntryPoint (
