...
//
#include <RedfishJsonStructure/CertificateCollection/EfiCertificateCollection.h>
CHAR8 mResourceTypeStr [] = "#CertificateCollection";
...
{
  EFI_REDFISH_CERTIFICATECOLLECTION *CertificateCollection;
  EFI_REDFISH_CERTIFICATECOLLECTION_CS *CertificateCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "CertificateCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_CertificateCollection_To_CS (ResoruceRaw, &CertificateCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  CertificateCollection = (EFI_REDFISH_CERTIFICATECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CERTIFICATECOLLECTION));
  if (CertificateCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)CertificateCollection;
  CertificateCollection->CertificateCollection = CertificateCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("CertificateCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("CertificateCollection"), "CertificateCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("CertificateCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("CertificateCollection"), "CertificateCollection");
...
/**
  The wrapper function of CertificateCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
CertificateCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return CertificateCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_CertificateCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_CertificateCollection_JSON (*((EFI_REDFISH_CERTIFICATECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_CertificateCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in CertificateCollectionToStruct.
...
EFI_STATUS
EFIAPI
CertificateCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  CertificateCollection = (EFI_REDFISH_CERTIFICATECOLLECTION *)InterpProp;
  DestroyCertificateCollection_CS (CertificateCollection->CertificateCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from CertificateCollectionFromStruct
...
EFI_STATUS
EFIAPI
CertificateCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyCertificateCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishCertificateCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishCertificateCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
