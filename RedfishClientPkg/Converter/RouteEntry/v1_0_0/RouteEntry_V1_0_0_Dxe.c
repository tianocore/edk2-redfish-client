...
//
#include <RedfishJsonStructure/RouteEntry/v1_0_0/EfiRouteEntryV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#RouteEntry";
BOOLEAN IsRevisonController = TRUE;

// Support RouteEntry V1_0_0
...
{
  EFI_REDFISH_ROUTEENTRY_V1_0_0 *RouteEntryV1_0_0;
  EFI_REDFISH_ROUTEENTRY_V1_0_0_CS *RouteEntryV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "RouteEntry") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_RouteEntry_V1_0_0_To_CS (ResoruceRaw, &RouteEntryV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  RouteEntryV1_0_0 = (EFI_REDFISH_ROUTEENTRY_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ROUTEENTRY_V1_0_0));
  if (RouteEntryV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)RouteEntryV1_0_0;
  RouteEntryV1_0_0->RouteEntry = RouteEntryV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("RouteEntry"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("RouteEntry"), "RouteEntry");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("RouteEntry"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("RouteEntry"), "RouteEntry");
...
/**
  The wrapper function of RouteEntryToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_RouteEntry_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_RouteEntry_V1_0_0_JSON (*((EFI_REDFISH_ROUTEENTRY_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_RouteEntry_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in RouteEntryToStruct.
...
)
{
  ...
  RouteEntryV1_0_0 = (EFI_REDFISH_ROUTEENTRY_V1_0_0 *)InterpProp;
  DestroyRouteEntry_V1_0_0_CS (RouteEntryV1_0_0->RouteEntry); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from RouteEntryFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyRouteEntry_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishRouteEntry_V1_0_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishRouteEntry_V1_0_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
