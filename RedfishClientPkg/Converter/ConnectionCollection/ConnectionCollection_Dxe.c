...
//
#include <RedfishJsonStructure/ConnectionCollection/EfiConnectionCollection.h>
CHAR8 mResourceTypeStr [] = "#ConnectionCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ConnectionCollection 
...
{
  EFI_REDFISH_CONNECTIONCOLLECTION *ConnectionCollection;
  EFI_REDFISH_CONNECTIONCOLLECTION_CS *ConnectionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ConnectionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_ConnectionCollection_To_CS (ResoruceRaw, &ConnectionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ConnectionCollection = (EFI_REDFISH_CONNECTIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CONNECTIONCOLLECTION));
  if (ConnectionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ConnectionCollection;
  ConnectionCollection->ConnectionCollection = ConnectionCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ConnectionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ConnectionCollection"), "ConnectionCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ConnectionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ConnectionCollection"), "ConnectionCollection");
...
/**
  The wrapper function of ConnectionCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ConnectionCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ConnectionCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ConnectionCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ConnectionCollection_JSON (*((EFI_REDFISH_CONNECTIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ConnectionCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ConnectionCollectionToStruct.
...
EFI_STATUS
EFIAPI
ConnectionCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  ConnectionCollection = (EFI_REDFISH_CONNECTIONCOLLECTION *)InterpProp;
  DestroyConnectionCollection_CS (ConnectionCollection->ConnectionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ConnectionCollectionFromStruct
...
EFI_STATUS
EFIAPI
ConnectionCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyConnectionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishConnectionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishConnectionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
