...
//
#include <RedfishJsonStructure/PCIeFunctionCollection/EfiPCIeFunctionCollection.h>
CHAR8 mResourceTypeStr [] = "#PCIeFunctionCollection";
BOOLEAN IsRevisonController = FALSE;

// Support PCIeFunctionCollection 
...
EFI_STATUS
EFIAPI
PCIeFunctionCollectionToStruct (
...
{
  EFI_REDFISH_PCIEFUNCTIONCOLLECTION *PCIeFunctionCollection;
  EFI_REDFISH_PCIEFUNCTIONCOLLECTION_CS *PCIeFunctionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PCIeFunctionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PCIeFunctionCollection_To_CS (ResoruceRaw, &PCIeFunctionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PCIeFunctionCollection = (EFI_REDFISH_PCIEFUNCTIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_PCIEFUNCTIONCOLLECTION));
  if (PCIeFunctionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PCIeFunctionCollection;
  PCIeFunctionCollection->PCIeFunctionCollection = PCIeFunctionCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PCIeFunctionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PCIeFunctionCollection"), "PCIeFunctionCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PCIeFunctionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PCIeFunctionCollection"), "PCIeFunctionCollection");
...
/**
  The wrapper function of PCIeFunctionCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
PCIeFunctionCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return PCIeFunctionCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PCIeFunctionCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PCIeFunctionCollection_JSON (*((EFI_REDFISH_PCIEFUNCTIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PCIeFunctionCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PCIeFunctionCollectionToStruct.
...
EFI_STATUS
EFIAPI
PCIeFunctionCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  PCIeFunctionCollection = (EFI_REDFISH_PCIEFUNCTIONCOLLECTION *)InterpProp;
  DestroyPCIeFunctionCollection_CS (PCIeFunctionCollection->PCIeFunctionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PCIeFunctionCollectionFromStruct
...
EFI_STATUS
EFIAPI
PCIeFunctionCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyPCIeFunctionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPCIeFunctionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPCIeFunctionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
