...
//
#include <RedfishJsonStructure/Endpoint/v1_0_3/EfiEndpointV1_0_3.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Endpoint_V1_0_3_To_CS (ResoruceRaw, &EndpointV1_0_3Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)EndpointV1_0_3;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Endpoint_V1_0_3_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Endpoint_V1_0_3_JSON (*((EFI_REDFISH_ENDPOINT_V1_0_3_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Endpoint_V1_0_3_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyEndpoint_V1_0_3_Json (ResoruceRaw);
}
