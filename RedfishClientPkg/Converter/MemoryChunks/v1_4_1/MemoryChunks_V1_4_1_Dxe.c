...
//
#include <RedfishJsonStructure/MemoryChunks/v1_4_1/EfiMemoryChunksV1_4_1.h>
CHAR8 mResourceTypeStr [] = "#MemoryChunks";
BOOLEAN IsRevisonController = TRUE;

// Support MemoryChunks V1_4_1
...
{
  EFI_REDFISH_MEMORYCHUNKS_V1_4_1 *MemoryChunksV1_4_1;
  EFI_REDFISH_MEMORYCHUNKS_V1_4_1_CS *MemoryChunksV1_4_1Cs;
  ...
  Status = (EFI_STATUS)Json_MemoryChunks_V1_4_1_To_CS (ResoruceRaw, &MemoryChunksV1_4_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MemoryChunksV1_4_1 = (EFI_REDFISH_MEMORYCHUNKS_V1_4_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MEMORYCHUNKS_V1_4_1));
  if (MemoryChunksV1_4_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)MemoryChunksV1_4_1;
  MemoryChunksV1_4_1->MemoryChunks = MemoryChunksV1_4_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryChunks"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("MemoryChunks"), "MemoryChunks");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryChunks"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("MemoryChunks"), "MemoryChunks");
...
/**
  The wrapper function of MemoryChunksToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_MemoryChunks_V1_4_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_MemoryChunks_V1_4_1_JSON (*((EFI_REDFISH_MEMORYCHUNKS_V1_4_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_MemoryChunks_V1_4_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  MemoryChunksV1_4_1 = (EFI_REDFISH_MEMORYCHUNKS_V1_4_1 *)InterpProp;
  DestroyMemoryChunks_V1_4_1_CS (MemoryChunksV1_4_1->MemoryChunks); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MemoryChunksFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyMemoryChunks_V1_4_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMemoryChunks_V1_4_1EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMemoryChunks_V1_4_1Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
