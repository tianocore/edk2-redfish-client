...
//
#include <RedfishJsonStructure/MemoryChunks/v1_0_1/EfiMemoryChunksV1_0_1.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_MemoryChunks_V1_0_1_To_CS (ResoruceRaw, &MemoryChunksV1_0_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)MemoryChunksV1_0_1;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_MemoryChunks_V1_0_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_MemoryChunks_V1_0_1_JSON (*((EFI_REDFISH_MEMORYCHUNKS_V1_0_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_MemoryChunks_V1_0_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyMemoryChunks_V1_0_1_Json (ResoruceRaw);
}
