...
//
#include <RedfishJsonStructure/EndpointGroup/v1_0_2/EfiEndpointGroupV1_0_2.h>
CHAR8 mResourceTypeStr [] = "#EndpointGroup";
BOOLEAN IsRevisonController = TRUE;

// Support EndpointGroup V1_0_2
...
{
  EFI_REDFISH_ENDPOINTGROUP_V1_0_2 *EndpointGroupV1_0_2;
  EFI_REDFISH_ENDPOINTGROUP_V1_0_2_CS *EndpointGroupV1_0_2Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "EndpointGroup") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_EndpointGroup_V1_0_2_To_CS (ResoruceRaw, &EndpointGroupV1_0_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  EndpointGroupV1_0_2 = (EFI_REDFISH_ENDPOINTGROUP_V1_0_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ENDPOINTGROUP_V1_0_2));
  if (EndpointGroupV1_0_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)EndpointGroupV1_0_2;
  EndpointGroupV1_0_2->EndpointGroup = EndpointGroupV1_0_2Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("EndpointGroup"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("EndpointGroup"), "EndpointGroup");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("EndpointGroup"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("EndpointGroup"), "EndpointGroup");
...
/**
  The wrapper function of EndpointGroupToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_EndpointGroup_V1_0_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_EndpointGroup_V1_0_2_JSON (*((EFI_REDFISH_ENDPOINTGROUP_V1_0_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_EndpointGroup_V1_0_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in EndpointGroupToStruct.
...
)
{
  ...
  EndpointGroupV1_0_2 = (EFI_REDFISH_ENDPOINTGROUP_V1_0_2 *)InterpProp;
  DestroyEndpointGroup_V1_0_2_CS (EndpointGroupV1_0_2->EndpointGroup); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from EndpointGroupFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyEndpointGroup_V1_0_2_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishEndpointGroup_V1_0_2EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishEndpointGroup_V1_0_2Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
