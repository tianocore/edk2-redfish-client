...
//
#include <RedfishJsonStructure/Processor/v1_3_2/EfiProcessorV1_3_2.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Processor_V1_3_2_To_CS (ResoruceRaw, &ProcessorV1_3_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ProcessorV1_3_2;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Processor_V1_3_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Processor_V1_3_2_JSON (*((EFI_REDFISH_PROCESSOR_V1_3_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Processor_V1_3_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyProcessor_V1_3_2_Json (ResoruceRaw);
}
