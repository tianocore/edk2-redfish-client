...
//
#include <RedfishJsonStructure/Processor/v1_0_6/EfiProcessorV1_0_6.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Processor_V1_0_6_To_CS (ResoruceRaw, &ProcessorV1_0_6Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ProcessorV1_0_6;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Processor_V1_0_6_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Processor_V1_0_6_JSON (*((EFI_REDFISH_PROCESSOR_V1_0_6_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Processor_V1_0_6_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyProcessor_V1_0_6_Json (ResoruceRaw);
}
