...
//
#include <RedfishJsonStructure/Fabric/v1_0_3/EfiFabricV1_0_3.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Fabric_V1_0_3_To_CS (ResoruceRaw, &FabricV1_0_3Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)FabricV1_0_3;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Fabric_V1_0_3_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Fabric_V1_0_3_JSON (*((EFI_REDFISH_FABRIC_V1_0_3_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Fabric_V1_0_3_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyFabric_V1_0_3_Json (ResoruceRaw);
}
