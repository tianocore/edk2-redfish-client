...
//
#include <RedfishJsonStructure/PowerSubsystem/v1_0_0/EfiPowerSubsystemV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#PowerSubsystem";
BOOLEAN IsRevisonController = TRUE;

// Support PowerSubsystem V1_0_0
...
{
  EFI_REDFISH_POWERSUBSYSTEM_V1_0_0 *PowerSubsystemV1_0_0;
  EFI_REDFISH_POWERSUBSYSTEM_V1_0_0_CS *PowerSubsystemV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PowerSubsystem") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PowerSubsystem_V1_0_0_To_CS (ResoruceRaw, &PowerSubsystemV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerSubsystemV1_0_0 = (EFI_REDFISH_POWERSUBSYSTEM_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWERSUBSYSTEM_V1_0_0));
  if (PowerSubsystemV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerSubsystemV1_0_0;
  PowerSubsystemV1_0_0->PowerSubsystem = PowerSubsystemV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerSubsystem"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PowerSubsystem"), "PowerSubsystem");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerSubsystem"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PowerSubsystem"), "PowerSubsystem");
...
/**
  The wrapper function of PowerSubsystemToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PowerSubsystem_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PowerSubsystem_V1_0_0_JSON (*((EFI_REDFISH_POWERSUBSYSTEM_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PowerSubsystem_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PowerSubsystemToStruct.
...
)
{
  ...
  PowerSubsystemV1_0_0 = (EFI_REDFISH_POWERSUBSYSTEM_V1_0_0 *)InterpProp;
  DestroyPowerSubsystem_V1_0_0_CS (PowerSubsystemV1_0_0->PowerSubsystem); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerSubsystemFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyPowerSubsystem_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPowerSubsystem_V1_0_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPowerSubsystem_V1_0_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
