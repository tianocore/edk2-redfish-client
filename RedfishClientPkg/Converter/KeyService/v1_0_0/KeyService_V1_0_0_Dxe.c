...
//
#include <RedfishJsonStructure/KeyService/v1_0_0/EfiKeyServiceV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#KeyService";
BOOLEAN IsRevisonController = TRUE;

// Support KeyService V1_0_0
...
{
  EFI_REDFISH_KEYSERVICE_V1_0_0 *KeyServiceV1_0_0;
  EFI_REDFISH_KEYSERVICE_V1_0_0_CS *KeyServiceV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "KeyService") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_KeyService_V1_0_0_To_CS (ResoruceRaw, &KeyServiceV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  KeyServiceV1_0_0 = (EFI_REDFISH_KEYSERVICE_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_KEYSERVICE_V1_0_0));
  if (KeyServiceV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)KeyServiceV1_0_0;
  KeyServiceV1_0_0->KeyService = KeyServiceV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("KeyService"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("KeyService"), "KeyService");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("KeyService"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("KeyService"), "KeyService");
...
/**
  The wrapper function of KeyServiceToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_KeyService_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_KeyService_V1_0_0_JSON (*((EFI_REDFISH_KEYSERVICE_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_KeyService_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in KeyServiceToStruct.
...
)
{
  ...
  KeyServiceV1_0_0 = (EFI_REDFISH_KEYSERVICE_V1_0_0 *)InterpProp;
  DestroyKeyService_V1_0_0_CS (KeyServiceV1_0_0->KeyService); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from KeyServiceFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyKeyService_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishKeyService_V1_0_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishKeyService_V1_0_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
