...
//
#include <RedfishJsonStructure/CableCollection/EfiCableCollection.h>
CHAR8 mResourceTypeStr [] = "#CableCollection";
BOOLEAN IsRevisonController = FALSE;

// Support CableCollection 
...
{
  EFI_REDFISH_CABLECOLLECTION *CableCollection;
  EFI_REDFISH_CABLECOLLECTION_CS *CableCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "CableCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_CableCollection_To_CS (ResoruceRaw, &CableCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  CableCollection = (EFI_REDFISH_CABLECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CABLECOLLECTION));
  if (CableCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)CableCollection;
  CableCollection->CableCollection = CableCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("CableCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("CableCollection"), "CableCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("CableCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("CableCollection"), "CableCollection");
...
/**
  The wrapper function of CableCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
CableCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return CableCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_CableCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_CableCollection_JSON (*((EFI_REDFISH_CABLECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_CableCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in CableCollectionToStruct.
...
)
{
  ...
  CableCollection = (EFI_REDFISH_CABLECOLLECTION *)InterpProp;
  DestroyCableCollection_CS (CableCollection->CableCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from CableCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyCableCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishCableCollectionEntryPoint (
