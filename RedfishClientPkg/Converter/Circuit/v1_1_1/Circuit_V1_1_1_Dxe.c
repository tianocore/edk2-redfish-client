...
//
#include <RedfishJsonStructure/Circuit/v1_1_1/EfiCircuitV1_1_1.h>
CHAR8 mResourceTypeStr [] = "#Circuit";
BOOLEAN IsRevisonController = TRUE;

// Support Circuit V1_1_1
...
{
  EFI_REDFISH_CIRCUIT_V1_1_1 *CircuitV1_1_1;
  EFI_REDFISH_CIRCUIT_V1_1_1_CS *CircuitV1_1_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Circuit") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Circuit_V1_1_1_To_CS (ResoruceRaw, &CircuitV1_1_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  CircuitV1_1_1 = (EFI_REDFISH_CIRCUIT_V1_1_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CIRCUIT_V1_1_1));
  if (CircuitV1_1_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)CircuitV1_1_1;
  CircuitV1_1_1->Circuit = CircuitV1_1_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Circuit"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Circuit"), "Circuit");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Circuit"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Circuit"), "Circuit");
...
/**
  The wrapper function of CircuitToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Circuit_V1_1_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Circuit_V1_1_1_JSON (*((EFI_REDFISH_CIRCUIT_V1_1_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Circuit_V1_1_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in CircuitToStruct.
...
)
{
  ...
  CircuitV1_1_1 = (EFI_REDFISH_CIRCUIT_V1_1_1 *)InterpProp;
  DestroyCircuit_V1_1_1_CS (CircuitV1_1_1->Circuit); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from CircuitFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyCircuit_V1_1_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishCircuit_V1_1_1EntryPoint (
