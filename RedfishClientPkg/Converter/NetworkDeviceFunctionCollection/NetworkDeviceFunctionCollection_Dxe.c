...
//
#include <RedfishJsonStructure/NetworkDeviceFunctionCollection/EfiNetworkDeviceFunctionCollection.h>
...
EFI_STATUS
EFIAPI
NetworkDeviceFunctionCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_NetworkDeviceFunctionCollection_To_CS (ResoruceRaw, &NetworkDeviceFunctionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)NetworkDeviceFunctionCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkDeviceFunctionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("NetworkDeviceFunctionCollection"), "NetworkDeviceFunctionCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkDeviceFunctionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("NetworkDeviceFunctionCollection"), "NetworkDeviceFunctionCollection");
...
/**
  The wrapper function of NetworkDeviceFunctionCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
NetworkDeviceFunctionCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return NetworkDeviceFunctionCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
NetworkDeviceFunctionCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_NetworkDeviceFunctionCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_NetworkDeviceFunctionCollection_JSON (*((EFI_REDFISH_NETWORKDEVICEFUNCTIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_NetworkDeviceFunctionCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
NetworkDeviceFunctionCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from NetworkDeviceFunctionCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyNetworkDeviceFunctionCollection_Json (ResoruceRaw);
}
