...
//
#include <RedfishJsonStructure/PowerDistributionCollection/EfiPowerDistributionCollection.h>
CHAR8 mResourceTypeStr [] = "#PowerDistributionCollection";
BOOLEAN IsRevisonController = FALSE;

// Support PowerDistributionCollection 
...
EFI_STATUS
EFIAPI
PowerDistributionCollectionToStruct (
...
{
  EFI_REDFISH_POWERDISTRIBUTIONCOLLECTION *PowerDistributionCollection;
  EFI_REDFISH_POWERDISTRIBUTIONCOLLECTION_CS *PowerDistributionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PowerDistributionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PowerDistributionCollection_To_CS (ResoruceRaw, &PowerDistributionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerDistributionCollection = (EFI_REDFISH_POWERDISTRIBUTIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWERDISTRIBUTIONCOLLECTION));
  if (PowerDistributionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerDistributionCollection;
  PowerDistributionCollection->PowerDistributionCollection = PowerDistributionCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerDistributionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PowerDistributionCollection"), "PowerDistributionCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerDistributionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PowerDistributionCollection"), "PowerDistributionCollection");
...
/**
  The wrapper function of PowerDistributionCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
PowerDistributionCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return PowerDistributionCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
PowerDistributionCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PowerDistributionCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PowerDistributionCollection_JSON (*((EFI_REDFISH_POWERDISTRIBUTIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PowerDistributionCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PowerDistributionCollectionToStruct.
...
EFI_STATUS
EFIAPI
PowerDistributionCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  PowerDistributionCollection = (EFI_REDFISH_POWERDISTRIBUTIONCOLLECTION *)InterpProp;
  DestroyPowerDistributionCollection_CS (PowerDistributionCollection->PowerDistributionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerDistributionCollectionFromStruct
...
EFI_STATUS
EFIAPI
PowerDistributionCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyPowerDistributionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPowerDistributionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPowerDistributionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
