...
//
#include <RedfishJsonStructure/OperatingConfigCollection/EfiOperatingConfigCollection.h>
CHAR8 mResourceTypeStr [] = "#OperatingConfigCollection";
BOOLEAN IsRevisonController = FALSE;

// Support OperatingConfigCollection 
...
EFI_STATUS
EFIAPI
OperatingConfigCollectionToStruct (
...
{
  EFI_REDFISH_OPERATINGCONFIGCOLLECTION *OperatingConfigCollection;
  EFI_REDFISH_OPERATINGCONFIGCOLLECTION_CS *OperatingConfigCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "OperatingConfigCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_OperatingConfigCollection_To_CS (ResoruceRaw, &OperatingConfigCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  OperatingConfigCollection = (EFI_REDFISH_OPERATINGCONFIGCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_OPERATINGCONFIGCOLLECTION));
  if (OperatingConfigCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)OperatingConfigCollection;
  OperatingConfigCollection->OperatingConfigCollection = OperatingConfigCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("OperatingConfigCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("OperatingConfigCollection"), "OperatingConfigCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("OperatingConfigCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("OperatingConfigCollection"), "OperatingConfigCollection");
...
/**
  The wrapper function of OperatingConfigCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
OperatingConfigCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return OperatingConfigCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
OperatingConfigCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_OperatingConfigCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_OperatingConfigCollection_JSON (*((EFI_REDFISH_OPERATINGCONFIGCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_OperatingConfigCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in OperatingConfigCollectionToStruct.
...
EFI_STATUS
EFIAPI
OperatingConfigCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  OperatingConfigCollection = (EFI_REDFISH_OPERATINGCONFIGCOLLECTION *)InterpProp;
  DestroyOperatingConfigCollection_CS (OperatingConfigCollection->OperatingConfigCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from OperatingConfigCollectionFromStruct
...
EFI_STATUS
EFIAPI
OperatingConfigCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyOperatingConfigCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishOperatingConfigCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishOperatingConfigCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
