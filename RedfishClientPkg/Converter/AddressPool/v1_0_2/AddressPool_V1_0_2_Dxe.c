...
//
#include <RedfishJsonStructure/AddressPool/v1_0_2/EfiAddressPoolV1_0_2.h>
CHAR8 mResourceTypeStr [] = "#AddressPool";
BOOLEAN IsRevisonController = TRUE;

// Support AddressPool V1_0_2
...
{
  EFI_REDFISH_ADDRESSPOOL_V1_0_2 *AddressPoolV1_0_2;
  EFI_REDFISH_ADDRESSPOOL_V1_0_2_CS *AddressPoolV1_0_2Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "AddressPool") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_AddressPool_V1_0_2_To_CS (ResoruceRaw, &AddressPoolV1_0_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AddressPoolV1_0_2 = (EFI_REDFISH_ADDRESSPOOL_V1_0_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ADDRESSPOOL_V1_0_2));
  if (AddressPoolV1_0_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AddressPoolV1_0_2;
  AddressPoolV1_0_2->AddressPool = AddressPoolV1_0_2Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("AddressPool"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("AddressPool"), "AddressPool");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("AddressPool"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("AddressPool"), "AddressPool");
...
/**
  The wrapper function of AddressPoolToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AddressPool_V1_0_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AddressPool_V1_0_2_JSON (*((EFI_REDFISH_ADDRESSPOOL_V1_0_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AddressPool_V1_0_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AddressPoolToStruct.
...
)
{
  ...
  AddressPoolV1_0_2 = (EFI_REDFISH_ADDRESSPOOL_V1_0_2 *)InterpProp;
  DestroyAddressPool_V1_0_2_CS (AddressPoolV1_0_2->AddressPool); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AddressPoolFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyAddressPool_V1_0_2_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAddressPool_V1_0_2EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishAddressPool_V1_0_2Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
