...
//
#include <RedfishJsonStructure/ManagerAccount/v1_0_6/EfiManagerAccountV1_0_6.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_ManagerAccount_V1_0_6_To_CS (ResoruceRaw, &ManagerAccountV1_0_6Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ManagerAccountV1_0_6;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ManagerAccount_V1_0_6_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ManagerAccount_V1_0_6_JSON (*((EFI_REDFISH_MANAGERACCOUNT_V1_0_6_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ManagerAccount_V1_0_6_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyManagerAccount_V1_0_6_Json (ResoruceRaw);
}
