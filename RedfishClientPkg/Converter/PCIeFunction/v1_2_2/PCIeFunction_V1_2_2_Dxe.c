...
//
#include <RedfishJsonStructure/PCIeFunction/v1_2_2/EfiPCIeFunctionV1_2_2.h>
CHAR8 mResourceTypeStr [] = "#PCIeFunction";
BOOLEAN IsRevisonController = TRUE;

// Support PCIeFunction V1_2_2
...
{
  EFI_REDFISH_PCIEFUNCTION_V1_2_2 *PCIeFunctionV1_2_2;
  EFI_REDFISH_PCIEFUNCTION_V1_2_2_CS *PCIeFunctionV1_2_2Cs;
  ...
  Status = (EFI_STATUS)Json_PCIeFunction_V1_2_2_To_CS (ResoruceRaw, &PCIeFunctionV1_2_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PCIeFunctionV1_2_2 = (EFI_REDFISH_PCIEFUNCTION_V1_2_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_PCIEFUNCTION_V1_2_2));
  if (PCIeFunctionV1_2_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PCIeFunctionV1_2_2;
  PCIeFunctionV1_2_2->PCIeFunction = PCIeFunctionV1_2_2Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PCIeFunction"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PCIeFunction"), "PCIeFunction");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PCIeFunction"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PCIeFunction"), "PCIeFunction");
...
/**
  The wrapper function of PCIeFunctionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PCIeFunction_V1_2_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PCIeFunction_V1_2_2_JSON (*((EFI_REDFISH_PCIEFUNCTION_V1_2_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PCIeFunction_V1_2_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  PCIeFunctionV1_2_2 = (EFI_REDFISH_PCIEFUNCTION_V1_2_2 *)InterpProp;
  DestroyPCIeFunction_V1_2_2_CS (PCIeFunctionV1_2_2->PCIeFunction); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PCIeFunctionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyPCIeFunction_V1_2_2_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPCIeFunction_V1_2_2EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPCIeFunction_V1_2_2Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
