...
//
#include <RedfishJsonStructure/FabricAdapter/v1_2_0/EfiFabricAdapterV1_2_0.h>
CHAR8 mResourceTypeStr [] = "#FabricAdapter";
BOOLEAN IsRevisonController = TRUE;

// Support FabricAdapter V1_2_0
...
{
  EFI_REDFISH_FABRICADAPTER_V1_2_0 *FabricAdapterV1_2_0;
  EFI_REDFISH_FABRICADAPTER_V1_2_0_CS *FabricAdapterV1_2_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "FabricAdapter") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_FabricAdapter_V1_2_0_To_CS (ResoruceRaw, &FabricAdapterV1_2_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  FabricAdapterV1_2_0 = (EFI_REDFISH_FABRICADAPTER_V1_2_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_FABRICADAPTER_V1_2_0));
  if (FabricAdapterV1_2_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)FabricAdapterV1_2_0;
  FabricAdapterV1_2_0->FabricAdapter = FabricAdapterV1_2_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("FabricAdapter"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("FabricAdapter"), "FabricAdapter");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("FabricAdapter"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("FabricAdapter"), "FabricAdapter");
...
/**
  The wrapper function of FabricAdapterToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_FabricAdapter_V1_2_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_FabricAdapter_V1_2_0_JSON (*((EFI_REDFISH_FABRICADAPTER_V1_2_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_FabricAdapter_V1_2_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in FabricAdapterToStruct.
...
)
{
  ...
  FabricAdapterV1_2_0 = (EFI_REDFISH_FABRICADAPTER_V1_2_0 *)InterpProp;
  DestroyFabricAdapter_V1_2_0_CS (FabricAdapterV1_2_0->FabricAdapter); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from FabricAdapterFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyFabricAdapter_V1_2_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishFabricAdapter_V1_2_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishFabricAdapter_V1_2_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
