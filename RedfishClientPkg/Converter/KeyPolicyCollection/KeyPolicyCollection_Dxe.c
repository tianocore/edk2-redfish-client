...
//
#include <RedfishJsonStructure/KeyPolicyCollection/EfiKeyPolicyCollection.h>
CHAR8 mResourceTypeStr [] = "#KeyPolicyCollection";
BOOLEAN IsRevisonController = FALSE;

// Support KeyPolicyCollection 
...
{
  EFI_REDFISH_KEYPOLICYCOLLECTION *KeyPolicyCollection;
  EFI_REDFISH_KEYPOLICYCOLLECTION_CS *KeyPolicyCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "KeyPolicyCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_KeyPolicyCollection_To_CS (ResoruceRaw, &KeyPolicyCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  KeyPolicyCollection = (EFI_REDFISH_KEYPOLICYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_KEYPOLICYCOLLECTION));
  if (KeyPolicyCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)KeyPolicyCollection;
  KeyPolicyCollection->KeyPolicyCollection = KeyPolicyCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("KeyPolicyCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("KeyPolicyCollection"), "KeyPolicyCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("KeyPolicyCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("KeyPolicyCollection"), "KeyPolicyCollection");
...
/**
  The wrapper function of KeyPolicyCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
KeyPolicyCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return KeyPolicyCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_KeyPolicyCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_KeyPolicyCollection_JSON (*((EFI_REDFISH_KEYPOLICYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_KeyPolicyCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in KeyPolicyCollectionToStruct.
...
EFI_STATUS
EFIAPI
KeyPolicyCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  KeyPolicyCollection = (EFI_REDFISH_KEYPOLICYCOLLECTION *)InterpProp;
  DestroyKeyPolicyCollection_CS (KeyPolicyCollection->KeyPolicyCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from KeyPolicyCollectionFromStruct
...
EFI_STATUS
EFIAPI
KeyPolicyCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyKeyPolicyCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishKeyPolicyCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishKeyPolicyCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
