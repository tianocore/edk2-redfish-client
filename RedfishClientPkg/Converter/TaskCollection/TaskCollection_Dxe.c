...
//
#include <RedfishJsonStructure/TaskCollection/EfiTaskCollection.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_TaskCollection_To_CS (ResoruceRaw, &TaskCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)TaskCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("TaskCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("TaskCollection"), "TaskCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("TaskCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("TaskCollection"), "TaskCollection");
...
/**
  The wrapper function of TaskCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_TaskCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_TaskCollection_JSON (*((EFI_REDFISH_TASKCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_TaskCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
/**
  This function destory JSON raw text returned from TaskCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyTaskCollection_Json (ResoruceRaw);
}
