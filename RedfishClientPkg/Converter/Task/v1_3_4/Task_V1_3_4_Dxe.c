...
//
#include <RedfishJsonStructure/Task/v1_3_4/EfiTaskV1_3_4.h>
...
{
  EFI_REDFISH_TASK_V1_3_4 *TaskV1_3_4;
  EFI_REDFISH_TASK_V1_3_4_CS *TaskV1_3_4Cs;
  ...
  Status = (EFI_STATUS)Json_Task_V1_3_4_To_CS (ResoruceRaw, &TaskV1_3_4Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  TaskV1_3_4 = (EFI_REDFISH_TASK_V1_3_4 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_TASK_V1_3_4));
  if (TaskV1_3_4 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)TaskV1_3_4;
  TaskV1_3_4->Task = TaskV1_3_4Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Task"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Task"), "Task");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Task"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Task"), "Task");
...
/**
  The wrapper function of TaskToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Task_V1_3_4_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Task_V1_3_4_JSON (*((EFI_REDFISH_TASK_V1_3_4_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Task_V1_3_4_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  TaskV1_3_4 = (EFI_REDFISH_TASK_V1_3_4 *)InterpProp;
  DestroyTask_V1_3_4_CS (TaskV1_3_4->Task); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from TaskFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyTask_V1_3_4_Json (ResoruceRaw);
}
