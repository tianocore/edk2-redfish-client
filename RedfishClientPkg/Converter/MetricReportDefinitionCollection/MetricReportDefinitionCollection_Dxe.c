...
//
#include <RedfishJsonStructure/MetricReportDefinitionCollection/EfiMetricReportDefinitionCollection.h>
...
EFI_STATUS
EFIAPI
MetricReportDefinitionCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_MetricReportDefinitionCollection_To_CS (ResoruceRaw, &MetricReportDefinitionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)MetricReportDefinitionCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MetricReportDefinitionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("MetricReportDefinitionCollection"), "MetricReportDefinitionCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MetricReportDefinitionCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("MetricReportDefinitionCollection"), "MetricReportDefinitionCollection");
...
/**
  The wrapper function of MetricReportDefinitionCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
MetricReportDefinitionCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return MetricReportDefinitionCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
MetricReportDefinitionCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_MetricReportDefinitionCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_MetricReportDefinitionCollection_JSON (*((EFI_REDFISH_METRICREPORTDEFINITIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_MetricReportDefinitionCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
MetricReportDefinitionCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from MetricReportDefinitionCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyMetricReportDefinitionCollection_Json (ResoruceRaw);
}
