...
//
#include <RedfishJsonStructure/AllowDenyCollection/EfiAllowDenyCollection.h>
CHAR8 mResourceTypeStr [] = "#AllowDenyCollection";
BOOLEAN IsRevisonController = FALSE;

// Support AllowDenyCollection 
...
{
  EFI_REDFISH_ALLOWDENYCOLLECTION *AllowDenyCollection;
  EFI_REDFISH_ALLOWDENYCOLLECTION_CS *AllowDenyCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "AllowDenyCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_AllowDenyCollection_To_CS (ResoruceRaw, &AllowDenyCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  AllowDenyCollection = (EFI_REDFISH_ALLOWDENYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ALLOWDENYCOLLECTION));
  if (AllowDenyCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)AllowDenyCollection;
  AllowDenyCollection->AllowDenyCollection = AllowDenyCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("AllowDenyCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("AllowDenyCollection"), "AllowDenyCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("AllowDenyCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("AllowDenyCollection"), "AllowDenyCollection");
...
/**
  The wrapper function of AllowDenyCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
AllowDenyCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return AllowDenyCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_AllowDenyCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_AllowDenyCollection_JSON (*((EFI_REDFISH_ALLOWDENYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_AllowDenyCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in AllowDenyCollectionToStruct.
...
EFI_STATUS
EFIAPI
AllowDenyCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  AllowDenyCollection = (EFI_REDFISH_ALLOWDENYCOLLECTION *)InterpProp;
  DestroyAllowDenyCollection_CS (AllowDenyCollection->AllowDenyCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from AllowDenyCollectionFromStruct
...
EFI_STATUS
EFIAPI
AllowDenyCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyAllowDenyCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishAllowDenyCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishAllowDenyCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
