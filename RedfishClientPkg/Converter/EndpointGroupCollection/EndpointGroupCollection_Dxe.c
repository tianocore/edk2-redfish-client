...
//
#include <RedfishJsonStructure/EndpointGroupCollection/EfiEndpointGroupCollection.h>
CHAR8 mResourceTypeStr [] = "#EndpointGroupCollection";
BOOLEAN IsRevisonController = FALSE;

// Support EndpointGroupCollection 
...
EFI_STATUS
EFIAPI
EndpointGroupCollectionToStruct (
...
{
  EFI_REDFISH_ENDPOINTGROUPCOLLECTION *EndpointGroupCollection;
  EFI_REDFISH_ENDPOINTGROUPCOLLECTION_CS *EndpointGroupCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "EndpointGroupCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_EndpointGroupCollection_To_CS (ResoruceRaw, &EndpointGroupCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  EndpointGroupCollection = (EFI_REDFISH_ENDPOINTGROUPCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ENDPOINTGROUPCOLLECTION));
  if (EndpointGroupCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)EndpointGroupCollection;
  EndpointGroupCollection->EndpointGroupCollection = EndpointGroupCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("EndpointGroupCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("EndpointGroupCollection"), "EndpointGroupCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("EndpointGroupCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("EndpointGroupCollection"), "EndpointGroupCollection");
...
/**
  The wrapper function of EndpointGroupCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
EndpointGroupCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return EndpointGroupCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_EndpointGroupCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_EndpointGroupCollection_JSON (*((EFI_REDFISH_ENDPOINTGROUPCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_EndpointGroupCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in EndpointGroupCollectionToStruct.
...
EFI_STATUS
EFIAPI
EndpointGroupCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  EndpointGroupCollection = (EFI_REDFISH_ENDPOINTGROUPCOLLECTION *)InterpProp;
  DestroyEndpointGroupCollection_CS (EndpointGroupCollection->EndpointGroupCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from EndpointGroupCollectionFromStruct
...
EFI_STATUS
EFIAPI
EndpointGroupCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyEndpointGroupCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishEndpointGroupCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishEndpointGroupCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
