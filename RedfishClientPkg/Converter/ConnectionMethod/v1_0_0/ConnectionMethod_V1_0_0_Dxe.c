...
//
#include <RedfishJsonStructure/ConnectionMethod/v1_0_0/EfiConnectionMethodV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#ConnectionMethod";
BOOLEAN IsRevisonController = TRUE;

// Support ConnectionMethod V1_0_0
...
{
  EFI_REDFISH_CONNECTIONMETHOD_V1_0_0 *ConnectionMethodV1_0_0;
  EFI_REDFISH_CONNECTIONMETHOD_V1_0_0_CS *ConnectionMethodV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ConnectionMethod") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_ConnectionMethod_V1_0_0_To_CS (ResoruceRaw, &ConnectionMethodV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ConnectionMethodV1_0_0 = (EFI_REDFISH_CONNECTIONMETHOD_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CONNECTIONMETHOD_V1_0_0));
  if (ConnectionMethodV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ConnectionMethodV1_0_0;
  ConnectionMethodV1_0_0->ConnectionMethod = ConnectionMethodV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ConnectionMethod"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ConnectionMethod"), "ConnectionMethod");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ConnectionMethod"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ConnectionMethod"), "ConnectionMethod");
...
/**
  The wrapper function of ConnectionMethodToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ConnectionMethodToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ConnectionMethodToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ConnectionMethod_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ConnectionMethod_V1_0_0_JSON (*((EFI_REDFISH_CONNECTIONMETHOD_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ConnectionMethod_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ConnectionMethodToStruct.
...
)
{
  ...
  ConnectionMethodV1_0_0 = (EFI_REDFISH_CONNECTIONMETHOD_V1_0_0 *)InterpProp;
  DestroyConnectionMethod_V1_0_0_CS (ConnectionMethodV1_0_0->ConnectionMethod); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ConnectionMethodFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyConnectionMethod_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishConnectionMethod_V1_0_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishConnectionMethod_V1_0_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
