...
//
#include <RedfishJsonStructure/Sensor/v1_1_0/EfiSensorV1_1_0.h>
CHAR8 mResourceTypeStr [] = "#Sensor";
BOOLEAN IsRevisonController = TRUE;

// Support Sensor V1_1_0
...
{
  EFI_REDFISH_SENSOR_V1_1_0 *SensorV1_1_0;
  EFI_REDFISH_SENSOR_V1_1_0_CS *SensorV1_1_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Sensor") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Sensor_V1_1_0_To_CS (ResoruceRaw, &SensorV1_1_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SensorV1_1_0 = (EFI_REDFISH_SENSOR_V1_1_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SENSOR_V1_1_0));
  if (SensorV1_1_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SensorV1_1_0;
  SensorV1_1_0->Sensor = SensorV1_1_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Sensor"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Sensor"), "Sensor");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Sensor"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Sensor"), "Sensor");
...
/**
  The wrapper function of SensorToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Sensor_V1_1_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Sensor_V1_1_0_JSON (*((EFI_REDFISH_SENSOR_V1_1_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Sensor_V1_1_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in SensorToStruct.
...
)
{
  ...
  SensorV1_1_0 = (EFI_REDFISH_SENSOR_V1_1_0 *)InterpProp;
  DestroySensor_V1_1_0_CS (SensorV1_1_0->Sensor); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SensorFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroySensor_V1_1_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSensor_V1_1_0EntryPoint (
