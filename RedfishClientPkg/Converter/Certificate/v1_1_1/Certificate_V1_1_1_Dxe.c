...
//
#include <RedfishJsonStructure/Certificate/v1_1_1/EfiCertificateV1_1_1.h>
CHAR8 mResourceTypeStr [] = "#Certificate";
BOOLEAN IsRevisonController = TRUE;

// Support Certificate V1_1_1
...
{
  EFI_REDFISH_CERTIFICATE_V1_1_1 *CertificateV1_1_1;
  EFI_REDFISH_CERTIFICATE_V1_1_1_CS *CertificateV1_1_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Certificate") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Certificate_V1_1_1_To_CS (ResoruceRaw, &CertificateV1_1_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  CertificateV1_1_1 = (EFI_REDFISH_CERTIFICATE_V1_1_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CERTIFICATE_V1_1_1));
  if (CertificateV1_1_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)CertificateV1_1_1;
  CertificateV1_1_1->Certificate = CertificateV1_1_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Certificate"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Certificate"), "Certificate");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Certificate"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Certificate"), "Certificate");
...
/**
  The wrapper function of CertificateToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Certificate_V1_1_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Certificate_V1_1_1_JSON (*((EFI_REDFISH_CERTIFICATE_V1_1_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Certificate_V1_1_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in CertificateToStruct.
...
)
{
  ...
  CertificateV1_1_1 = (EFI_REDFISH_CERTIFICATE_V1_1_1 *)InterpProp;
  DestroyCertificate_V1_1_1_CS (CertificateV1_1_1->Certificate); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from CertificateFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyCertificate_V1_1_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishCertificate_V1_1_1EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishCertificate_V1_1_1Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
