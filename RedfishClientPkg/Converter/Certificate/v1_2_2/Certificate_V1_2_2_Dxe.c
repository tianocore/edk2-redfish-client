...
//
#include <RedfishJsonStructure/Certificate/v1_2_2/EfiCertificateV1_2_2.h>
CHAR8 mResourceTypeStr [] = "#Certificate";
BOOLEAN IsRevisonController = TRUE;

// Support Certificate V1_2_2
...
{
  EFI_REDFISH_CERTIFICATE_V1_2_2 *CertificateV1_2_2;
  EFI_REDFISH_CERTIFICATE_V1_2_2_CS *CertificateV1_2_2Cs;
  ...
  Status = (EFI_STATUS)Json_Certificate_V1_2_2_To_CS (ResoruceRaw, &CertificateV1_2_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  CertificateV1_2_2 = (EFI_REDFISH_CERTIFICATE_V1_2_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CERTIFICATE_V1_2_2));
  if (CertificateV1_2_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)CertificateV1_2_2;
  CertificateV1_2_2->Certificate = CertificateV1_2_2Cs;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Certificate_V1_2_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Certificate_V1_2_2_JSON (*((EFI_REDFISH_CERTIFICATE_V1_2_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Certificate_V1_2_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  CertificateV1_2_2 = (EFI_REDFISH_CERTIFICATE_V1_2_2 *)InterpProp;
  DestroyCertificate_V1_2_2_CS (CertificateV1_2_2->Certificate); 
  return EFI_SUCCESS;
}
...
)
{
  ...
  return (EFI_STATUS)DestroyCertificate_V1_2_2_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishCertificate_V1_2_2EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishCertificate_V1_2_2Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
