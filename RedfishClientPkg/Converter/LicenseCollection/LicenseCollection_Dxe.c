...
//
#include <RedfishJsonStructure/LicenseCollection/EfiLicenseCollection.h>
CHAR8 mResourceTypeStr [] = "#LicenseCollection";
BOOLEAN IsRevisonController = FALSE;

// Support LicenseCollection 
...
{
  EFI_REDFISH_LICENSECOLLECTION *LicenseCollection;
  EFI_REDFISH_LICENSECOLLECTION_CS *LicenseCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "LicenseCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_LicenseCollection_To_CS (ResoruceRaw, &LicenseCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  LicenseCollection = (EFI_REDFISH_LICENSECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_LICENSECOLLECTION));
  if (LicenseCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)LicenseCollection;
  LicenseCollection->LicenseCollection = LicenseCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("LicenseCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("LicenseCollection"), "LicenseCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("LicenseCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("LicenseCollection"), "LicenseCollection");
...
/**
  The wrapper function of LicenseCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
LicenseCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return LicenseCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_LicenseCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_LicenseCollection_JSON (*((EFI_REDFISH_LICENSECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_LicenseCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in LicenseCollectionToStruct.
...
EFI_STATUS
EFIAPI
LicenseCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  LicenseCollection = (EFI_REDFISH_LICENSECOLLECTION *)InterpProp;
  DestroyLicenseCollection_CS (LicenseCollection->LicenseCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from LicenseCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyLicenseCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishLicenseCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishLicenseCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
