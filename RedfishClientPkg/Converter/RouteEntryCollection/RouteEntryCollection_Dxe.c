...
//
#include <RedfishJsonStructure/RouteEntryCollection/EfiRouteEntryCollection.h>
CHAR8 mResourceTypeStr [] = "#RouteEntryCollection";
BOOLEAN IsRevisonController = FALSE;

// Support RouteEntryCollection 
...
{
  EFI_REDFISH_ROUTEENTRYCOLLECTION *RouteEntryCollection;
  EFI_REDFISH_ROUTEENTRYCOLLECTION_CS *RouteEntryCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "RouteEntryCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_RouteEntryCollection_To_CS (ResoruceRaw, &RouteEntryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  RouteEntryCollection = (EFI_REDFISH_ROUTEENTRYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ROUTEENTRYCOLLECTION));
  if (RouteEntryCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)RouteEntryCollection;
  RouteEntryCollection->RouteEntryCollection = RouteEntryCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("RouteEntryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("RouteEntryCollection"), "RouteEntryCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("RouteEntryCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("RouteEntryCollection"), "RouteEntryCollection");
...
/**
  The wrapper function of RouteEntryCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
RouteEntryCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return RouteEntryCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_RouteEntryCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_RouteEntryCollection_JSON (*((EFI_REDFISH_ROUTEENTRYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_RouteEntryCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in RouteEntryCollectionToStruct.
...
)
{
  ...
  RouteEntryCollection = (EFI_REDFISH_ROUTEENTRYCOLLECTION *)InterpProp;
  DestroyRouteEntryCollection_CS (RouteEntryCollection->RouteEntryCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from RouteEntryCollectionFromStruct
...
EFI_STATUS
EFIAPI
RouteEntryCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyRouteEntryCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishRouteEntryCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishRouteEntryCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
