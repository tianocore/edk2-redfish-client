...
//
#include <RedfishJsonStructure/SerialInterfaceCollection/EfiSerialInterfaceCollection.h>
...
EFI_STATUS
EFIAPI
SerialInterfaceCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_SerialInterfaceCollection_To_CS (ResoruceRaw, &SerialInterfaceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)SerialInterfaceCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SerialInterfaceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("SerialInterfaceCollection"), "SerialInterfaceCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SerialInterfaceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("SerialInterfaceCollection"), "SerialInterfaceCollection");
...
/**
  The wrapper function of SerialInterfaceCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
SerialInterfaceCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return SerialInterfaceCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_SerialInterfaceCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_SerialInterfaceCollection_JSON (*((EFI_REDFISH_SERIALINTERFACECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_SerialInterfaceCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
/**
  This function destory JSON raw text returned from SerialInterfaceCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroySerialInterfaceCollection_Json (ResoruceRaw);
}
