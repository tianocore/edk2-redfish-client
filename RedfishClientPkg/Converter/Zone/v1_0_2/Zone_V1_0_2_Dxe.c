...
//
#include <RedfishJsonStructure/Zone/v1_0_2/EfiZoneV1_0_2.h>
...
)
{
  ...
  Status = (EFI_STATUS)Json_Zone_V1_0_2_To_CS (ResoruceRaw, &ZoneV1_0_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ZoneV1_0_2;
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Zone_V1_0_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Zone_V1_0_2_JSON (*((EFI_REDFISH_ZONE_V1_0_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Zone_V1_0_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
)
{
  ...
  return (EFI_STATUS)DestroyZone_V1_0_2_Json (ResoruceRaw);
}
