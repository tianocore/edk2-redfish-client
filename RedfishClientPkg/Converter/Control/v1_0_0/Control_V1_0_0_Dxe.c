...
//
#include <RedfishJsonStructure/Control/v1_0_0/EfiControlV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#Control";
BOOLEAN IsRevisonController = TRUE;

// Support Control V1_0_0
...
{
  EFI_REDFISH_CONTROL_V1_0_0 *ControlV1_0_0;
  EFI_REDFISH_CONTROL_V1_0_0_CS *ControlV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Control") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Control_V1_0_0_To_CS (ResoruceRaw, &ControlV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ControlV1_0_0 = (EFI_REDFISH_CONTROL_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CONTROL_V1_0_0));
  if (ControlV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ControlV1_0_0;
  ControlV1_0_0->Control = ControlV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Control"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Control"), "Control");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Control"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Control"), "Control");
...
/**
  The wrapper function of ControlToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Control_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Control_V1_0_0_JSON (*((EFI_REDFISH_CONTROL_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Control_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ControlToStruct.
...
)
{
  ...
  ControlV1_0_0 = (EFI_REDFISH_CONTROL_V1_0_0 *)InterpProp;
  DestroyControl_V1_0_0_CS (ControlV1_0_0->Control); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ControlFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyControl_V1_0_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishControl_V1_0_0EntryPoint (
