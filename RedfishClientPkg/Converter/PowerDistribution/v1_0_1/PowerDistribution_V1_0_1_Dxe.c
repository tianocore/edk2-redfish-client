...
//
#include <RedfishJsonStructure/PowerDistribution/v1_0_1/EfiPowerDistributionV1_0_1.h>
CHAR8 mResourceTypeStr [] = "#PowerDistribution";
BOOLEAN IsRevisonController = TRUE;

// Support PowerDistribution V1_0_1
...
{
  EFI_REDFISH_POWERDISTRIBUTION_V1_0_1 *PowerDistributionV1_0_1;
  EFI_REDFISH_POWERDISTRIBUTION_V1_0_1_CS *PowerDistributionV1_0_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PowerDistribution") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PowerDistribution_V1_0_1_To_CS (ResoruceRaw, &PowerDistributionV1_0_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerDistributionV1_0_1 = (EFI_REDFISH_POWERDISTRIBUTION_V1_0_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWERDISTRIBUTION_V1_0_1));
  if (PowerDistributionV1_0_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerDistributionV1_0_1;
  PowerDistributionV1_0_1->PowerDistribution = PowerDistributionV1_0_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerDistribution"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PowerDistribution"), "PowerDistribution");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerDistribution"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PowerDistribution"), "PowerDistribution");
...
/**
  The wrapper function of PowerDistributionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
PowerDistributionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return PowerDistributionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PowerDistribution_V1_0_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PowerDistribution_V1_0_1_JSON (*((EFI_REDFISH_POWERDISTRIBUTION_V1_0_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PowerDistribution_V1_0_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PowerDistributionToStruct.
...
EFI_STATUS
EFIAPI
PowerDistributionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  PowerDistributionV1_0_1 = (EFI_REDFISH_POWERDISTRIBUTION_V1_0_1 *)InterpProp;
  DestroyPowerDistribution_V1_0_1_CS (PowerDistributionV1_0_1->PowerDistribution); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerDistributionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyPowerDistribution_V1_0_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPowerDistribution_V1_0_1EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPowerDistribution_V1_0_1Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
