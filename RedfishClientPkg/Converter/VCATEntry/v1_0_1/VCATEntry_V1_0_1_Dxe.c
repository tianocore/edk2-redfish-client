...
//
#include <RedfishJsonStructure/VCATEntry/v1_0_1/EfiVCATEntryV1_0_1.h>
CHAR8 mResourceTypeStr [] = "#VCATEntry";
BOOLEAN IsRevisonController = TRUE;

// Support VCATEntry V1_0_1
...
{
  EFI_REDFISH_VCATENTRY_V1_0_1 *VCATEntryV1_0_1;
  EFI_REDFISH_VCATENTRY_V1_0_1_CS *VCATEntryV1_0_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "VCATEntry") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_VCATEntry_V1_0_1_To_CS (ResoruceRaw, &VCATEntryV1_0_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  VCATEntryV1_0_1 = (EFI_REDFISH_VCATENTRY_V1_0_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_VCATENTRY_V1_0_1));
  if (VCATEntryV1_0_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)VCATEntryV1_0_1;
  VCATEntryV1_0_1->VCATEntry = VCATEntryV1_0_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("VCATEntry"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("VCATEntry"), "VCATEntry");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("VCATEntry"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("VCATEntry"), "VCATEntry");
...
/**
  The wrapper function of VCATEntryToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_VCATEntry_V1_0_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_VCATEntry_V1_0_1_JSON (*((EFI_REDFISH_VCATENTRY_V1_0_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_VCATEntry_V1_0_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in VCATEntryToStruct.
...
)
{
  ...
  VCATEntryV1_0_1 = (EFI_REDFISH_VCATENTRY_V1_0_1 *)InterpProp;
  DestroyVCATEntry_V1_0_1_CS (VCATEntryV1_0_1->VCATEntry); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from VCATEntryFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyVCATEntry_V1_0_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishVCATEntry_V1_0_1EntryPoint (
