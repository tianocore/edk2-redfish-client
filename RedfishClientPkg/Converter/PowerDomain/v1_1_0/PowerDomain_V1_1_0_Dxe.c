...
//
#include <RedfishJsonStructure/PowerDomain/v1_1_0/EfiPowerDomainV1_1_0.h>
CHAR8 mResourceTypeStr [] = "#PowerDomain";
BOOLEAN IsRevisonController = TRUE;

// Support PowerDomain V1_1_0
...
{
  EFI_REDFISH_POWERDOMAIN_V1_1_0 *PowerDomainV1_1_0;
  EFI_REDFISH_POWERDOMAIN_V1_1_0_CS *PowerDomainV1_1_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PowerDomain") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PowerDomain_V1_1_0_To_CS (ResoruceRaw, &PowerDomainV1_1_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerDomainV1_1_0 = (EFI_REDFISH_POWERDOMAIN_V1_1_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWERDOMAIN_V1_1_0));
  if (PowerDomainV1_1_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PowerDomainV1_1_0;
  PowerDomainV1_1_0->PowerDomain = PowerDomainV1_1_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerDomain"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PowerDomain"), "PowerDomain");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PowerDomain"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PowerDomain"), "PowerDomain");
...
/**
  The wrapper function of PowerDomainToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PowerDomain_V1_1_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PowerDomain_V1_1_0_JSON (*((EFI_REDFISH_POWERDOMAIN_V1_1_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PowerDomain_V1_1_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PowerDomainToStruct.
...
)
{
  ...
  PowerDomainV1_1_0 = (EFI_REDFISH_POWERDOMAIN_V1_1_0 *)InterpProp;
  DestroyPowerDomain_V1_1_0_CS (PowerDomainV1_1_0->PowerDomain); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerDomainFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyPowerDomain_V1_1_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPowerDomain_V1_1_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPowerDomain_V1_1_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
