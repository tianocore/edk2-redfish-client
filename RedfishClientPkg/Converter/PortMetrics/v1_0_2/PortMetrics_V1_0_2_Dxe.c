...
//
#include <RedfishJsonStructure/PortMetrics/v1_0_2/EfiPortMetricsV1_0_2.h>
CHAR8 mResourceTypeStr [] = "#PortMetrics";
BOOLEAN IsRevisonController = TRUE;

// Support PortMetrics V1_0_2
...
{
  EFI_REDFISH_PORTMETRICS_V1_0_2 *PortMetricsV1_0_2;
  EFI_REDFISH_PORTMETRICS_V1_0_2_CS *PortMetricsV1_0_2Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PortMetrics") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PortMetrics_V1_0_2_To_CS (ResoruceRaw, &PortMetricsV1_0_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PortMetricsV1_0_2 = (EFI_REDFISH_PORTMETRICS_V1_0_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_PORTMETRICS_V1_0_2));
  if (PortMetricsV1_0_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PortMetricsV1_0_2;
  PortMetricsV1_0_2->PortMetrics = PortMetricsV1_0_2Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PortMetrics"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PortMetrics"), "PortMetrics");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PortMetrics"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PortMetrics"), "PortMetrics");
...
/**
  The wrapper function of PortMetricsToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PortMetrics_V1_0_2_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PortMetrics_V1_0_2_JSON (*((EFI_REDFISH_PORTMETRICS_V1_0_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PortMetrics_V1_0_2_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PortMetricsToStruct.
...
)
{
  ...
  PortMetricsV1_0_2 = (EFI_REDFISH_PORTMETRICS_V1_0_2 *)InterpProp;
  DestroyPortMetrics_V1_0_2_CS (PortMetricsV1_0_2->PortMetrics); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PortMetricsFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyPortMetrics_V1_0_2_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPortMetrics_V1_0_2EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPortMetrics_V1_0_2Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
