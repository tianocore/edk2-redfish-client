...
//
#include <RedfishJsonStructure/PCIeDeviceCollection/EfiPCIeDeviceCollection.h>
CHAR8 mResourceTypeStr [] = "#PCIeDeviceCollection";
...
{
  EFI_REDFISH_PCIEDEVICECOLLECTION *PCIeDeviceCollection;
  EFI_REDFISH_PCIEDEVICECOLLECTION_CS *PCIeDeviceCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PCIeDeviceCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_PCIeDeviceCollection_To_CS (ResoruceRaw, &PCIeDeviceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PCIeDeviceCollection = (EFI_REDFISH_PCIEDEVICECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_PCIEDEVICECOLLECTION));
  if (PCIeDeviceCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)PCIeDeviceCollection;
  PCIeDeviceCollection->PCIeDeviceCollection = PCIeDeviceCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PCIeDeviceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("PCIeDeviceCollection"), "PCIeDeviceCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PCIeDeviceCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("PCIeDeviceCollection"), "PCIeDeviceCollection");
...
/**
  The wrapper function of PCIeDeviceCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
PCIeDeviceCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return PCIeDeviceCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_PCIeDeviceCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_PCIeDeviceCollection_JSON (*((EFI_REDFISH_PCIEDEVICECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_PCIeDeviceCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in PCIeDeviceCollectionToStruct.
...
EFI_STATUS
EFIAPI
PCIeDeviceCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  PCIeDeviceCollection = (EFI_REDFISH_PCIEDEVICECOLLECTION *)InterpProp;
  DestroyPCIeDeviceCollection_CS (PCIeDeviceCollection->PCIeDeviceCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PCIeDeviceCollectionFromStruct
...
EFI_STATUS
EFIAPI
PCIeDeviceCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyPCIeDeviceCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPCIeDeviceCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishPCIeDeviceCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
