...
//
#include <RedfishJsonStructure/Connection/v1_1_0/EfiConnectionV1_1_0.h>
CHAR8 mResourceTypeStr [] = "#Connection";
BOOLEAN IsRevisonController = TRUE;

// Support Connection V1_1_0
...
{
  EFI_REDFISH_CONNECTION_V1_1_0 *ConnectionV1_1_0;
  EFI_REDFISH_CONNECTION_V1_1_0_CS *ConnectionV1_1_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Connection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Connection_V1_1_0_To_CS (ResoruceRaw, &ConnectionV1_1_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ConnectionV1_1_0 = (EFI_REDFISH_CONNECTION_V1_1_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CONNECTION_V1_1_0));
  if (ConnectionV1_1_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ConnectionV1_1_0;
  ConnectionV1_1_0->Connection = ConnectionV1_1_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Connection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Connection"), "Connection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Connection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Connection"), "Connection");
...
/**
  The wrapper function of ConnectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Connection_V1_1_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Connection_V1_1_0_JSON (*((EFI_REDFISH_CONNECTION_V1_1_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Connection_V1_1_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ConnectionToStruct.
...
)
{
  ...
  ConnectionV1_1_0 = (EFI_REDFISH_CONNECTION_V1_1_0 *)InterpProp;
  DestroyConnection_V1_1_0_CS (ConnectionV1_1_0->Connection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ConnectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyConnection_V1_1_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishConnection_V1_1_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishConnection_V1_1_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
