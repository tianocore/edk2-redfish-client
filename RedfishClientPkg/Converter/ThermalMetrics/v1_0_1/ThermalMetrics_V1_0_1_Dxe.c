...
//
#include <RedfishJsonStructure/ThermalMetrics/v1_0_1/EfiThermalMetricsV1_0_1.h>
CHAR8 mResourceTypeStr [] = "#ThermalMetrics";
BOOLEAN IsRevisonController = TRUE;

// Support ThermalMetrics V1_0_1
...
{
  EFI_REDFISH_THERMALMETRICS_V1_0_1 *ThermalMetricsV1_0_1;
  EFI_REDFISH_THERMALMETRICS_V1_0_1_CS *ThermalMetricsV1_0_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ThermalMetrics") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_ThermalMetrics_V1_0_1_To_CS (ResoruceRaw, &ThermalMetricsV1_0_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ThermalMetricsV1_0_1 = (EFI_REDFISH_THERMALMETRICS_V1_0_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_THERMALMETRICS_V1_0_1));
  if (ThermalMetricsV1_0_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ThermalMetricsV1_0_1;
  ThermalMetricsV1_0_1->ThermalMetrics = ThermalMetricsV1_0_1Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ThermalMetrics"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ThermalMetrics"), "ThermalMetrics");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ThermalMetrics"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ThermalMetrics"), "ThermalMetrics");
...
/**
  The wrapper function of ThermalMetricsToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ThermalMetrics_V1_0_1_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ThermalMetrics_V1_0_1_JSON (*((EFI_REDFISH_THERMALMETRICS_V1_0_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ThermalMetrics_V1_0_1_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ThermalMetricsToStruct.
...
)
{
  ...
  ThermalMetricsV1_0_1 = (EFI_REDFISH_THERMALMETRICS_V1_0_1 *)InterpProp;
  DestroyThermalMetrics_V1_0_1_CS (ThermalMetricsV1_0_1->ThermalMetrics); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ThermalMetricsFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyThermalMetrics_V1_0_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishThermalMetrics_V1_0_1EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishThermalMetrics_V1_0_1Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
