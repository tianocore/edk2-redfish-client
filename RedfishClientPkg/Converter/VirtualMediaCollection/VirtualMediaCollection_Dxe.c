...
//
#include <RedfishJsonStructure/VirtualMediaCollection/EfiVirtualMediaCollection.h>
...
EFI_STATUS
EFIAPI
VirtualMediaCollectionToStruct (
...
)
{
  ...
  Status = (EFI_STATUS)Json_VirtualMediaCollection_To_CS (ResoruceRaw, &VirtualMediaCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ...
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)VirtualMediaCollection;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("VirtualMediaCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("VirtualMediaCollection"), "VirtualMediaCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("VirtualMediaCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("VirtualMediaCollection"), "VirtualMediaCollection");
...
/**
  The wrapper function of VirtualMediaCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
VirtualMediaCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return VirtualMediaCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_VirtualMediaCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_VirtualMediaCollection_JSON (*((EFI_REDFISH_VIRTUALMEDIACOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_VirtualMediaCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
...
EFI_STATUS
EFIAPI
VirtualMediaCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
...
/**
  This function destory JSON raw text returned from VirtualMediaCollectionFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyVirtualMediaCollection_Json (ResoruceRaw);
}
