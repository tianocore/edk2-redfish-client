...
//
#include <RedfishJsonStructure/ConnectionMethodCollection/EfiConnectionMethodCollection.h>
CHAR8 mResourceTypeStr [] = "#ConnectionMethodCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ConnectionMethodCollection 
...
EFI_STATUS
EFIAPI
ConnectionMethodCollectionToStruct (
...
{
  EFI_REDFISH_CONNECTIONMETHODCOLLECTION *ConnectionMethodCollection;
  EFI_REDFISH_CONNECTIONMETHODCOLLECTION_CS *ConnectionMethodCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ConnectionMethodCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_ConnectionMethodCollection_To_CS (ResoruceRaw, &ConnectionMethodCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ConnectionMethodCollection = (EFI_REDFISH_CONNECTIONMETHODCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CONNECTIONMETHODCOLLECTION));
  if (ConnectionMethodCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)ConnectionMethodCollection;
  ConnectionMethodCollection->ConnectionMethodCollection = ConnectionMethodCollectionCs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ConnectionMethodCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("ConnectionMethodCollection"), "ConnectionMethodCollection");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ConnectionMethodCollection"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("ConnectionMethodCollection"), "ConnectionMethodCollection");
...
/**
  The wrapper function of ConnectionMethodCollectionToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
ConnectionMethodCollectionToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return ConnectionMethodCollectionToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
EFI_STATUS
EFIAPI
ConnectionMethodCollectionToJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_ConnectionMethodCollection_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_ConnectionMethodCollection_JSON (*((EFI_REDFISH_CONNECTIONMETHODCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_ConnectionMethodCollection_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in ConnectionMethodCollectionToStruct.
...
EFI_STATUS
EFIAPI
ConnectionMethodCollectionDestoryStruct(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL   *This,
  IN EFI_REST_JSON_STRUCTURE_HEADER  *InterpProp
)
{
  ...
  ConnectionMethodCollection = (EFI_REDFISH_CONNECTIONMETHODCOLLECTION *)InterpProp;
  DestroyConnectionMethodCollection_CS (ConnectionMethodCollection->ConnectionMethodCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ConnectionMethodCollectionFromStruct
...
EFI_STATUS
EFIAPI
ConnectionMethodCollectionDestoryJson(
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL *This,
...
)
{
  ...
  return (EFI_STATUS)DestroyConnectionMethodCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishConnectionMethodCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishConnectionMethodCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
