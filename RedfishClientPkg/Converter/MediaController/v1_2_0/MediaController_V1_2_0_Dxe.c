...
//
#include <RedfishJsonStructure/MediaController/v1_2_0/EfiMediaControllerV1_2_0.h>
CHAR8 mResourceTypeStr [] = "#MediaController";
BOOLEAN IsRevisonController = TRUE;

// Support MediaController V1_2_0
...
{
  EFI_REDFISH_MEDIACONTROLLER_V1_2_0 *MediaControllerV1_2_0;
  EFI_REDFISH_MEDIACONTROLLER_V1_2_0_CS *MediaControllerV1_2_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MediaController") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_MediaController_V1_2_0_To_CS (ResoruceRaw, &MediaControllerV1_2_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MediaControllerV1_2_0 = (EFI_REDFISH_MEDIACONTROLLER_V1_2_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MEDIACONTROLLER_V1_2_0));
  if (MediaControllerV1_2_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)MediaControllerV1_2_0;
  MediaControllerV1_2_0->MediaController = MediaControllerV1_2_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MediaController"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("MediaController"), "MediaController");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MediaController"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("MediaController"), "MediaController");
...
/**
  The wrapper function of MediaControllerToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
EFI_STATUS
EFIAPI
MediaControllerToStructWrapper (
  IN EFI_REST_JSON_STRUCTURE_PROTOCOL         *This,
  IN EFI_REST_JSON_RESOURCE_TYPE_IDENTIFIER   *RsrcIdentifier OPTIONAL,
  IN CHAR8                                    *ResoruceRaw,
  OUT EFI_REST_JSON_STRUCTURE_HEADER          **InterpProp
...
{
  return MediaControllerToStruct (
			  This,
			  RsrcIdentifier,
			  ResoruceRaw,
			  NULL,
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_MediaController_V1_2_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_MediaController_V1_2_0_JSON (*((EFI_REDFISH_MEDIACONTROLLER_V1_2_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_MediaController_V1_2_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in MediaControllerToStruct.
...
)
{
  ...
  MediaControllerV1_2_0 = (EFI_REDFISH_MEDIACONTROLLER_V1_2_0 *)InterpProp;
  DestroyMediaController_V1_2_0_CS (MediaControllerV1_2_0->MediaController); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MediaControllerFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyMediaController_V1_2_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMediaController_V1_2_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMediaController_V1_2_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
  //
