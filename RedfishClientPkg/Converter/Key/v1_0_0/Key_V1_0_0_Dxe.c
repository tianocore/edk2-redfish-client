...
//
#include <RedfishJsonStructure/Key/v1_0_0/EfiKeyV1_0_0.h>
CHAR8 mResourceTypeStr [] = "#Key";
...
{
  EFI_REDFISH_KEY_V1_0_0 *KeyV1_0_0;
  EFI_REDFISH_KEY_V1_0_0_CS *KeyV1_0_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Key") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = (EFI_STATUS)Json_Key_V1_0_0_To_CS (ResoruceRaw, &KeyV1_0_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  KeyV1_0_0 = (EFI_REDFISH_KEY_V1_0_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_KEY_V1_0_0));
  if (KeyV1_0_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)KeyV1_0_0;
  KeyV1_0_0->Key = KeyV1_0_0Cs;
  ...
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Key"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, AsciiStrSize ("Key"), "Key");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Key"));
  AsciiStrCpyS (RestIntrpHeader->JsonRsrcIdentifier.DataType, AsciiStrSize ("Key"), "Key");
...
/**
  The wrapper function of KeyToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  DEBUG((DEBUG_MANAGEABILITY, "%a: Call to CS_To_Key_V1_0_0_JSON\n", __func__));
  RedfishStatus = (EFI_STATUS)CS_To_Key_V1_0_0_JSON (*((EFI_REDFISH_KEY_V1_0_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);
  if (EFI_ERROR(RedfishStatus)) {
    DEBUG((DEBUG_MANAGEABILITY, "%a Call to CS_To_Key_V1_0_0_JSON fail (%d)\n", __func__, RedfishStatus));
  }
  return RedfishStatus;
}

/**
  This function destory structure retunred in KeyToStruct.
...
)
{
  ...
  KeyV1_0_0 = (EFI_REDFISH_KEY_V1_0_0 *)InterpProp;
  DestroyKey_V1_0_0_CS (KeyV1_0_0->Key); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from KeyFromStruct
...
)
{
  ...
  return (EFI_STATUS)DestroyKey_V1_0_0_Json (ResoruceRaw);
}
