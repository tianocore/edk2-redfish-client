...
#include"Redfish_ThermalMetrics_v1_0_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
//Generate C structure for #ThermalMetrics.ResetMetrics
//
static RedfishCS_status GenResetMetricsCs(RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishThermalMetrics_V1_0_0_ResetMetrics_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishThermalMetrics_V1_0_0_ResetMetrics_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenOemActionsCs(RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishThermalMetrics_V1_0_0_OemActions_CS **Dst)
{
  RedfishCS_status Status;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishThermalMetrics_V1_0_0_OemActions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
...
//
static RedfishCS_status GenActionsCs(RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishThermalMetrics_V1_0_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishThermalMetrics_V1_0_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // #ThermalMetrics.ResetMetrics
  Status = GenResetMetricsCs (Cs, TempJsonObj, "#ThermalMetrics.ResetMetrics", &(*Dst)->ThermalMetrics_ResetMetrics);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//Generate C structure for Oem
//
static RedfishCS_status GenOemCs(RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
  RedfishCS_status Status;
  RedfishCS_Type_JSON_Data *CsTypeJson;
  RedfishCS_Type_EmptyProp_CS_Data *CsTypeEmptyPropCS;
...
//
//Generate C structure for TemperatureSummaryCelsius
//
static RedfishCS_status GenTemperatureSummaryCs(RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishThermalMetrics_V1_0_0_TemperatureSummary_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishThermalMetrics_V1_0_0_TemperatureSummary_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Ambient
  InitializeLinkHead (&(*Dst)->Ambient);
  Status = CreateCsUriOrJsonByNode (Cs, TempJsonObj, "Ambient", Cs->Header.ThisUri, &(*Dst)->Ambient);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Exhaust
  InitializeLinkHead (&(*Dst)->Exhaust);
  Status = CreateCsUriOrJsonByNode (Cs, TempJsonObj, "Exhaust", Cs->Header.ThisUri, &(*Dst)->Exhaust);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Intake
  InitializeLinkHead (&(*Dst)->Intake);
  Status = CreateCsUriOrJsonByNode (Cs, TempJsonObj, "Intake", Cs->Header.ThisUri, &(*Dst)->Intake);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Internal
  InitializeLinkHead (&(*Dst)->Internal);
  Status = CreateCsUriOrJsonByNode (Cs, TempJsonObj, "Internal", Cs->Header.ThisUri, &(*Dst)->Internal);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishThermalMetrics_V1_0_0_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
...
static RedfishCS_status CS_To_JSON_ActionsThermalMetrics_ResetMetrics(json_t *CsJson, char *Key, RedfishThermalMetrics_V1_0_0_ResetMetrics_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishThermalMetrics_V1_0_0_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  ...
  // #ThermalMetrics.ResetMetrics
  if (CS_To_JSON_ActionsThermalMetrics_ResetMetrics(CsJson, "#ThermalMetrics.ResetMetrics", CSPtr->ThermalMetrics_ResetMetrics) != RedfishCS_status_success) {goto Error;}
...
static RedfishCS_status CS_To_JSON_TemperatureSummaryCelsius(json_t *CsJson, char *Key, RedfishThermalMetrics_V1_0_0_TemperatureSummary_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  // Ambient 
  if (InsertJsonLinkObj (CsJson, "Ambient", &CSPtr->Ambient) != RedfishCS_status_success) {goto Error;}

  // Exhaust 
  if (InsertJsonLinkObj (CsJson, "Exhaust", &CSPtr->Exhaust) != RedfishCS_status_success) {goto Error;}

  // Intake 
  if (InsertJsonLinkObj (CsJson, "Intake", &CSPtr->Intake) != RedfishCS_status_success) {goto Error;}

  // Internal 
  if (InsertJsonLinkObj (CsJson, "Internal", &CSPtr->Internal) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for ThermalMetrics.V1_0_0.ThermalMetrics
//
RedfishCS_status CS_To_ThermalMetrics_V1_0_0_JSON(RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  if (InsertJsonLinkArrayObj (CsJson, "TemperatureReadingsCelsius", &CSPtr->TemperatureReadingsCelsius) != RedfishCS_status_success) {goto Error;}

  // TemperatureReadingsCelsius@odata.count 
  if (InsertJsonInt64Obj (CsJson, "TemperatureReadingsCelsius@odata.count", CSPtr->TemperatureReadingsCelsiusodata_count) != RedfishCS_status_success) {goto Error;}

  // TemperatureSummaryCelsius
  if (CS_To_JSON_TemperatureSummaryCelsius(CsJson, "TemperatureSummaryCelsius", CSPtr->TemperatureSummaryCelsius) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for ThermalMetrics.V1_0_0.ThermalMetrics
//
RedfishCS_status DestroyThermalMetrics_V1_0_0_CS(RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for ThermalMetrics.V1_0_0.ThermalMetrics
//
RedfishCS_status DestroyThermalMetrics_V1_0_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for ThermalMetrics.V1_0_0.ThermalMetrics
//
RedfishCS_status
Json_ThermalMetrics_V1_0_0_To_CS(RedfishCS_char *JsonRawText, RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "ThermalMetrics", "v1_0_0", "ThermalMetrics", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishThermalMetrics_V1_0_0_ThermalMetrics_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // TemperatureReadingsCelsius
  InitializeLinkHead (&Cs->TemperatureReadingsCelsius);
  Status = CreateCsUriOrJsonByNodeArray (Cs, JsonObj, "TemperatureReadingsCelsius", Cs->Header.ThisUri, &Cs->TemperatureReadingsCelsius);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TemperatureReadingsCelsius@odata.count
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "TemperatureReadingsCelsius@odata.count", &Cs->TemperatureReadingsCelsiusodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TemperatureSummaryCelsius
  Status = GenTemperatureSummaryCs (Cs, JsonObj, "TemperatureSummaryCelsius", &Cs->TemperatureSummaryCelsius);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroyThermalMetrics_V1_0_0_CS (Cs);
  return Status;
}
