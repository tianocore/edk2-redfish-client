...
#include"Redfish_ConnectionCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishConnectionCollection_ConnectionCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for ConnectionCollection.ConnectionCollection
//
RedfishCS_status CS_To_ConnectionCollection_JSON(RedfishConnectionCollection_ConnectionCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for ConnectionCollection.ConnectionCollection
//
RedfishCS_status DestroyConnectionCollection_CS(RedfishConnectionCollection_ConnectionCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for ConnectionCollection.ConnectionCollection
//
RedfishCS_status DestroyConnectionCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for ConnectionCollection.NOVERSIONED.ConnectionCollection
//
RedfishCS_status
Json_ConnectionCollection_To_CS(RedfishCS_char *JsonRawText, RedfishConnectionCollection_ConnectionCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishConnectionCollection_ConnectionCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "ConnectionCollection", "noversioned", "ConnectionCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishConnectionCollection_ConnectionCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyConnectionCollection_CS (Cs);
  return Status;
}
