...
#include"Redfish_AllowDenyCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishAllowDenyCollection_AllowDenyCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for AllowDenyCollection.AllowDenyCollection
//
RedfishCS_status CS_To_AllowDenyCollection_JSON(RedfishAllowDenyCollection_AllowDenyCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for AllowDenyCollection.AllowDenyCollection
//
RedfishCS_status DestroyAllowDenyCollection_CS(RedfishAllowDenyCollection_AllowDenyCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for AllowDenyCollection.AllowDenyCollection
//
RedfishCS_status DestroyAllowDenyCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for AllowDenyCollection.NOVERSIONED.AllowDenyCollection
//
RedfishCS_status
Json_AllowDenyCollection_To_CS(RedfishCS_char *JsonRawText, RedfishAllowDenyCollection_AllowDenyCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishAllowDenyCollection_AllowDenyCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "AllowDenyCollection", "noversioned", "AllowDenyCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishAllowDenyCollection_AllowDenyCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyAllowDenyCollection_CS (Cs);
  return Status;
}
