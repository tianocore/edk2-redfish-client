...
#include"Redfish_BatteryCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishBatteryCollection_BatteryCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for BatteryCollection.BatteryCollection
//
RedfishCS_status CS_To_BatteryCollection_JSON(RedfishBatteryCollection_BatteryCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for BatteryCollection.BatteryCollection
//
RedfishCS_status DestroyBatteryCollection_CS(RedfishBatteryCollection_BatteryCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for BatteryCollection.BatteryCollection
//
RedfishCS_status DestroyBatteryCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for BatteryCollection.NOVERSIONED.BatteryCollection
//
RedfishCS_status
Json_BatteryCollection_To_CS(RedfishCS_char *JsonRawText, RedfishBatteryCollection_BatteryCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishBatteryCollection_BatteryCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "BatteryCollection", "noversioned", "BatteryCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishBatteryCollection_BatteryCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyBatteryCollection_CS (Cs);
  return Status;
}
