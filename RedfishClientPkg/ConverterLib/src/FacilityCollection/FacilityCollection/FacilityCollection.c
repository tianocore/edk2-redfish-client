...
#include"Redfish_FacilityCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishFacilityCollection_FacilityCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for FacilityCollection.FacilityCollection
//
RedfishCS_status CS_To_FacilityCollection_JSON(RedfishFacilityCollection_FacilityCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for FacilityCollection.FacilityCollection
//
RedfishCS_status DestroyFacilityCollection_CS(RedfishFacilityCollection_FacilityCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for FacilityCollection.FacilityCollection
//
RedfishCS_status DestroyFacilityCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for FacilityCollection.NOVERSIONED.FacilityCollection
//
RedfishCS_status
Json_FacilityCollection_To_CS(RedfishCS_char *JsonRawText, RedfishFacilityCollection_FacilityCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishFacilityCollection_FacilityCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "FacilityCollection", "noversioned", "FacilityCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishFacilityCollection_FacilityCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyFacilityCollection_CS (Cs);
  return Status;
}
