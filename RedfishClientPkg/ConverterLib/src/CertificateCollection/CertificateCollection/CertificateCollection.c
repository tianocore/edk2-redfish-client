...
#include"Redfish_CertificateCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishCertificateCollection_CertificateCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for CertificateCollection.CertificateCollection
//
RedfishCS_status CS_To_CertificateCollection_JSON(RedfishCertificateCollection_CertificateCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for CertificateCollection.CertificateCollection
//
RedfishCS_status DestroyCertificateCollection_CS(RedfishCertificateCollection_CertificateCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for CertificateCollection.CertificateCollection
//
RedfishCS_status DestroyCertificateCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for CertificateCollection.NOVERSIONED.CertificateCollection
//
RedfishCS_status
Json_CertificateCollection_To_CS(RedfishCS_char *JsonRawText, RedfishCertificateCollection_CertificateCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishCertificateCollection_CertificateCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "CertificateCollection", "noversioned", "CertificateCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishCertificateCollection_CertificateCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyCertificateCollection_CS (Cs);
  return Status;
}
