...
#include"Redfish_SecureBootDatabaseCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishSecureBootDatabaseCollection_SecureBootDatabaseCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for SecureBootDatabaseCollection.SecureBootDatabaseCollection
//
RedfishCS_status CS_To_SecureBootDatabaseCollection_JSON(RedfishSecureBootDatabaseCollection_SecureBootDatabaseCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for SecureBootDatabaseCollection.SecureBootDatabaseCollection
//
RedfishCS_status DestroySecureBootDatabaseCollection_CS(RedfishSecureBootDatabaseCollection_SecureBootDatabaseCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for SecureBootDatabaseCollection.SecureBootDatabaseCollection
//
RedfishCS_status DestroySecureBootDatabaseCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for SecureBootDatabaseCollection.NOVERSIONED.SecureBootDatabaseCollection
//
RedfishCS_status
Json_SecureBootDatabaseCollection_To_CS(RedfishCS_char *JsonRawText, RedfishSecureBootDatabaseCollection_SecureBootDatabaseCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishSecureBootDatabaseCollection_SecureBootDatabaseCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "SecureBootDatabaseCollection", "noversioned", "SecureBootDatabaseCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishSecureBootDatabaseCollection_SecureBootDatabaseCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroySecureBootDatabaseCollection_CS (Cs);
  return Status;
}
