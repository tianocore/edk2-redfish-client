...
#include"Redfish_Facility_v1_3_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenOemActionsCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, char *Key, RedfishFacility_V1_3_0_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishFacility_V1_3_0_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, char *Key, RedfishFacility_V1_3_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishFacility_V1_3_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
static RedfishCS_status GenLinksCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, char *Key, RedfishFacility_V1_3_0_Links_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishFacility_V1_3_0_Links_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // ContainedByFacility
  InitializeLinkHead (&(*Dst)->ContainedByFacility);
  Status = CreateCsUriOrJsonByNode (Cs, TempJsonObj, "ContainedByFacility", Cs->Header.ThisUri, &(*Dst)->ContainedByFacility);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ContainsChassis
  InitializeLinkHead (&(*Dst)->ContainsChassis);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "ContainsChassis", Cs->Header.ThisUri, &(*Dst)->ContainsChassis);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ContainsChassis@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "ContainsChassis@odata.count", &(*Dst)->ContainsChassisodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ContainsFacilities
  InitializeLinkHead (&(*Dst)->ContainsFacilities);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "ContainsFacilities", Cs->Header.ThisUri, &(*Dst)->ContainsFacilities);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ContainsFacilities@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "ContainsFacilities@odata.count", &(*Dst)->ContainsFacilitiesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ElectricalBuses
  InitializeLinkHead (&(*Dst)->ElectricalBuses);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "ElectricalBuses", Cs->Header.ThisUri, &(*Dst)->ElectricalBuses);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ElectricalBuses@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "ElectricalBuses@odata.count", &(*Dst)->ElectricalBusesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // FloorPDUs
  InitializeLinkHead (&(*Dst)->FloorPDUs);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "FloorPDUs", Cs->Header.ThisUri, &(*Dst)->FloorPDUs);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // FloorPDUs@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "FloorPDUs@odata.count", &(*Dst)->FloorPDUsodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // PowerShelves
  InitializeLinkHead (&(*Dst)->PowerShelves);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "PowerShelves", Cs->Header.ThisUri, &(*Dst)->PowerShelves);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // PowerShelves@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "PowerShelves@odata.count", &(*Dst)->PowerShelvesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // RackPDUs
  InitializeLinkHead (&(*Dst)->RackPDUs);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "RackPDUs", Cs->Header.ThisUri, &(*Dst)->RackPDUs);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // RackPDUs@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "RackPDUs@odata.count", &(*Dst)->RackPDUsodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Switchgear
  InitializeLinkHead (&(*Dst)->Switchgear);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "Switchgear", Cs->Header.ThisUri, &(*Dst)->Switchgear);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Switchgear@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "Switchgear@odata.count", &(*Dst)->Switchgearodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TransferSwitches
  InitializeLinkHead (&(*Dst)->TransferSwitches);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "TransferSwitches", Cs->Header.ThisUri, &(*Dst)->TransferSwitches);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TransferSwitches@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "TransferSwitches@odata.count", &(*Dst)->TransferSwitchesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenidRefCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, char *Key, Redfishodatav4_idRef_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status GenCondition_Array_ElementCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  RedfishResource_Condition_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenCondition_ArrayCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Condition_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
...
//
static RedfishCS_status GenStatusCs(RedfishFacility_V1_3_0_Facility_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishFacility_V1_3_0_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishFacility_V1_3_0_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Links(json_t *CsJson, char *Key, RedfishFacility_V1_3_0_Links_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  // ContainedByFacility 
  if (InsertJsonLinkObj (CsJson, "ContainedByFacility", &CSPtr->ContainedByFacility) != RedfishCS_status_success) {goto Error;}

  // ContainsChassis 
  if (InsertJsonLinkArrayObj (CsJson, "ContainsChassis", &CSPtr->ContainsChassis) != RedfishCS_status_success) {goto Error;}

  // ContainsChassis@odata.count 
  if (InsertJsonInt64Obj (CsJson, "ContainsChassis@odata.count", CSPtr->ContainsChassisodata_count) != RedfishCS_status_success) {goto Error;}

  // ContainsFacilities 
  if (InsertJsonLinkArrayObj (CsJson, "ContainsFacilities", &CSPtr->ContainsFacilities) != RedfishCS_status_success) {goto Error;}

  // ContainsFacilities@odata.count 
  if (InsertJsonInt64Obj (CsJson, "ContainsFacilities@odata.count", CSPtr->ContainsFacilitiesodata_count) != RedfishCS_status_success) {goto Error;}

  // ElectricalBuses 
  if (InsertJsonLinkArrayObj (CsJson, "ElectricalBuses", &CSPtr->ElectricalBuses) != RedfishCS_status_success) {goto Error;}

  // ElectricalBuses@odata.count 
  if (InsertJsonInt64Obj (CsJson, "ElectricalBuses@odata.count", CSPtr->ElectricalBusesodata_count) != RedfishCS_status_success) {goto Error;}

  // FloorPDUs 
  if (InsertJsonLinkArrayObj (CsJson, "FloorPDUs", &CSPtr->FloorPDUs) != RedfishCS_status_success) {goto Error;}

  // FloorPDUs@odata.count 
  if (InsertJsonInt64Obj (CsJson, "FloorPDUs@odata.count", CSPtr->FloorPDUsodata_count) != RedfishCS_status_success) {goto Error;}
  ...
  // PowerShelves 
  if (InsertJsonLinkArrayObj (CsJson, "PowerShelves", &CSPtr->PowerShelves) != RedfishCS_status_success) {goto Error;}

  // PowerShelves@odata.count 
  if (InsertJsonInt64Obj (CsJson, "PowerShelves@odata.count", CSPtr->PowerShelvesodata_count) != RedfishCS_status_success) {goto Error;}

  // RackPDUs 
  if (InsertJsonLinkArrayObj (CsJson, "RackPDUs", &CSPtr->RackPDUs) != RedfishCS_status_success) {goto Error;}

  // RackPDUs@odata.count 
  if (InsertJsonInt64Obj (CsJson, "RackPDUs@odata.count", CSPtr->RackPDUsodata_count) != RedfishCS_status_success) {goto Error;}

  // Switchgear 
  if (InsertJsonLinkArrayObj (CsJson, "Switchgear", &CSPtr->Switchgear) != RedfishCS_status_success) {goto Error;}

  // Switchgear@odata.count 
  if (InsertJsonInt64Obj (CsJson, "Switchgear@odata.count", CSPtr->Switchgearodata_count) != RedfishCS_status_success) {goto Error;}

  // TransferSwitches 
  if (InsertJsonLinkArrayObj (CsJson, "TransferSwitches", &CSPtr->TransferSwitches) != RedfishCS_status_success) {goto Error;}

  // TransferSwitches@odata.count 
  if (InsertJsonInt64Obj (CsJson, "TransferSwitches@odata.count", CSPtr->TransferSwitchesodata_count) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for Facility.V1_3_0.Facility
//
RedfishCS_status CS_To_Facility_V1_3_0_JSON(RedfishFacility_V1_3_0_Facility_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // AmbientMetrics 
  if (InsertJsonLinkObj (CsJson, "AmbientMetrics", &CSPtr->AmbientMetrics) != RedfishCS_status_success) {goto Error;}
  ...
  // FacilityType 
  if (InsertJsonStringObj (CsJson, "FacilityType", CSPtr->FacilityType) != RedfishCS_status_success) {goto Error;}
  ...
  // PowerDomains 
  if (InsertJsonLinkObj (CsJson, "PowerDomains", &CSPtr->PowerDomains) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for Facility.V1_3_0.Facility
//
RedfishCS_status DestroyFacility_V1_3_0_CS(RedfishFacility_V1_3_0_Facility_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for Facility.V1_3_0.Facility
//
RedfishCS_status DestroyFacility_V1_3_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for Facility.V1_3_0.Facility
//
RedfishCS_status
Json_Facility_V1_3_0_To_CS(RedfishCS_char *JsonRawText, RedfishFacility_V1_3_0_Facility_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishFacility_V1_3_0_Facility_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "Facility", "v1_3_0", "Facility", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishFacility_V1_3_0_Facility_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // AmbientMetrics
  InitializeLinkHead (&Cs->AmbientMetrics);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "AmbientMetrics", Cs->Header.ThisUri, &Cs->AmbientMetrics);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // FacilityType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "FacilityType", &Cs->FacilityType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){goto Error;}}
  ...
  // PowerDomains
  InitializeLinkHead (&Cs->PowerDomains);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "PowerDomains", Cs->Header.ThisUri, &Cs->PowerDomains);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
