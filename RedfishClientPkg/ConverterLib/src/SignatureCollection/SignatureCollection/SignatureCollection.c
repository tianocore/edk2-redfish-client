...
#include"Redfish_SignatureCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishSignatureCollection_SignatureCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for SignatureCollection.SignatureCollection
//
RedfishCS_status CS_To_SignatureCollection_JSON(RedfishSignatureCollection_SignatureCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for SignatureCollection.SignatureCollection
//
RedfishCS_status DestroySignatureCollection_CS(RedfishSignatureCollection_SignatureCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for SignatureCollection.SignatureCollection
//
RedfishCS_status DestroySignatureCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for SignatureCollection.NOVERSIONED.SignatureCollection
//
RedfishCS_status
Json_SignatureCollection_To_CS(RedfishCS_char *JsonRawText, RedfishSignatureCollection_SignatureCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishSignatureCollection_SignatureCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "SignatureCollection", "noversioned", "SignatureCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishSignatureCollection_SignatureCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroySignatureCollection_CS (Cs);
  return Status;
}
