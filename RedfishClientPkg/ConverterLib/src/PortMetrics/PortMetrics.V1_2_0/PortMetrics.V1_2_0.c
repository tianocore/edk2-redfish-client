...
#include"Redfish_PortMetrics_v1_2_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemActionsCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishPortMetrics_V1_2_0_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPortMetrics_V1_2_0_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishPortMetrics_V1_2_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPortMetrics_V1_2_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
static RedfishCS_status GenFibreChannelCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishPortMetrics_V1_2_0_FibreChannel_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPortMetrics_V1_2_0_FibreChannel_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // CorrectableFECErrors
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "CorrectableFECErrors", &(*Dst)->CorrectableFECErrors);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // InvalidCRCs
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "InvalidCRCs", &(*Dst)->InvalidCRCs);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // InvalidTXWords
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "InvalidTXWords", &(*Dst)->InvalidTXWords);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // LinkFailures
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "LinkFailures", &(*Dst)->LinkFailures);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // LossesOfSignal
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "LossesOfSignal", &(*Dst)->LossesOfSignal);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // LossesOfSync
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "LossesOfSync", &(*Dst)->LossesOfSync);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // RXBBCreditZero
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "RXBBCreditZero", &(*Dst)->RXBBCreditZero);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // RXExchanges
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "RXExchanges", &(*Dst)->RXExchanges);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // RXSequences
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "RXSequences", &(*Dst)->RXSequences);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TXBBCreditZero
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "TXBBCreditZero", &(*Dst)->TXBBCreditZero);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TXBBCreditZeroDurationMilliseconds
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "TXBBCreditZeroDurationMilliseconds", &(*Dst)->TXBBCreditZeroDurationMilliseconds);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TXBBCredits
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "TXBBCredits", &(*Dst)->TXBBCredits);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TXExchanges
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "TXExchanges", &(*Dst)->TXExchanges);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // TXSequences
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "TXSequences", &(*Dst)->TXSequences);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // UncorrectableFECErrors
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "UncorrectableFECErrors", &(*Dst)->UncorrectableFECErrors);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
static RedfishCS_status GenGenZCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishPortMetrics_V1_2_0_GenZ_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPortMetrics_V1_2_0_GenZ_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // AccessKeyViolations
...
//
static RedfishCS_status GenNetworkingCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishPortMetrics_V1_2_0_Networking_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPortMetrics_V1_2_0_Networking_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
...
//Generate C structure for Oem
//
static RedfishCS_status GenOemCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
  RedfishCS_status Status;
  RedfishCS_Type_JSON_Data *CsTypeJson;
  RedfishCS_Type_EmptyProp_CS_Data *CsTypeEmptyPropCS;
...
static RedfishCS_status GenSAS_Array_ElementCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  RedfishPortMetrics_V1_2_0_SAS_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsZeroMemory(Cs, sizeof(RedfishPortMetrics_V1_2_0_SAS_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // InvalidDwordCount
...
//
//Generate C structure for RedfishPortMetrics_V1_2_0_SAS_Array_CS
//
static RedfishCS_status GenSAS_ArrayCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishPortMetrics_V1_2_0_SAS_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
  RedfishCS_uint64 Index;
  RedfishPortMetrics_V1_2_0_SAS_Array_CS *ThisElement;
  ...
  Status = allocateArrayRecordCsMemory(Cs, sizeof (RedfishPortMetrics_V1_2_0_SAS_Array_CS), ArraySize, (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  if (*Dst == NULL) {
...
static RedfishCS_status GenTransceiver_Array_ElementCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  RedfishPortMetrics_V1_2_0_Transceiver_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsZeroMemory(Cs, sizeof(RedfishPortMetrics_V1_2_0_Transceiver_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
...
//
//Generate C structure for RedfishPortMetrics_V1_2_0_Transceiver_Array_CS
//
static RedfishCS_status GenTransceiver_ArrayCs(RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs, json_t *JsonObj, char *Key, RedfishPortMetrics_V1_2_0_Transceiver_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
  RedfishCS_uint64 Index;
  RedfishPortMetrics_V1_2_0_Transceiver_Array_CS *ThisElement;
  ...
  ArraySize = json_array_size (TempJsonObj);
  Status = allocateArrayRecordCsMemory(Cs, sizeof (RedfishPortMetrics_V1_2_0_Transceiver_Array_CS), ArraySize, (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  if (*Dst == NULL) {
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishPortMetrics_V1_2_0_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishPortMetrics_V1_2_0_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_FibreChannel(json_t *CsJson, char *Key, RedfishPortMetrics_V1_2_0_FibreChannel_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  // CorrectableFECErrors 
  if (InsertJsonInt64Obj (CsJson, "CorrectableFECErrors", CSPtr->CorrectableFECErrors) != RedfishCS_status_success) {goto Error;}

  // InvalidCRCs 
  if (InsertJsonInt64Obj (CsJson, "InvalidCRCs", CSPtr->InvalidCRCs) != RedfishCS_status_success) {goto Error;}

  // InvalidTXWords 
  if (InsertJsonInt64Obj (CsJson, "InvalidTXWords", CSPtr->InvalidTXWords) != RedfishCS_status_success) {goto Error;}

  // LinkFailures 
  if (InsertJsonInt64Obj (CsJson, "LinkFailures", CSPtr->LinkFailures) != RedfishCS_status_success) {goto Error;}

  // LossesOfSignal 
  if (InsertJsonInt64Obj (CsJson, "LossesOfSignal", CSPtr->LossesOfSignal) != RedfishCS_status_success) {goto Error;}

  // LossesOfSync 
  if (InsertJsonInt64Obj (CsJson, "LossesOfSync", CSPtr->LossesOfSync) != RedfishCS_status_success) {goto Error;}

  // RXBBCreditZero 
  if (InsertJsonInt64Obj (CsJson, "RXBBCreditZero", CSPtr->RXBBCreditZero) != RedfishCS_status_success) {goto Error;}

  // RXExchanges 
  if (InsertJsonInt64Obj (CsJson, "RXExchanges", CSPtr->RXExchanges) != RedfishCS_status_success) {goto Error;}

  // RXSequences 
  if (InsertJsonInt64Obj (CsJson, "RXSequences", CSPtr->RXSequences) != RedfishCS_status_success) {goto Error;}

  // TXBBCreditZero 
  if (InsertJsonInt64Obj (CsJson, "TXBBCreditZero", CSPtr->TXBBCreditZero) != RedfishCS_status_success) {goto Error;}

  // TXBBCreditZeroDurationMilliseconds 
  if (InsertJsonInt64Obj (CsJson, "TXBBCreditZeroDurationMilliseconds", CSPtr->TXBBCreditZeroDurationMilliseconds) != RedfishCS_status_success) {goto Error;}

  // TXBBCredits 
  if (InsertJsonInt64Obj (CsJson, "TXBBCredits", CSPtr->TXBBCredits) != RedfishCS_status_success) {goto Error;}

  // TXExchanges 
  if (InsertJsonInt64Obj (CsJson, "TXExchanges", CSPtr->TXExchanges) != RedfishCS_status_success) {goto Error;}

  // TXSequences 
  if (InsertJsonInt64Obj (CsJson, "TXSequences", CSPtr->TXSequences) != RedfishCS_status_success) {goto Error;}

  // UncorrectableFECErrors 
  if (InsertJsonInt64Obj (CsJson, "UncorrectableFECErrors", CSPtr->UncorrectableFECErrors) != RedfishCS_status_success) {goto Error;}

  return RedfishCS_status_success;
Error:;
  return RedfishCS_status_unsupported;
}
static RedfishCS_status CS_To_JSON_GenZ(json_t *CsJson, char *Key, RedfishPortMetrics_V1_2_0_GenZ_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Networking(json_t *CsJson, char *Key, RedfishPortMetrics_V1_2_0_Networking_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_SAS(json_t *CsJson, char *Key, RedfishPortMetrics_V1_2_0_SAS_Array_CS *CSPtr)
{
  json_t *ArrayJson;
  json_t *ArrayMember;
  RedfishPortMetrics_V1_2_0_SAS_Array_CS *NextArray;
  RedfishPortMetrics_V1_2_0_SAS_CS *NextArrayItem;
...
static RedfishCS_status CS_To_JSON_Transceivers(json_t *CsJson, char *Key, RedfishPortMetrics_V1_2_0_Transceiver_Array_CS *CSPtr)
{
  json_t *ArrayJson;
  json_t *ArrayMember;
  RedfishPortMetrics_V1_2_0_Transceiver_Array_CS *NextArray;
  RedfishPortMetrics_V1_2_0_Transceiver_CS *NextArrayItem;
...
//
// C structure to JSON for PortMetrics.V1_2_0.PortMetrics
//
RedfishCS_status CS_To_PortMetrics_V1_2_0_JSON(RedfishPortMetrics_V1_2_0_PortMetrics_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for PortMetrics.V1_2_0.PortMetrics
//
RedfishCS_status DestroyPortMetrics_V1_2_0_CS(RedfishPortMetrics_V1_2_0_PortMetrics_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for PortMetrics.V1_2_0.PortMetrics
//
RedfishCS_status DestroyPortMetrics_V1_2_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for PortMetrics.V1_2_0.PortMetrics
//
RedfishCS_status
Json_PortMetrics_V1_2_0_To_CS(RedfishCS_char *JsonRawText, RedfishPortMetrics_V1_2_0_PortMetrics_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishPortMetrics_V1_2_0_PortMetrics_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "PortMetrics", "v1_2_0", "PortMetrics", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishPortMetrics_V1_2_0_PortMetrics_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyPortMetrics_V1_2_0_CS (Cs);
  return Status;
}
