...
#include"Redfish_KeyPolicy_v1_0_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishKeyPolicy_V1_0_0_KeyPolicy_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenOemActionsCs(RedfishKeyPolicy_V1_0_0_KeyPolicy_CS *Cs, json_t *JsonObj, char *Key, RedfishKeyPolicy_V1_0_0_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishKeyPolicy_V1_0_0_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishKeyPolicy_V1_0_0_KeyPolicy_CS *Cs, json_t *JsonObj, char *Key, RedfishKeyPolicy_V1_0_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishKeyPolicy_V1_0_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
//Generate C structure for NVMeoF
//
static RedfishCS_status GenNVMeoFCs(RedfishKeyPolicy_V1_0_0_KeyPolicy_CS *Cs, json_t *JsonObj, char *Key, RedfishKeyPolicy_V1_0_0_NVMeoF_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishKeyPolicy_V1_0_0_NVMeoF_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // CipherSuiteAllowList
  Status = GenRedfishCS_char_ArrayCs (Cs, TempJsonObj, "CipherSuiteAllowList", &(*Dst)->CipherSuiteAllowList);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // DHGroupAllowList
  Status = GenRedfishCS_char_ArrayCs (Cs, TempJsonObj, "DHGroupAllowList", &(*Dst)->DHGroupAllowList);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // OEMSecurityProtocolAllowList
  Status = GenRedfishCS_char_ArrayCs (Cs, TempJsonObj, "OEMSecurityProtocolAllowList", &(*Dst)->OEMSecurityProtocolAllowList);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SecureHashAllowList
  Status = GenRedfishCS_char_ArrayCs (Cs, TempJsonObj, "SecureHashAllowList", &(*Dst)->SecureHashAllowList);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SecurityProtocolAllowList
  Status = GenRedfishCS_char_ArrayCs (Cs, TempJsonObj, "SecurityProtocolAllowList", &(*Dst)->SecurityProtocolAllowList);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SecurityTransportAllowList
  Status = GenRedfishCS_char_ArrayCs (Cs, TempJsonObj, "SecurityTransportAllowList", &(*Dst)->SecurityTransportAllowList);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//Generate C structure for Oem
//
static RedfishCS_status GenOemCs(RedfishKeyPolicy_V1_0_0_KeyPolicy_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
  RedfishCS_status Status;
  RedfishCS_Type_JSON_Data *CsTypeJson;
  RedfishCS_Type_EmptyProp_CS_Data *CsTypeEmptyPropCS;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishKeyPolicy_V1_0_0_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishKeyPolicy_V1_0_0_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_NVMeoF(json_t *CsJson, char *Key, RedfishKeyPolicy_V1_0_0_NVMeoF_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  // CipherSuiteAllowList 
  if (InsertJsonStringArrayObj (CsJson, "CipherSuiteAllowList", CSPtr->CipherSuiteAllowList) != RedfishCS_status_success) {goto Error;}

  // DHGroupAllowList 
  if (InsertJsonStringArrayObj (CsJson, "DHGroupAllowList", CSPtr->DHGroupAllowList) != RedfishCS_status_success) {goto Error;}

  // OEMSecurityProtocolAllowList 
  if (InsertJsonStringArrayObj (CsJson, "OEMSecurityProtocolAllowList", CSPtr->OEMSecurityProtocolAllowList) != RedfishCS_status_success) {goto Error;}

  // SecureHashAllowList 
  if (InsertJsonStringArrayObj (CsJson, "SecureHashAllowList", CSPtr->SecureHashAllowList) != RedfishCS_status_success) {goto Error;}

  // SecurityProtocolAllowList 
  if (InsertJsonStringArrayObj (CsJson, "SecurityProtocolAllowList", CSPtr->SecurityProtocolAllowList) != RedfishCS_status_success) {goto Error;}

  // SecurityTransportAllowList 
  if (InsertJsonStringArrayObj (CsJson, "SecurityTransportAllowList", CSPtr->SecurityTransportAllowList) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for KeyPolicy.V1_0_0.KeyPolicy
//
RedfishCS_status CS_To_KeyPolicy_V1_0_0_JSON(RedfishKeyPolicy_V1_0_0_KeyPolicy_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // IsDefault 
  if (InsertJsonBoolObj (CsJson, "IsDefault", CSPtr->IsDefault) != RedfishCS_status_success) {goto Error;}

  // KeyPolicyType 
  if (InsertJsonStringObj (CsJson, "KeyPolicyType", CSPtr->KeyPolicyType) != RedfishCS_status_success) {goto Error;}

  // NVMeoF
  if (CS_To_JSON_NVMeoF(CsJson, "NVMeoF", CSPtr->NVMeoF) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for KeyPolicy.V1_0_0.KeyPolicy
//
RedfishCS_status DestroyKeyPolicy_V1_0_0_CS(RedfishKeyPolicy_V1_0_0_KeyPolicy_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for KeyPolicy.V1_0_0.KeyPolicy
//
RedfishCS_status DestroyKeyPolicy_V1_0_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for KeyPolicy.V1_0_0.KeyPolicy
//
RedfishCS_status
Json_KeyPolicy_V1_0_0_To_CS(RedfishCS_char *JsonRawText, RedfishKeyPolicy_V1_0_0_KeyPolicy_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishKeyPolicy_V1_0_0_KeyPolicy_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "KeyPolicy", "v1_0_0", "KeyPolicy", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishKeyPolicy_V1_0_0_KeyPolicy_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // IsDefault
  Status = GetRedfishPropertyBoolean (Cs, JsonObj, "IsDefault", &Cs->IsDefault);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // KeyPolicyType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "KeyPolicyType", &Cs->KeyPolicyType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // NVMeoF
  Status = GenNVMeoFCs (Cs, JsonObj, "NVMeoF", &Cs->NVMeoF);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
