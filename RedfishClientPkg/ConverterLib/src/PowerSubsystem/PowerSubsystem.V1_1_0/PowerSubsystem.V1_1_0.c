...
#include"Redfish_PowerSubsystem_v1_1_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenOemActionsCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishPowerSubsystem_V1_1_0_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPowerSubsystem_V1_1_0_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishPowerSubsystem_V1_1_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPowerSubsystem_V1_1_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
//Generate C structure for Allocation
//
static RedfishCS_status GenPowerAllocationCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishPowerSubsystem_V1_1_0_PowerAllocation_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPowerSubsystem_V1_1_0_PowerAllocation_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // AllocatedWatts
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "AllocatedWatts", &(*Dst)->AllocatedWatts);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // RequestedWatts
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "RequestedWatts", &(*Dst)->RequestedWatts);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenidRefCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, char *Key, Redfishodatav4_idRef_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status GenCondition_Array_ElementCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  RedfishResource_Condition_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenCondition_ArrayCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Condition_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
...
//
static RedfishCS_status GenStatusCs(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishPowerSubsystem_V1_1_0_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishPowerSubsystem_V1_1_0_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Allocation(json_t *CsJson, char *Key, RedfishPowerSubsystem_V1_1_0_PowerAllocation_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  // AllocatedWatts 
  if (InsertJsonInt64Obj (CsJson, "AllocatedWatts", CSPtr->AllocatedWatts) != RedfishCS_status_success) {goto Error;}

  // RequestedWatts 
  if (InsertJsonInt64Obj (CsJson, "RequestedWatts", CSPtr->RequestedWatts) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for PowerSubsystem.V1_1_0.PowerSubsystem
//
RedfishCS_status CS_To_PowerSubsystem_V1_1_0_JSON(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // Allocation
  if (CS_To_JSON_Allocation(CsJson, "Allocation", CSPtr->Allocation) != RedfishCS_status_success) {goto Error;}

  // Batteries 
  if (InsertJsonLinkObj (CsJson, "Batteries", &CSPtr->Batteries) != RedfishCS_status_success) {goto Error;}

  // CapacityWatts 
  if (InsertJsonInt64Obj (CsJson, "CapacityWatts", CSPtr->CapacityWatts) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for PowerSubsystem.V1_1_0.PowerSubsystem
//
RedfishCS_status DestroyPowerSubsystem_V1_1_0_CS(RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for PowerSubsystem.V1_1_0.PowerSubsystem
//
RedfishCS_status DestroyPowerSubsystem_V1_1_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for PowerSubsystem.V1_1_0.PowerSubsystem
//
RedfishCS_status
Json_PowerSubsystem_V1_1_0_To_CS(RedfishCS_char *JsonRawText, RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "PowerSubsystem", "v1_1_0", "PowerSubsystem", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishPowerSubsystem_V1_1_0_PowerSubsystem_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // Allocation
  Status = GenPowerAllocationCs (Cs, JsonObj, "Allocation", &Cs->Allocation);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Batteries
  InitializeLinkHead (&Cs->Batteries);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "Batteries", Cs->Header.ThisUri, &Cs->Batteries);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // CapacityWatts
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "CapacityWatts", &Cs->CapacityWatts);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroyPowerSubsystem_V1_1_0_CS (Cs);
  return Status;
}
