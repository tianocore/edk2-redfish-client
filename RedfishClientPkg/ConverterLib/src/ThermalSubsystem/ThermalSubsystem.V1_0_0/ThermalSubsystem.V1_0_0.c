...
#include"Redfish_ThermalSubsystem_v1_0_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenOemActionsCs(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishThermalSubsystem_V1_0_0_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishThermalSubsystem_V1_0_0_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishThermalSubsystem_V1_0_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishThermalSubsystem_V1_0_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenidRefCs(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs, json_t *JsonObj, char *Key, Redfishodatav4_idRef_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status GenCondition_Array_ElementCs(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  RedfishResource_Condition_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenCondition_ArrayCs(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Condition_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
...
//
static RedfishCS_status GenStatusCs(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishThermalSubsystem_V1_0_0_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishThermalSubsystem_V1_0_0_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
//
// C structure to JSON for ThermalSubsystem.V1_0_0.ThermalSubsystem
//
RedfishCS_status CS_To_ThermalSubsystem_V1_0_0_JSON(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // FanRedundancy 
  if (InsertJsonLinkArrayObj (CsJson, "FanRedundancy", &CSPtr->FanRedundancy) != RedfishCS_status_success) {goto Error;}

  // Fans 
  if (InsertJsonLinkObj (CsJson, "Fans", &CSPtr->Fans) != RedfishCS_status_success) {goto Error;}
  ...
  // ThermalMetrics 
  if (InsertJsonLinkObj (CsJson, "ThermalMetrics", &CSPtr->ThermalMetrics) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for ThermalSubsystem.V1_0_0.ThermalSubsystem
//
RedfishCS_status DestroyThermalSubsystem_V1_0_0_CS(RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for ThermalSubsystem.V1_0_0.ThermalSubsystem
//
RedfishCS_status DestroyThermalSubsystem_V1_0_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for ThermalSubsystem.V1_0_0.ThermalSubsystem
//
RedfishCS_status
Json_ThermalSubsystem_V1_0_0_To_CS(RedfishCS_char *JsonRawText, RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "ThermalSubsystem", "v1_0_0", "ThermalSubsystem", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishThermalSubsystem_V1_0_0_ThermalSubsystem_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // FanRedundancy
  InitializeLinkHead (&Cs->FanRedundancy);
  Status = CreateCsUriOrJsonByNodeArray (Cs, JsonObj, "FanRedundancy", Cs->Header.ThisUri, &Cs->FanRedundancy);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Fans
  InitializeLinkHead (&Cs->Fans);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "Fans", Cs->Header.ThisUri, &Cs->Fans);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // ThermalMetrics
  InitializeLinkHead (&Cs->ThermalMetrics);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "ThermalMetrics", Cs->Header.ThisUri, &Cs->ThermalMetrics);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroyThermalSubsystem_V1_0_0_CS (Cs);
  return Status;
}
