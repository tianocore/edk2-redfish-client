...
#include"Redfish_FanCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishFanCollection_FanCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for FanCollection.FanCollection
//
RedfishCS_status CS_To_FanCollection_JSON(RedfishFanCollection_FanCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for FanCollection.FanCollection
//
RedfishCS_status DestroyFanCollection_CS(RedfishFanCollection_FanCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for FanCollection.FanCollection
//
RedfishCS_status DestroyFanCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for FanCollection.NOVERSIONED.FanCollection
//
RedfishCS_status
Json_FanCollection_To_CS(RedfishCS_char *JsonRawText, RedfishFanCollection_FanCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishFanCollection_FanCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "FanCollection", "noversioned", "FanCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishFanCollection_FanCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
