...
#include"Redfish_ControlCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishControlCollection_ControlCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for ControlCollection.ControlCollection
//
RedfishCS_status CS_To_ControlCollection_JSON(RedfishControlCollection_ControlCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for ControlCollection.ControlCollection
//
RedfishCS_status DestroyControlCollection_CS(RedfishControlCollection_ControlCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for ControlCollection.ControlCollection
//
RedfishCS_status DestroyControlCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for ControlCollection.NOVERSIONED.ControlCollection
//
RedfishCS_status
Json_ControlCollection_To_CS(RedfishCS_char *JsonRawText, RedfishControlCollection_ControlCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishControlCollection_ControlCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "ControlCollection", "noversioned", "ControlCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishControlCollection_ControlCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyControlCollection_CS (Cs);
  return Status;
}
