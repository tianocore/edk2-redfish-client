...
#include"Redfish_Cable_v1_2_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenOemActionsCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, char *Key, RedfishCable_V1_2_0_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishCable_V1_2_0_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, char *Key, RedfishCable_V1_2_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishCable_V1_2_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
static RedfishCS_status GenLinksCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, char *Key, RedfishCable_V1_2_0_Links_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishCable_V1_2_0_Links_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // DownstreamChassis
  InitializeLinkHead (&(*Dst)->DownstreamChassis);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "DownstreamChassis", Cs->Header.ThisUri, &(*Dst)->DownstreamChassis);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // DownstreamChassis@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "DownstreamChassis@odata.count", &(*Dst)->DownstreamChassisodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // DownstreamPorts
  InitializeLinkHead (&(*Dst)->DownstreamPorts);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "DownstreamPorts", Cs->Header.ThisUri, &(*Dst)->DownstreamPorts);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // DownstreamPorts@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "DownstreamPorts@odata.count", &(*Dst)->DownstreamPortsodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // DownstreamResources
  InitializeLinkHead (&(*Dst)->DownstreamResources);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "DownstreamResources", Cs->Header.ThisUri, &(*Dst)->DownstreamResources);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // DownstreamResources@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "DownstreamResources@odata.count", &(*Dst)->DownstreamResourcesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // UpstreamChassis
  InitializeLinkHead (&(*Dst)->UpstreamChassis);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "UpstreamChassis", Cs->Header.ThisUri, &(*Dst)->UpstreamChassis);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // UpstreamChassis@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "UpstreamChassis@odata.count", &(*Dst)->UpstreamChassisodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // UpstreamPorts
  InitializeLinkHead (&(*Dst)->UpstreamPorts);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "UpstreamPorts", Cs->Header.ThisUri, &(*Dst)->UpstreamPorts);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // UpstreamPorts@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "UpstreamPorts@odata.count", &(*Dst)->UpstreamPortsodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // UpstreamResources
  InitializeLinkHead (&(*Dst)->UpstreamResources);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "UpstreamResources", Cs->Header.ThisUri, &(*Dst)->UpstreamResources);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // UpstreamResources@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "UpstreamResources@odata.count", &(*Dst)->UpstreamResourcesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
static RedfishCS_status GenidRefCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, char *Key, Redfishodatav4_idRef_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status GenCondition_Array_ElementCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  RedfishResource_Condition_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenCondition_ArrayCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Condition_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
...
//
static RedfishCS_status GenStatusCs(RedfishCable_V1_2_0_Cable_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishCable_V1_2_0_OemActions_CS *CSPtr)
{
  json_t *CsParentJson;
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishCable_V1_2_0_Actions_CS *CSPtr)
{
  json_t *CsParentJson;
...
static RedfishCS_status CS_To_JSON_Links(json_t *CsJson, char *Key, RedfishCable_V1_2_0_Links_CS *CSPtr)
{
  json_t *CsParentJson;
  ...
  // DownstreamChassis 
  if (InsertJsonLinkArrayObj (CsJson, "DownstreamChassis", &CSPtr->DownstreamChassis) != RedfishCS_status_success) {goto Error;}

  // DownstreamChassis@odata.count 
  if (InsertJsonInt64Obj (CsJson, "DownstreamChassis@odata.count", CSPtr->DownstreamChassisodata_count) != RedfishCS_status_success) {goto Error;}

  // DownstreamPorts 
  if (InsertJsonLinkArrayObj (CsJson, "DownstreamPorts", &CSPtr->DownstreamPorts) != RedfishCS_status_success) {goto Error;}

  // DownstreamPorts@odata.count 
  if (InsertJsonInt64Obj (CsJson, "DownstreamPorts@odata.count", CSPtr->DownstreamPortsodata_count) != RedfishCS_status_success) {goto Error;}

  // DownstreamResources 
  if (InsertJsonLinkArrayObj (CsJson, "DownstreamResources", &CSPtr->DownstreamResources) != RedfishCS_status_success) {goto Error;}

  // DownstreamResources@odata.count 
  if (InsertJsonInt64Obj (CsJson, "DownstreamResources@odata.count", CSPtr->DownstreamResourcesodata_count) != RedfishCS_status_success) {goto Error;}

  // Oem
  if (CS_To_JSON_LinksOem(CsJson, "Oem", CSPtr->Oem) != RedfishCS_status_success) {goto Error;}

  // UpstreamChassis 
  if (InsertJsonLinkArrayObj (CsJson, "UpstreamChassis", &CSPtr->UpstreamChassis) != RedfishCS_status_success) {goto Error;}

  // UpstreamChassis@odata.count 
  if (InsertJsonInt64Obj (CsJson, "UpstreamChassis@odata.count", CSPtr->UpstreamChassisodata_count) != RedfishCS_status_success) {goto Error;}

  // UpstreamPorts 
  if (InsertJsonLinkArrayObj (CsJson, "UpstreamPorts", &CSPtr->UpstreamPorts) != RedfishCS_status_success) {goto Error;}

  // UpstreamPorts@odata.count 
  if (InsertJsonInt64Obj (CsJson, "UpstreamPorts@odata.count", CSPtr->UpstreamPortsodata_count) != RedfishCS_status_success) {goto Error;}

  // UpstreamResources 
  if (InsertJsonLinkArrayObj (CsJson, "UpstreamResources", &CSPtr->UpstreamResources) != RedfishCS_status_success) {goto Error;}

  // UpstreamResources@odata.count 
  if (InsertJsonInt64Obj (CsJson, "UpstreamResources@odata.count", CSPtr->UpstreamResourcesodata_count) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for Cable.V1_2_0.Cable
//
RedfishCS_status CS_To_Cable_V1_2_0_JSON(RedfishCable_V1_2_0_Cable_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // CableClass 
  if (InsertJsonStringObj (CsJson, "CableClass", CSPtr->CableClass) != RedfishCS_status_success) {goto Error;}

  // CableStatus 
  if (InsertJsonStringObj (CsJson, "CableStatus", CSPtr->CableStatus) != RedfishCS_status_success) {goto Error;}

  // CableType 
  if (InsertJsonStringObj (CsJson, "CableType", CSPtr->CableType) != RedfishCS_status_success) {goto Error;}

  // Description 
  if (InsertJsonStringObj (CsJson, "Description", CSPtr->Description) != RedfishCS_status_success) {goto Error;}

  // DownstreamConnectorTypes 
  if (InsertJsonStringArrayObj (CsJson, "DownstreamConnectorTypes", CSPtr->DownstreamConnectorTypes) != RedfishCS_status_success) {goto Error;}

  // DownstreamName 
  if (InsertJsonStringObj (CsJson, "DownstreamName", CSPtr->DownstreamName) != RedfishCS_status_success) {goto Error;}

  // Id 
  if (InsertJsonStringObj (CsJson, "Id", CSPtr->Id) != RedfishCS_status_success) {goto Error;}

  // LengthMeters 
  if (InsertJsonInt64Obj (CsJson, "LengthMeters", CSPtr->LengthMeters) != RedfishCS_status_success) {goto Error;}
  ...
  if (InsertJsonStringArrayObj (CsJson, "UpstreamConnectorTypes", CSPtr->UpstreamConnectorTypes) != RedfishCS_status_success) {goto Error;}

  // UpstreamName 
  if (InsertJsonStringObj (CsJson, "UpstreamName", CSPtr->UpstreamName) != RedfishCS_status_success) {goto Error;}

  // UserDescription 
  if (InsertJsonStringObj (CsJson, "UserDescription", CSPtr->UserDescription) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for Cable.V1_2_0.Cable
//
RedfishCS_status DestroyCable_V1_2_0_CS(RedfishCable_V1_2_0_Cable_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for Cable.V1_2_0.Cable
//
RedfishCS_status DestroyCable_V1_2_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for Cable.V1_2_0.Cable
//
RedfishCS_status
Json_Cable_V1_2_0_To_CS(RedfishCS_char *JsonRawText, RedfishCable_V1_2_0_Cable_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishCable_V1_2_0_Cable_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "Cable", "v1_2_0", "Cable", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishCable_V1_2_0_Cable_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // CableClass
  Status = GetRedfishPropertyStr (Cs, JsonObj, "CableClass", &Cs->CableClass);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // CableStatus
  Status = GetRedfishPropertyStr (Cs, JsonObj, "CableStatus", &Cs->CableStatus);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // CableType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "CableType", &Cs->CableType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // DownstreamConnectorTypes
  Status = GenRedfishCS_char_ArrayCs (Cs, JsonObj, "DownstreamConnectorTypes", &Cs->DownstreamConnectorTypes);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // DownstreamName
  Status = GetRedfishPropertyStr (Cs, JsonObj, "DownstreamName", &Cs->DownstreamName);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // LengthMeters
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "LengthMeters", &Cs->LengthMeters);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // UpstreamConnectorTypes
  Status = GenRedfishCS_char_ArrayCs (Cs, JsonObj, "UpstreamConnectorTypes", &Cs->UpstreamConnectorTypes);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // UpstreamName
  Status = GetRedfishPropertyStr (Cs, JsonObj, "UpstreamName", &Cs->UpstreamName);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // UserDescription
  Status = GetRedfishPropertyStr (Cs, JsonObj, "UserDescription", &Cs->UserDescription);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
