...
#include"Redfish_CircuitCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishCircuitCollection_CircuitCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for CircuitCollection.CircuitCollection
//
RedfishCS_status CS_To_CircuitCollection_JSON(RedfishCircuitCollection_CircuitCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for CircuitCollection.CircuitCollection
//
RedfishCS_status DestroyCircuitCollection_CS(RedfishCircuitCollection_CircuitCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for CircuitCollection.CircuitCollection
//
RedfishCS_status DestroyCircuitCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for CircuitCollection.NOVERSIONED.CircuitCollection
//
RedfishCS_status
Json_CircuitCollection_To_CS(RedfishCS_char *JsonRawText, RedfishCircuitCollection_CircuitCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishCircuitCollection_CircuitCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "CircuitCollection", "noversioned", "CircuitCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishCircuitCollection_CircuitCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyCircuitCollection_CS (Cs);
  return Status;
}
