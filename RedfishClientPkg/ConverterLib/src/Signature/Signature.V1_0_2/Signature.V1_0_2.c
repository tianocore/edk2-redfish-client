...
#include"Redfish_Signature_v1_0_2_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemActionsCs(RedfishSignature_V1_0_2_Signature_CS *Cs, json_t *JsonObj, char *Key, RedfishSignature_V1_0_2_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishSignature_V1_0_2_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishSignature_V1_0_2_Signature_CS *Cs, json_t *JsonObj, char *Key, RedfishSignature_V1_0_2_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishSignature_V1_0_2_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//Generate C structure for Oem
//
static RedfishCS_status GenOemCs(RedfishSignature_V1_0_2_Signature_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
  RedfishCS_status Status;
  RedfishCS_Type_JSON_Data *CsTypeJson;
  RedfishCS_Type_EmptyProp_CS_Data *CsTypeEmptyPropCS;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishSignature_V1_0_2_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishSignature_V1_0_2_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
//
// C structure to JSON for Signature.V1_0_2.Signature
//
RedfishCS_status CS_To_Signature_V1_0_2_JSON(RedfishSignature_V1_0_2_Signature_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // SignatureString 
  if (InsertJsonStringObj (CsJson, "SignatureString", CSPtr->SignatureString) != RedfishCS_status_success) {goto Error;}

  // SignatureType 
  if (InsertJsonStringObj (CsJson, "SignatureType", CSPtr->SignatureType) != RedfishCS_status_success) {goto Error;}

  // SignatureTypeRegistry 
  if (InsertJsonStringObj (CsJson, "SignatureTypeRegistry", CSPtr->SignatureTypeRegistry) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for Signature.V1_0_2.Signature
//
RedfishCS_status DestroySignature_V1_0_2_CS(RedfishSignature_V1_0_2_Signature_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for Signature.V1_0_2.Signature
//
RedfishCS_status DestroySignature_V1_0_2_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for Signature.V1_0_2.Signature
//
RedfishCS_status
Json_Signature_V1_0_2_To_CS(RedfishCS_char *JsonRawText, RedfishSignature_V1_0_2_Signature_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishSignature_V1_0_2_Signature_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "Signature", "v1_0_2", "Signature", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishSignature_V1_0_2_Signature_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // SignatureString
  Status = GetRedfishPropertyStr (Cs, JsonObj, "SignatureString", &Cs->SignatureString);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SignatureType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "SignatureType", &Cs->SignatureType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SignatureTypeRegistry
  Status = GetRedfishPropertyStr (Cs, JsonObj, "SignatureTypeRegistry", &Cs->SignatureTypeRegistry);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
