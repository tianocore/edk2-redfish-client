...
#include"Redfish_VCATEntryCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishVCATEntryCollection_VCATEntryCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for VCATEntryCollection.VCATEntryCollection
//
RedfishCS_status CS_To_VCATEntryCollection_JSON(RedfishVCATEntryCollection_VCATEntryCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for VCATEntryCollection.VCATEntryCollection
//
RedfishCS_status DestroyVCATEntryCollection_CS(RedfishVCATEntryCollection_VCATEntryCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for VCATEntryCollection.VCATEntryCollection
//
RedfishCS_status DestroyVCATEntryCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for VCATEntryCollection.NOVERSIONED.VCATEntryCollection
//
RedfishCS_status
Json_VCATEntryCollection_To_CS(RedfishCS_char *JsonRawText, RedfishVCATEntryCollection_VCATEntryCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishVCATEntryCollection_VCATEntryCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "VCATEntryCollection", "noversioned", "VCATEntryCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishVCATEntryCollection_VCATEntryCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyVCATEntryCollection_CS (Cs);
  return Status;
}
